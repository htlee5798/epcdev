 ...
 */
		...
		function doCreate() {
	       ...
	       if(newProdDescTxtVal == "" && newProdDescBodyVal.indexOf('<IMG')=='-1' && newProdDescBodyVal.indexOf('<img')=='-1' && newProdDescBodyVal.toUpperCase().indexOf('<IFRAME')=='-1' && newProdDescBodyVal.toUpperCase().indexOf('<iframe')=='-1'  && newProdDescBodyVal.toUpperCase().indexOf('<EMBED')=='-1' && newProdDescBodyVal.toUpperCase().indexOf('<embed')=='-1'){
			...
			if(form.startDate.value.replace( /-/gi, '' )+form.startTime.value  <= <%=Date%>+String(nowTime)){
	...
	<form name="dataForm" id="dataForm" method="post" enctype="multipart/form-data">
		...
		<div class="popup_contents">
			...
			<div class="bbs_search3">
				<ul class="tit">
					<li class="btn">
						<a href="javascript:void(0)" class="btn" onclick="doCreate()"><span><spring:message code="button.common.save" /></span></a>
				...
				<table class="bbs_grid2" cellpadding="0" cellspacing="0" border="0">
					...
					<tr>
						<th>
							<span class="star">*&nbsp</span>제목
						</th>
						<td colspan="3">
							<input type="text" id="title" name="title" size="88"  onKeyUp="javascript:fnChkByte(this,'200','byteInfo1')" onKeyPress="keyCode(event)" /> <b> (<span id="byteInfo1">0</span>/200Byte) </b> 
						</td>
					</tr>

					<tr>
						<th class="fst">
							<span class="star">*&nbsp</span>공지기간
						</th>
						<td class="text">
						...
						<a href="javascript:void(0)" id="btnStartDate">
						...
						<a href="javascript:void(0)" id="btnEndDate">
						<img src="${lfn:getString('system.cdn.static.path')}/images/epc/layout/btn_cal.gif" alt="absmiddle" class="middle" /></a>
					</td>
					</tr>
