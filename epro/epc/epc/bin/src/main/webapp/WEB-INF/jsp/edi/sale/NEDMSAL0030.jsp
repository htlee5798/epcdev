...
<script>
	...
	function doSearch() {
		...
		if (dateValid()) {
			$.ajax({
				...
				dataType : 'json',
				async : false,
				url : '<c:url value="/edi/sale/NEDMSAL0030Select.json"/>',
	...
	function popupSearch(tbName1, tbName2, tbName3){		
		...
		var bodyValue = "";
		bodyValue += "<CAPTION><spring:message	code='epc.sal.dash.prod' /><br>";
	...
	function forwarding(val) {
		...
		$("#searchForm").attr("action", "<c:url value='/edi/sale/NEDMSAL0040.do'/>");
		$("#searchForm").submit();
	}
	...
	function printText(){
		...
		if(dateValid()){
			...
			$("#searchForm").attr("action", "<c:url value='/edi/sale/NEDMSAL0030Text.do'/>");
			$("#searchForm").submit();
		}	
	}
</script>
...
<script id="dataListTemplate" type="text/x-jquery-tmpl">
	<tr class="r1">
		<td align="center"><a href="javascript:forwarding('<c:out value="\${prodCd}"/>');"><c:out value="\${prodCd}"/></a></td>
				...
				<div class="bbs_list">
					...
					<table class="bbs_list" cellpadding="0" cellspacing="0" border="0" id="testTable3">
						<tr id="sumRow" style="display:none;" class="r1">
							<th align="center" colspan="3"><b><spring:message	code="epc.sal.header.sum" /></b></th>
							<th align="right" width=150><b id="total_qty"></b></th>
							<th align="right" width=150><b id="total_amt"></b></th>
							<th width=17>&nbsp;</th>
						</tr>
						<tr id="emptyRow" style="display:none;"><td colspan=5>&nbsp;</td><th width=17>&nbsp;</th>
						</tr>
					</table>
				</div>
			</div>
		...
		<div id="footbox">
			...
			<div class="location">
				<ul>
					...
					<li class="last"><spring:message	code='epc.sal.perProd' /></li>
				</ul>
			</div>
		</div>
	</div>
