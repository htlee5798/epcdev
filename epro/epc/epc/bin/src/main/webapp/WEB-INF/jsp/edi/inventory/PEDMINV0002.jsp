...
<script>
	...
	function popupSearch(tbName1, tbName2){		
		...
		form.staticTableBodyValue.value = "<CAPTION>재고정보 현재고(상품) 현황표<br>"+
	...
	function printText(){
		...
		if(dateValid(form)){
			...
			form.action  = "<c:url value='/edi/inventory/PEDMINV0002Text.do'/>";
			form.submit();	
	...
	function printText2(){
		...
		if(form.entp_cd.value==""){
			alert("협력업체 코드 전체가 아닌 하나만 선택하세요.");
			return;
		}
		
		if (!confirm("데이터 량에 따라 1분이상 소요 될 수 있습니다.")) {
            return;
        }
		...
		if(dateValid(form)){
			loadingMaskFixPos();
			form.action  = "<c:url value='/edi/inventory/PEDMINV0002Text2.do'/>";
			form.submit();	
				...
				<div class="bbs_search">
					<ul class="tit">
						...
						<li class="btn">
							...
							<!-- a href="#" class="btn" onclick="javascript:alert('공사중입니다.(2/24일부터 사용하세요.)');"><span>점포별 텍스트파일</span></a -->
						</li>
					</ul>
				...
				<div class="bbs_list">
					...
					<c:if test="${not empty inventoryList }">
						...
						<c:forEach items="${inventoryList}" var="list" varStatus="index" >
							...
							<tr class="r1">
								<td rowspan="2" align="center" style="mso-number-format:'\@'">${list.srcmkCd }</td>
								<td rowspan="2" align="center" style="mso-number-format:'\@'">${list.prodCd }</td>
								<td rowspan="2" align="center">${list.prodNm }</td>
