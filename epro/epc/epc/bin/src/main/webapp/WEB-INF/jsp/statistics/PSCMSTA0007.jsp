...
function goPage(pageIndex){
	...
	var url = '<c:url value="/statistics/selectDaumEdmSalesSummaryList.do"/>';
...
function doExcel(){
	...
	var url = '<c:url value="/statistics/selectDaumEdmSalesSummaryListExcel.do"/>';
	f.pageIndex.value = 1;
	f.action = url;
	f.submit();	
				...
				<td>
					...
					<select name="affiliateLinkNo" class="select">
						<c:forEach items="${affiliateLinkNoList}" var="affiliateLinkNo" begin="0">
 							<option value="${affiliateLinkNo.AFFILIATE_LINK_NO}" <c:if test="${affiliateLinkNo.AFFILIATE_LINK_NO == searchVO.affiliateLinkNo}">selected</c:if>>${affiliateLinkNo.AFFILIATE_LINK_NM}</option>
 						</c:forEach>
					</select>
				</td>
			</tr>
