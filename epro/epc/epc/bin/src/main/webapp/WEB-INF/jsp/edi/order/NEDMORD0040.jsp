...
<script>
	...
	function doSearch() {
		...
		if (dateValid()) {
			$.ajax({
				...
				url : '<c:url value="/edi/order/NEDMORD0040Select.json"/>',
				data : JSON.stringify(searchInfo),
				success : function(data) {
					$('#testTable2').html(data);
				}
			});	
		}
	}
	...
	function doExcel(tbName1, tbName2) {
		...
		var bodyValue = "";
		bodyValue += "<CAPTION><spring:message code='epc.ord.orderDashBoardPerStore'/><br>";
	...
	function printText(){
		...
		if (dateValid()) {
			...
			$("#searchForm").attr("action", "<c:url value='/edi/order/NEDMORD0040Text.do'/>");
			$("#searchForm").submit();
		}	
	}
</script>
					...
					<div class="bbs_list">
						...
						<table class="bbs_list" cellpadding="0" cellspacing="0" border="0" id="testTable1">
							...
							<tr>
								...
								<th><spring:message code='epc.ord.buyDan'/></th>
			...
			<div id="footbox">
				...
				<div class="location">
					<ul>
						...
						<li class="last"><spring:message code='epc.ord.perStore'/></li>
					</ul>
				</div>
			</div>
		</div>
