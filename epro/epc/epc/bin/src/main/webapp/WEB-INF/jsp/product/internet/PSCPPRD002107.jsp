...
$(document).ready(function(){
	...
	// Sort: 헤더클릭시 정렬, ColMove: 마우스로 헤더이동, ColResize:컬럼Resize, HeaderCheck: 헤더에 CheckBox 표시
	ibdata.Cols = [
	    {Header:"순번",		        	Type:"Int" ,				SaveName:"NUM", 		        Align:"Center", Width:30, Edit:0, Hidden:true}
	  ...
	  , {Header:"상품승인여부", 	Type:"Text", 			SaveName:"APRV_YN",  			Align:"Center", Width:60, Edit:0}
...
function goPage(currentPage){
	...
	var url = '<c:url value="/product/selectProdMstChgHist.do"/>';
...
// 		var url = '<c:url value="/product/prodAllAprv.do"/>';
// 		param.typeCd =  '004'
// 		param.mode = 'update';
		
		mySheet.DoSave( '<c:url value="/product/prodAllAprv.do"/>', {Quest:0});
}
...
function checkRows()
{
	...
	for(var i = 1; i <= mySheet.GetTotalRows(); i++)
	{
...
function mySheet_OnSearchEnd(cd,msg) {
	...
	if( mySheet.GetTotalRows() > 0 ){
		for(i = 1; i <= mySheet.GetTotalRows(); i++) {
			...
			// 체크박스 비활성화
			if( mySheet.GetCellValue(i,  "APRV_CD") != '002' ){
		...
		<div class="bbs_search">
			...
			<ul class="tit">
				...
				<li class="btn">
					...
					<a href="#" class="btn" id="aprv"><span>재요청</span></a>
				</li>
			</ul>
			<!-------------------------------------------------- end of title -- -->
