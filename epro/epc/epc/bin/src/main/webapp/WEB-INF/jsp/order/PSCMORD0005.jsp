...
function doExcel(){
	...
	if(dateValid(form)){
		form.action = '<c:url value="/order/selectSaleInfoByDateExcel.do"/>';
		form.submit();
	}
}
...
function doSearch() {
	...
	if(dateValid(form)){
		loadingMaskFixPos();
		form.action = "<c:url value='/order/selectSaleInfoByDate.do'/>";
		form.submit();
	}
}
...
function printText(){
	...
	form.action = "<c:url value='/order/selectSaleInfoByDateText.do'/>";
	form.submit();
}
</script>
</head>
		...
		<div class="bbs_list">
			...
			<table class="bbs_list" cellpadding="0" cellspacing="0" border="0" id="testTable2">
				...
				<c:if test="${not empty saleList}">
					...
					<tr class="r1">
						<td align="center"><a href="javascript:forwarding('${result.SALE_DY}');">${result.SALE_DY}</a></td>
