...
<script>
	...
	function doSearch() {
		...
		if(dateValid(form)){
			loadingMaskFixPos();
			form.action  = "<c:url value='/edi/inventory/PEDMINV0021Select.do'/>";
			form.submit();	
		}	
	...
	function productDetailPopup(val1,val2,val3,val4){
		var form = document.forms[0];

		form.p_str_cd.value=val1;
		form.p_occur_dy.value=val2;
		form.p_prod_cd.value=val3;
		form.p_send_fg.value=val4;

		form.target="_blankPop";
		form.action ="<c:url value='/edi/inventory/PEDPINV0021.do'/>";
			...
			<div class="wrap_con">
				...
				<div class="bbs_list">
					...
					<c:forEach items="${inventoryList}" var="list" varStatus="index" >
						<tr class=r1>
							<td align="center">${list.sendDate }</td>
							<td align="center" style="mso-number-format:'\@'">${list.prodCd }</td>
							<td align="left">&nbsp; -<%-- <a href="javascript:productDetailPopup('${list.strCd}','${list.occurdy}','${list.prodCd}','${list.sendFg}');">${list.prodNm}</a> --%></td>
							<td align="center">${list.occurDate }</td>
							<td align="center">${list.strNm }</td>
							<td align="center">${list.badL1Nm }</td>
							<td align="center">${list.badCatNm }</td>
							
							<c:if test="${list.procGb eq false }">
								<td align="center"><input type="checkbox" disabled="true"></td>
							</c:if>
							<c:if test="${list.procGb eq true }">
