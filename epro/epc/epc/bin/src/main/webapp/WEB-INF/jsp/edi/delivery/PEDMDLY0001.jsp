...
<script>
	...
	function doSearch() {
		...
		if(dateValid(form)){
			loadingMaskFixPos();
			form.action  = "<c:url value='/edi/delivery/PEDMDLY0001Select.do'/>";
			form.submit();	
		}	
	...
	function popupSearch(tbName1, tbName2){		
		...
		form.staticTableBodyValue.value = "<CAPTION>배달정보  현황표<br>"+
			...
			<div class="wrap_con">
				...
				<div class="bbs_list">
					...
					<tr>
						<th rowspan="2">접수일</th>
						<th rowspan="2">접수</th>
						<th rowspan="2">접수확인</th>
						<th rowspan="2">배달완료</th>
						<th colspan="3">배달연기</th>
						<th colspan="4">배달취소</th>
						<th rowspan="2">배달진도율</th>
					</tr>
					<tr>
						<th>주소불명</th>
						<th>고객부재</th>
					...
					<c:if test="${not empty deliveryList }">
						<c:forEach items="${deliveryList}" var="list" varStatus="index" >
							<tr class="r1">
								...
								<td align="right"><fmt:formatNumber value="${list.SALE_CNT }" type="number" currencySymbol="" /></td>
								<td align="right"><fmt:formatNumber value="${list.ACCEPT_CF_CNT }" type="number" currencySymbol="" /></td>
								<td align="right"><fmt:formatNumber value="${list.DELI_END_CNT }" type="number" currencySymbol="" /></td>
								<td align="right"><fmt:formatNumber value="${list.UDELI_DELAY_ADDR }" type="number" currencySymbol="" /></td>
								<td align="right"><fmt:formatNumber value="${list.UDELI_DELAY_CUST }" type="number" currencySymbol="" /></td>
								<td align="right"><fmt:formatNumber value="${list.UDELI_DELAY_ELSE }" type="number" currencySymbol="" /></td>
								<td align="right"><fmt:formatNumber value="${list.UDELI_FAIL_INVEN }" type="number" currencySymbol="" /></td>
								<td align="right"><fmt:formatNumber value="${list.UDELI_FAIL_RECV }" type="number" currencySymbol="" /></td>
								<td align="right"><fmt:formatNumber value="${list.UDELI_FAIL_BOARD }" type="number" currencySymbol="" /></td>
								<td align="right"><fmt:formatNumber value="${list.UDELI_FAIL_ELSE }" type="number" currencySymbol="" /></td>
								<td align="right">${list.DELI_END_RATE }</td>
							</tr>
						</c:forEach>
					</c:if>
