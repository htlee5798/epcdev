	...
	<script type="text/javascript">
		$(document).ready(
				function() {
					...
					, { Header : "항목값", Type : "Text", SaveName : "colVal", Align : "Center", Width : 250,  Wrap:1, EditLen: 666 , Edit : 1 }
		...
		function seachDtl() {
			...
			var url = '<c:url value="/system/selectSheet.do"/>';
		...
		function doUpdate() {
			if (confirm("선택항목을 저장하시겠습니까?")) {
				...
				var norProdSeq = $('#norProdSeq').val();
				var url = '<c:url value="/system/updateEscTem.do"/>';
				mySheet.DoAllSave(url, {
													Param:'title='+title+'&useYn='+useYn
													+'&infoGrpCd='+infoGrpCd 
													+'&norProdSeq='+norProdSeq,
													Quest : 0 }); 
			}
...
<body>
	...
	<input  type="hidden" value="${norProdSeq}" name="norProdSeq"  id="norProdSeq"/>
		<div id="popup">
			<!--  @title  -->
			<div id="p_title1">
				<h1>전상법 템플릿관리</h1>
		...
		<div class="popup_contents">
			...
			<div class="bbs_search3">
				<ul class="tit">
					<li class="btn">
						<a href="javascript:void(0)" class="btn" onclick="doUpdate()"><span><spring:message code="button.common.update"/></span></a>
				...
				<table class="bbs_grid2" cellpadding="0" cellspacing="0" border="1">
					...
					<tr>  
						<th>제목</th>
						<td colspan="3"><input type="text" id="title" name="title" size="77" onKeyUp="javascript:fnChkByte(this,'100')" value="${pscmsys0005vo.title}"  onKeyPress="keyCode(event)"/></td>
					</tr>
					<tr>  
						<th>상품유형</th>
						<td colspan="1">
							<select name="infoGrpCd" id="infoGrpCd"
							class="select" onchange="doSearch()" readonly style="width:60%;">
							<option value="${pscmsys0005vo.infoGrpNm}">${pscmsys0005vo.infoGrpNm}</option>
						</select>
						</td>
						<th>사용여부</th>
						<td colspan="1">
						<select id="useYn" name="useYn" class="select" style="width:60%;">
								<option value="Y"  ${pscmsys0005vo.useYn eq 'Y' ? 'selected' : ''} >사용</option>
								<option value="N"  ${pscmsys0005vo.useYn eq 'N' ? 'selected' : ''}>미사용</option>
						</select></td>
					</tr>
				</table>
				<br/>
