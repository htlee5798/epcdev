...
<script>
	...
	function popupSearch(tbName1, tbName2){		
		...
		form.staticTableBodyValue.value = "<CAPTION>미납정보 점포별 현황표<br>"+
					...
					<c:if test="${not empty usplyList }">
						<c:forEach items="${usplyList}" var="list" varStatus="index" >
							<tr class="r1">
								...
								<td align=center><a href="javascript:forwarding('${list.STR_CD }','${list.SPLY_DY }');">${list.STR_NM }</a></td>
								...
								<td align="right">${list.USPLY_BUY_AMT }&nbsp;</td>
							</tr>
						</c:forEach>
					</c:if>
