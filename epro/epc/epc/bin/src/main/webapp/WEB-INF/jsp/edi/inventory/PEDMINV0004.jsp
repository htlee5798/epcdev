...
<script>
	...
	function doSearch() {
		...
		if(dateValid(form)){
			loadingMaskFixPos();
			form.action  = "<c:url value='/edi/inventory/PEDMINV0004Select.do'/>";
			form.submit();		
		}	
	}
	...
	function popupSearch(tbName1, tbName2){		
		...
		form.staticTableBodyValue.value = "<CAPTION>재고정보 센터점출입 상세 현황표<br>"+
	...
	function printText(){
		...
		if(dateValid(form)){
			...
			form.action  = "<c:url value='/edi/inventory/PEDMINV0004Text.do'/>";
			form.submit();	
		}	
				...
				<div class="bbs_list">
					...
					<c:if test="${not empty inventoryList }">
						<c:forEach items="${inventoryList}" var="list" varStatus="index" >
							<tr class="r1">
								<td align="center">${list.srcmkCd }</td>
								<td align="center" style="mso-number-format:'\@'">${list.prodCd }</td>
								<td align="center">${list.prodNm }</td>
