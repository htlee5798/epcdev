...
function goPage(pageIndex){
	...
	var url = '<c:url value="/common/selectPartnerFirmsPopupSSL.do"/>';
	...
	<input type="hidden" name="pwd" value="${searchVO.pwd}"/>
		<div id="popup">
			...
			<c:choose>								
					...
					<c:forEach var="list" items="${list}">
						...
						<c:choose>
							<c:when test="${searchVO.gubn eq 'CONO' }">
								<td align="center"><a href="javascript: popupPushValue2('<c:out value="${list.VENDOR_ID}"/>','<c:out value="${VENDOR_NM}"/>','<c:out value="${list.PARAM_CONO}"/>');">${list.VENDOR_NM}</a></td>
							</c:when>
							<c:otherwise>
								<td align="center"><a href="javascript: popupPushValue('<c:out value="${list.VENDOR_ID}"/>','<c:out value="${list.VENDOR_NM}"/>');">${list.VENDOR_NM}</a></td>
							</c:otherwise>
						</c:choose>
				...
				<!--  2검색내역 // -->
				<div id="paging" style="text-align: center; margin-top: 10px;">
