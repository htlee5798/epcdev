...
function goPage(pageIndex){
	...
	var url = '<c:url value="/statistics/selectEnuriSummaryList.do"/>';
...
function doExcel(){
	...
	var url = '<c:url value="/statistics/selectEnuriSummaryListExcel.do"/>';
	f.pageIndex.value = 1;
	f.action = url;
	f.submit();	
}
...
// 구매상품리스트 팝업
function popupProdInfo(orderId){
	var targetUrl = '<c:url value="/statistics/enuri/orderProdInfo.do"/>'+'?orderId='+orderId;
	Common.centerPopupWindow(targetUrl, 'prdInfo', {width : 600, height : 400, scrollBars:"YES"});
}
				...
				<td>
					...
					<select name="affiliateLinkNo" class="select">
						<c:forEach items="${affiliateLinkNoList}" var="affiliateLinkNo" begin="0">
 							...
 							<c:if test="${affiliateLinkNo.AFFILIATE_LINK_NO == searchVO.affiliateLinkNo}">selected</c:if>>${affiliateLinkNo.AFFILIATE_LINK_NM}</option>
 						</c:forEach>
					</select>
				</td>
	...
	<div class="wrap_con">
		...
		<div class="bbs_list">
			...
			<!--	<td>${result.FIRST_ORDER_ID}</td>
			  -->	<td><a href="javascript:popupProdInfo('${result.ORDER_ID}')">${result.ORDER_ID}</a></td>
