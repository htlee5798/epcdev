...
function _init(){
	...
	if(opener == null || !opener.ordReqNo) {
		alert('선택된 점포 발주번호가 없습니다. 다시 선택하세요!');
		top.close();
		return;
	}
	else {
...
function doSearch(){
	...
	$.post(
			"<c:url value='/edi/weborder/PEDMWEB0010ProdSelect.do'/>",
			JSON.stringify(str),
			function(data){
				if(data == null || data.state !="0"){
					alert("발주 상품상세 조회중 오류가 발생하였습니다.\n[CODE:"+data.state+"]");
				}
...
function doSave(){
	...
	$.ajaxSetup({
 	...
 	$.post("<c:url value='/edi/weborder/PEDMWEB0010ProdDetUpdate.do'/>"
...
function doDelete_BACK(){
	...
	$.ajaxSetup({
 	...
 	$.post("<c:url value='/edi/weborder/PEDMWEB0010ProdDelete.do'/>"
 			,JSON.stringify(str)
  			,function(data){
 				if(data == null || data.state != "0"){
					if(data.state == "1")
	 	...
	 	<div class="popup_contents">
 			...
 			<div class="wrap_con">
				<div class="bbs_list" style="margin-top: 10px;">
					<ul class="tit">
						<li class="tit">점포별 상품상세 발주 정보 </li>
						<li class="btn">
				...
				<!-- 	<div style="width:100%; height:458px;overflow-x:hidden; overflow-y:scroll; overflow-x:scroll;  table-layout:fixed;white-space:nowrap;"> -->
					...
					<table id="dataTable" cellpadding="1" cellspacing="1" border="0" width=100% bgcolor=efefef>
                        ...
                        <thead>
                          ...
                          <th rowspan="2">No.</th>
                          <th rowspan="2">삭제</br><input type="checkbox" id="allCheck" name="allCheck"></th>
