...
<script>
	...
	function doSearch() {
		...
		if(dateValid(form)){
			loadingMaskFixPos();
			form.action  = "<c:url value='/edi/usply/PEDMUSP0003Select.do'/>";
			form.submit();	
		}	
	...
	function popupSearch(tbName1, tbName2){		
		...
		form.staticTableBodyValue.value = "<CAPTION>미납정보상품별 현황표<br>"+
	...
	function forwarding(val,val2) {
		...
		form.srcmkVal.value=val;
		form.startDate.value=val2;
		form.endDate.value=val2;
		
		form.action  = "<c:url value='/edi/usply/PEDMUSP0004Select.do'/>";
		form.submit();	
	}
</script>
		...
		<div id="wrap_menu">
			...
			<div class="wrap_search">
				...
				<div class="bbs_search">
					...
					<input type="hidden" id="srcmkVal" name="srcmkVal" />
					...
					<c:if test="${not empty usplyList }">
						<c:forEach items="${usplyList}" var="list" varStatus="index" >
							<tr class="r1">
								...
								<td align="center" style="mso-number-format:'\@'"><a href="javascript:forwarding('${list.SRCMK_CD }','${list.SPLY_DY }');">${list.SRCMK_CD }</a></td>
								<td>&nbsp;${list.PROD_NM }</td>
