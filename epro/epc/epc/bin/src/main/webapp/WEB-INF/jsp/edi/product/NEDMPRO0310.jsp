...
<script>
	...
	$(document).ready(function() {
		...
		// 계열사 change event!
		$("select[name='vkorg']").change(function() {
		...
		$("select[name='reqDisc']").change(function() {
			...
			if( reqType == '1002' ){
				...
				$.each($("#formTbody tr"), function(idx, item) {
					let reqSalesPrc = $(this).find('input[name=reqSalesPrc]').val().replaceAll(",",""); 	// 제안 판매가
					let salesPrc = $(this).find('input[name=salesPrc]').val().replaceAll(",",""); 			// 기존 판매가
	...
	function rowBtnEvent(gbn){
		...
		if(gbn == 'Add'){
			...
			let checkTdm = $("#formTbody tr").find(".tdm").length > 0;

			// checkTdm 이 있을 때만 초기화 실행
			if (checkTdm) {
				setTbodyInit("formTbody"); // dataList 초기화
			}
	...
	function eventDataInfo(){
		...
		if($("select[name='reqContyp'] option:selected").val() != '00') $("select[name='reqType'] option[value='8001']").remove();
	...
	function fncValChk(obj, event){
		...
		let reqPurPrc = $(obj).closest("tr").find('input[name=reqPurPrc]').val().replaceAll(",",""); 		// 제안 납품가
		let purPrc = $(obj).closest("tr").find('input[name=purPrc]').val().replaceAll(",",""); 				// 기존 납품가
		let reqSalesPrc = $(obj).closest("tr").find('input[name=reqSalesPrc]').val().replaceAll(",",""); 	// 제안 판매가
		let salesPrc = $(obj).closest("tr").find('input[name=salesPrc]').val().replaceAll(",",""); 			// 기존 판매가
		let reqType = $("select[name='reqType'] option:selected").val();
	...
	function btnEvent(gbn, obj){
		// 업로드 양식 다운로드(excelFrom), 판매바코드 찾기(srcBarcode), 행사 테마 조회(theme)
		if(gbn == 'excelFrom'){
			// 업로드 양식 다운로드
			...
			// 판매바코드 찾기
			Common.centerPopupWindow( "<c:url value='/edi/product/selSrcmkCdPopup.do'/>?trNum=" + $(obj).closest('tr').attr('class')
	...
	function btnServerEvent(gbn, obj){
		// 저장(sava), 삭제(del), 행사요청(eventReq), 업로드(excelUpl), 공문서&계약서( ecsIntgr )
	...
	function validation(info){
	    ...
	    // 행사가는 정상가 보다 높을 수 없음
	    $.each($("#formTbody tr"), function(idx, item) {
	    	let purPrc = $(this).find('input[name=purPrc]').val().replaceAll(",","");			// 정상 원가
	    	let reqPurPrc = $(this).find('input[name=reqPurPrc]').val().replaceAll(",","");		// 행사 원가
	    	let salesPrc = $(this).find('input[name=salesPrc]').val().replaceAll(",","");		// 정상 판매가
	    	let reqSalesPrc = $(this).find('input[name=reqSalesPrc]').val().replaceAll(",","");	// 행사 판매가
	    	if( purPrc < reqPurPrc ) purChk = false;
	    	if( salesPrc < reqSalesPrc ) salesChk = false;
		});
	    
	    if(!purChk){
	    	alert('행사 원가는 정상 원가보다 높을 수 없습니다.');
	    	return false;
	    }
	    
	    if(!salesChk){
	    	alert('행사 판매가는 정상 판매가보다 높을 수 없습니다.');
	    	return false;
	    }
...
<tr class="tr\${trCount}"  bgcolor=ffffff>
	...
	<td align="center"><input type="text" name="ean11" value="" style="width: 85%;" disabled/><i class="bi bi-search" style='margin-left: 5px; cursor: pointer;' onclick='btnEvent("srcBarcode", this)' ></i>
	<td align="center"><input type="text" name="maktx" value="" style="width: 99%;" disabled/></td>
	<td align="center" class="from1 calPurPrc"></td>
	<td align="center" class="from1 calComRate"></td>
	<td align="center" class="from1"><input type="text" name="purPrc" maxlength="17" value="" style="width: 99%;" maxlength="17" onkeyup="return fncValChk(this, event)" oninput="this.value = this.value.replace(/[^0-9.]/g, '').replace(/(\..*)\./g, '$1').replace(/(\.\d{0}).+/g, '$1').replace(/\B(?=(\d{3})+(?!\d))/g, ',');" /></td>	
	<td align="center" class="from1"><input type="text" name="reqPurPrc" maxlength="17" value="" style="width: 99%;" maxlength="17" onkeyup="return fncValChk(this, event)" oninput="this.value = this.value.replace(/[^0-9.]/g, '').replace(/(\..*)\./g, '$1').replace(/(\.\d{0}).+/g, '$1').replace(/\B(?=(\d{3})+(?!\d))/g, ',');" /></td>
	<td align="center" class="from5 ofrStd">해당 상품 동시구매시</td>
	<td align="center" class="from2 calSalesPrc"></td>
	<td align="center" class="from2 calSalesRate"></td>
	<td align="center" class="from2"><input type="text" name="salesPrc" maxlength="17" value="" style="width: 99%;" maxlength="17" onkeyup="return fncValChk(this, event)" oninput="this.value = this.value.replace(/[^0-9.]/g, '').replace(/(\..*)\./g, '$1').replace(/(\.\d{0}).+/g, '$1').replace(/\B(?=(\d{3})+(?!\d))/g, ',');"/></td>
	<td align="center" class="from2"><input type="text" name="reqSalesPrc" maxlength="17" value="" style="width: 99%;" maxlength="17" onkeyup="return fncValChk(this, event)" oninput="this.value = this.value.replace(/[^0-9.]/g, '').replace(/(\..*)\./g, '$1').replace(/(\.\d{0}).+/g, '$1').replace(/\B(?=(\d{3})+(?!\d))/g, ',');"/></td>
	<td align="center" class="from6"><input type="text" name="ofrStd" maxlength="17" value="" style="width: 99%;" oninput="this.value = this.value.replace(/[^0-9.]/g, '').replace(/(\..*)\./g, '$1').replace(/(\.\d{0}).+/g, '$1').replace(/\B(?=(\d{3})+(?!\d))/g, ',');" /></td>
	<td align="center" class="from6"><input type="text" name="giftAmt" maxlength="17" value="" style="width: 99%;" oninput="this.value = this.value.replace(/[^0-9.]/g, '').replace(/(\..*)\./g, '$1').replace(/(\.\d{0}).+/g, '$1').replace(/\B(?=(\d{3})+(?!\d))/g, ',');" /></td>
	<td align="center" class="from4"><input type="text" name="ofrM" maxlength="2" value="" style="width: 99%;" oninput="this.value = this.value.replace(/[^0-9.]/g, '').replace(/(\..*)\./g, '$1').replace(/(\.\d{2}).+/g, '$1');" /></td>
	<td align="center" class="from4"><input type="text" name="ofrN" maxlength="2" value="" style="width: 99%;" oninput="this.value = this.value.replace(/[^0-9.]/g, '').replace(/(\..*)\./g, '$1').replace(/(\.\d{2}).+/g, '$1');" /></td>
	<td align="center" class="from3"><input type="text" name="ofrStd1" maxlength="17" value="" style="width: 99%;" oninput="this.value = this.value.replace(/[^0-9.]/g, '').replace(/(\..*)\./g, '$1').replace(/(\.\d{0}).+/g, '$1').replace(/\B(?=(\d{3})+(?!\d))/g, ',');"/></td>
	<td align="center" class="from3"><input type="text" name="ofrDisc1" maxlength="17" value="" style="width: 99%;" oninput="this.value = this.value.replace(/[^0-9.]/g, '').replace(/(\..*)\./g, '$1').replace(/(\.\d{0}).+/g, '$1').replace(/\B(?=(\d{3})+(?!\d))/g, ',');" /></td>
	<td align="center" class="from3"><input type="text" name="ofrStd2" maxlength="17" value="" style="width: 99%;" oninput="this.value = this.value.replace(/[^0-9.]/g, '').replace(/(\..*)\./g, '$1').replace(/(\.\d{0}).+/g, '$1').replace(/\B(?=(\d{3})+(?!\d))/g, ',');"/></td>
	<td align="center" class="from3"><input type="text" name="ofrDisc2" maxlength="17" value="" style="width: 99%;" oninput="this.value = this.value.replace(/[^0-9.]/g, '').replace(/(\..*)\./g, '$1').replace(/(\.\d{0}).+/g, '$1').replace(/\B(?=(\d{3})+(?!\d))/g, ',');" /></td>
	<td align="center" class="from3"><input type="text" name="ofrStd3" maxlength="17" value="" style="width: 99%;" oninput="this.value = this.value.replace(/[^0-9.]/g, '').replace(/(\..*)\./g, '$1').replace(/(\.\d{0}).+/g, '$1').replace(/\B(?=(\d{3})+(?!\d))/g, ',');"/></td>
	<td align="center" class="from3"><input type="text" name="ofrDisc3" maxlength="17" value="" style="width: 99%;" oninput="this.value = this.value.replace(/[^0-9.]/g, '').replace(/(\..*)\./g, '$1').replace(/(\.\d{0}).+/g, '$1').replace(/\B(?=(\d{3})+(?!\d))/g, ',');" /></td>
	<td align="center" class="venRateTh"><input type="text" name="venRate" maxlength="3" value="" style="width: 99%;" oninput="this.value = this.value.replace(/[^0-9.]/g, '').replace(/(\..*)\./g, '$1').replace(/(\.\d{2}).+/g, '$1');" /></td>
	<td align="center"><input type="text" name="reqItemDesc" maxlength="80" value="" style="width: 99%;"/></td>
	<td align="center"><input type="text" name="zbigo" maxlength="80" value="" style="width: 99%;"/></td>
					...
					<div class="bbs_search">
						<ul class="tit">
							<li class="btn">
								...
								<c:if test="${ proEventInfo.apprStatus == '03' && proEventInfo.vkorg == 'KR02' }">
									<a href="#" class="btn" onclick="btnServerEvent('ecsIntgrMt');"><span>공문서&계약서 작성</span></a>
								</c:if>
								<c:if test="${ proEventInfo.apprStatus == '03' && proEventInfo.vkorg == 'KR04' }">
							...
							<table class="bbs_grid3" cellpadding="0" cellspacing="0"
								...
								<tr>
									...
									<td colspan="3">
										...
										<input type="text" name="subTxt" id="subTxt" value="<c:out value='${proEventInfo.subTxt}'/>" style="width: 36%;"/>
										<i class="bi bi-search" style='margin-left: 5px; cursor: pointer;' onclick='btnEvent("theme", this)' ></i>
									</td>
								</tr>
								<tr>
									<th><b><span class="star">*</span> 파트너사 행사코드</b></th>
									<td>
								...
								<tr>
									...
									<td>
										<input type="text" name="reqOfrTxt" id="reqOfrTxt" style="width: 98%;" value="<c:out value='${proEventInfo.reqOfrTxt}'/>"  />
									</td>
									<th><b><span class="star">*</span> 행사목적</b></th>
									<td>
										<input type="text" name="reqPurTxt" id="reqPurTxt" style="width: 98%;" value="<c:out value='${proEventInfo.reqPurTxt}'/>"  />
									</td>
								</tr>
								<tr class = "fromTd">
								...
								<tr class = "fromTd">
									...
									<td>
										<select id="costType" name="costType" class="required" style="width:150px;">
											...
											<!-- <option value="04" >수수료율 인하</option> -->
										</select>
								...
								<tr class = "fromTd">
									...
									<td>
										<input type="text" name="hdVenRate" id="hdVenRate" style="width: 98%;" value="<c:out value='${proEventInfo.hdVenRate}'/>" maxlength="3" oninput="this.value = this.value.replace(/[^0-9.]/g, '').replace(/(\..*)\./g, '$1').replace(/(\.\d{0}).+/g, '$1');"  />
									</td>
									<th><b><span class="star">*</span> 예상판촉비용</b></th>
									<td>
										<input type="text" name="ofrCost" id="ofrCost" style="width: 98%;" value="<c:out value='${proEventInfo.ofrCost}'/>" oninput="this.value = this.value.replace(/[^0-9.]/g, '').replace(/(\..*)\./g, '$1').replace(/(\.\d{0}).+/g, '$1').replace(/\B(?=(\d{3})+(?!\d))/g, ',');" />
									</td>
								</tr>
								<tr>
								...
								<tr>
									...
									<td colspan="3">
										<input type="text" name="reqOfrDesc" id="reqOfrDesc" style="width: 80%;" value="<c:out value='${proEventInfo.reqOfrDesc}'/>" />
								...
								<ul class="tit">
									...
									<li class="btn">
										<c:if test="${ empty proEventInfo.apprStatus || proEventInfo.apprStatus == '01' }">
											...
											<a href="#" class="btn" onclick="btnServerEvent('excelUpl');"><span><spring:message code="button.rep.product.excelUpload"/></span></a>
										</c:if>
									</li>
								</ul>
