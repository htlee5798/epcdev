...
// 		if(dateValid(form)){
			loadingMaskFixPos();
			form.action  = "<c:url value='/talk/orderTab/search.do'/>";
			form.submit();	
// 		}	
	//	hideLoadingMask();
	}
	...
	<div>
		...
		<div id="wrap_menu">
			...
			<div class="wrap_con">
				...
				<div class="bbs_list">
					<ul class="tit">
						<li class="tit">검색내역 - 2주간 주문내역을 조회합니다</li>
						<li class="btn">
					...
					<c:if test="${not empty orderList }">
						<c:forEach items="${orderList}" var="list" varStatus="index" >
							<tr class="r1">
								...
								<td align="center">${list.ORDER_ID}</td>
								<td align="center">${list.ORD_RTN_DIVN_CD_NM}</td>
								<td align="center">${list.ORD_STS_CD_NM}</td>
								<td align="center">${list.STR_CD_NM}</td>
								<td align="center">${list.ORD_DATE}</td>
								<td align="center">${list.HOPE_DELI_DATE}</td>
								<td align="center">${list.DELI_SEQNC}</td>
								<td align="right"><fmt:formatNumber value="${list.ORD_AMT}" type="number" currencySymbol="" /></td>
								<td align="right"><fmt:formatNumber value="${list.DPST_AMT}" type="number" currencySymbol="" /></td>
								<td align="center">${list.ORDER_PATH_CD}</td>
							</tr>
						</c:forEach>
					</c:if>
