...
<script>
	...
	$(document).ready(function($) {
		...
		//alert("<c:out value='${param.entp_cd}'/>");
	...
	function doSearch() {
		...
		if(dateValid(form)){
			loadingMaskFixPos();
			form.action  = "<c:url value='/edi/buy/PEDMBUY0003Select.do'/>";
			form.submit();	
		}	
	...
	function popupSearch(tbName1, tbName2, tbName3){		
		...
		form.staticTableBodyValue.value = "<CAPTION>매입정보 상품별 현황표<br>"+
	...
	function forwarding(val) {
		...
		form.action  = "<c:url value='/edi/buy/PEDMBUY0004Select.do'/>";
		form.submit();	
	}
	...
	function forwarding2(val,val2) {
		...
		form.action  = "<c:url value='/edi/buy/PEDMBUY0006Select.do'/>";
		form.submit();	
	}
</script>
				...
				<div class="wrap_con">
					...
					<div class="bbs_list">
						...
						<c:if test="${not empty buyList }">
							...
							<c:forEach items="${buyList}" var="list" varStatus="index" >
								<tr class="r1">
									...
									<td align="center"><a href="javascript:forwarding2('${list.PROD_CD }','${list.BUY_DY }');">${list.PROD_CD }</a></td>
						...
						<table class="bbs_list" cellpadding="0" cellspacing="0" border="0" id="testTable3">	
							...
							<c:if test="${not empty buyList }">
							<tr>
								<th colspan="6" class="fst" align=center><b>합계</b></th>
							...
							<c:if test="${empty buyList }"><tr><td colspan=12>&nbsp;</td><th width=17>&nbsp;</th></tr></c:if>
						</table>	
