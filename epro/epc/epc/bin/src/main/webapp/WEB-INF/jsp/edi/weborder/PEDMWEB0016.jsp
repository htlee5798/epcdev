...
/* 초기 설정 */
function _init(){
	_setTbodyNoResult($("#datalist"), 	11, '반품요청일자, 협력업체코드, 지역/점포, 상품 코드를 입력 또는 선택 후 [검색] 하세요!');	// prod tBody 설정
}	
...
function caldate(day){
 ...
 function doSearch(){
	...
	$.post(
			"<c:url value='/edi/weborder/PEDMWEB0016Select.do'/>",
			JSON.stringify(str),
			function(data){
				if(data == null || data.state != "0"){
					alert("자료검색중 오류가 발생하였습니다. [CODE:"+data.state+"]");				// 검색중 Exception 발생(data.message 로 확인가능) 
					hideLoadingMask();
					return;
				}else{
		...
		<div id="wrap_menu">
			...
			<div class="wrap_search">
				...
				<div class="bbs_search">
					...
					<tr>
						<th><span class="star">*</span> 반품요청일자 </th>
						<td >
					 ...
					 <td><input type="text" 	 id="vend_cd" 	name="vend_cd" 	maxlength="6" style="width: 50px; " onkeypress="onlyNumber();">
