...
#content_wrap{
	...
	<script type="text/javascript" >
		...
		function _eventSetTbodyValue(json) {
		    ...
		    if (data.length > 0) {
		        ...
		        for (var i = 0; i < data.length; i++) {
		        	...
		        	//승인된 값은 수정 불가 
		        	if(data[i].prdtStatus === "2"){
		        		$("#chkPropStore" + data[i].rnum).prop("disabled", true);
		        	}else if(data[i].prdtStatus === "1"){
						//반려건은 값 수정 가능 
		            ...
		            if(data[i].teamCd != null && data[i].teamCd != ""){
		            	if(data[i].prdtReqYn == "Y" || data[i].prdtStatus == '2'){
		...
		function _eventAddRow(){
		    ...
		    <c:forEach var="item" items="${frzList}">
			 ...
			 $("#dataListTemplate").tmpl(newRowData).appendTo("#dataListbody");
				...
				// 셀렉트 박스 동적 생성 (상온구분 셀렉트 박스)
				var frzSelect = lastRow.find("select[name='frzCd']");
	        	
				frzSelect.append('<option value="">선택</option>');
	        	
	            for(var i =0 ; i<frzList.length; i++){
	            	frzSelect.append('<option value='+frzList[i].frzCd+'>'+frzList[i].frzNm+'</option>');
			    }
		...
		function _eventDeleteRow(){
			...
			var rowDeleteOk = true;
			
			 // 선택된 행 삭제
		    $("input[name='chkPropStore']:checked").each(function () {
		        ...
		        if(rowDeleteOk == false){
					 alert("저장된 데이터가 존재 합니다.\n저장되지 않은 데이터만 행삭제가 가능합니다.");
					return false;
				 }else{
					//저장되지않은 행 삭제 
		...
		function _eventDelete(){
		    ...
		    $("input[name='chkPropStore']:checked").each(function () {
		        ...
		        row.find('input, select').each(function () {
		        	 ...
		        	 var value = $(this).val();
			          ...
			          if (name === "norProdPcost" || name === "prodSellPrc") value = value.replace(/,/g, '');
			          if (name === "delvPsbtDd") value = value.replace(/-/g, '');
		        	
		        	 rowData[name] = value;
		        });
		        
		        if(rowData.prdtStatus != "0"){
		        	flag = false;
		        }else{
		    ...
		    if(flag == false){
		    	alert("파트너사 등록상태가 아닌 선택 건이 존재 합니다.");
		    	return false;
			}
		...
		function _eventRequest(){
		    ...
		    if(flag == false){
		    	alert("파트너사 등록 및 반려 상태가 아닌 선택 건이 존재 합니다. ");
		    	return false;
			}
		    
		    if(!confirm('입점 제안을 요청 하시겠습니까?')){
				return;
			}
		    ...
		    $.ajax({
				...
				success: function (data) {
			            if (data.msg === "SUCCESS") {
			                alert("신규 입점 상품이 정상적으로 제안 요청 되었습니다.");
			                _eventSearch();
			            }
			        },
			        error: function () {
		...
		function excelUploadcallBack(data){
			 ...
			 for (var i = 0; i < list.length; i++) {
			        var newRowData = {};
				        ...
				        newRowData.inrcntQty = list[i].data11 || ""; // 입수
				        ...
				        newRowData.trgtSaleCurr = list[i].data16 || ""; // 목표매출
			            ...
			            for (var j = 0; j < frzList.length; j++) {
			            	if (newRowData.frzCd == frzList[j].frzCd) {
			            		frzSelect.append('<option value="' + frzList[j].frzCd + '" selected>' + frzList[j].frzNm + '</option>');
				            } else {
				            	frzSelect.append('<option value="' + frzList[j].frzCd + '">' + frzList[j].frzNm + '</option>');
				            }
	...
	<script id="dataListTemplate" type="text/x-jquery-tmpl">
		<tr class="r1" bgcolor=ffffff>
			...
			{%else%}
				<input type="text" name="prodShortNm" id="prodShortNm\${rnum}"  value="<c:out value="\${prodShortNm}"/>" />
			{%/if%}
			</td>
			<td align="center">
			{%if prdtReqYn == "Y" && (prdtStatus == '2' || prdtStatus == '0')%}
			...
			{%else%}
					<input type="text" name="prodStandardNm" id="prodStandardNm\${rnum}"  value="<c:out value="\${prodStandardNm}"/>" />
			{%/if%}
			</td>
			<td align="center">
			{%if prdtReqYn == "Y" && (prdtStatus == '2' || prdtStatus == '0')%}
			...
			{%else%}
					<input type="text" name="norProdPcost" class="priceNorFiled" id="norProdPcost\${rnum}" maxlength="10" onkeyup="inputNumberFormat(this);"    value="<c:out value="\${norProdPcost}"/>" />
			{%/if%}
			</td>	
			<td align="center">
			{%if prdtReqYn == "Y" && (prdtStatus == '2' || prdtStatus == '0')%}
			...
			{%else%}
					<input type="text" name="prodSellPrc" class="priceSelFiled" id="prodSellPrc\${rnum}" maxlength="10" onkeyup="inputNumberFormat(this);"    value="<c:out value="\${prodSellPrc}"/>" />
			{%/if%}
			</td>
			<td align="center">
			{%if prdtReqYn == "Y" && (prdtStatus == '2' || prdtStatus == '0')%}
			...
			{%else%}
					<input type="text" name="prftRate" id="prftRate\${rnum}" maxlength="10" onkeyup="inputPercentFormat(this);"  value="<c:out value="\${prftRate}"/>" />
			{%/if%}
			</td>
			<td align="center">
			{%if prdtReqYn == "Y" && (prdtStatus == '2' || prdtStatus == '0')%}
			...
			{%else%}
					<select name="norSesnCd" style="width:150px;">
						<option value="1" \${norSesnCd == '1' ? 'selected' : ''}>일반</option>
						<option value="2" \${norSesnCd == '2' ? 'selected' : ''}>시즌</option>
					</select>
			{%/if%}
			...
			{%else%}
					<select name="prdtRt" style="width:150px;">
						<option value="1" \${prdtRt == '1' ? 'selected' : ''}>루트</option>
					</select>
			{%/if%}
			</td>
			...
			{%else%}
				<input type="text" oninput="this.value = this.value.replace(/[^0-9.]/g, '').replace(/(\..*)\./g, '$1');" name="inrcntQty" id="inrcntQty\${rnum}" value="<c:out value="\${inrcntQty}"/>" />
			{%/if%}
			</td>
			<td align="center">
			{%if prdtReqYn == "Y" && (prdtStatus == '2' || prdtStatus == '0')%}
			...
			{%if prdtReqYn == "Y" && (prdtStatus == '2' || prdtStatus == '0')%}
				<c:out value="\${sellCd}"/>
			{%else%}
					<input type="text" name="sellCd" id="sellCd\${rnum}"  placeholder="판매코드를 입력해주세요(없을경우 미입력)"; value="<c:out value="\${sellCd}"/>" />
			{%/if%}
			</td>
			<td align="center">
			{%if prdtReqYn == "Y" && (prdtStatus == '2' || prdtStatus == '0')%}
			...
			{%else%}
				<input type="checkbox" id="sellChanCd\${rnum}_KR02" name="sellChanCd" class="chk" value="KR02">마트
				<input type="checkbox" id="sellChanCd\${rnum}_KR03" name="sellChanCd" class="chk" value="KR03">MAXX
				<input type="checkbox" id="sellChanCd\${rnum}_KR04" name="sellChanCd" class="chk"  value="KR04">슈퍼
				<input type="checkbox" id="sellChanCd\${rnum}_KR09" name="sellChanCd" class="chk" value="KR09">오카도
			{%/if%}
			</td>
			<td align="center">
			{%if prdtReqYn == "Y" && (prdtStatus == '2' || prdtStatus == '0')%}
			...
			{%else%}
				<select name="frzCd" style="width:150px;">
            	</select>
			{%/if%}
			</td>
			<td align="center">
			...
			{%else%}
					<input type="text" name="trgtSaleCurr" id="trgtSaleCurr\${rnum}"  value="<c:out value="\${trgtSaleCurr}"/>" />
			{%/if%}
			</td>
			<td align="center">
			...
			{%else%}
				{%if imgId == "" || imgId == null%}
					...
					<a href="javascript:imgFilePreView(\${imgId},\${imgfileUrl});" class="btn" name="fileNameLink" id="fileNameLink\${rnum}" target="_blank" style="display: none;"></a>
            		<input type="file"  name="prodImgInput"  id="prodImgInput\${rnum}" class="prodImgInput" style="display: none;" accept=".jpg">
            		<input type="hidden" name="imgfileUrl" id="imgfileUrl\${rnum}" value="<c:out value="\${imgfileUrl}" />" />
						...
						<div class="bbs_list">
							<ul class="tit">
								...
								<li class="btn">
									...
									<a href="javascript:void(0);" class="btn" onclick="_eventDeleteRow()"><span>행삭제</span></a>	
									<a href="javascript:void(0);" class="btn" onclick="_eventSave()"><span>저장</span></a>		
									<a href="javascript:void(0);" class="btn" onclick="_eventDelete()"><span>삭제</span></a>		
								</li>
							</ul>
