<%--
	Page Name 	: NEDMINVO0030.jsp
	Description : 재고정보 센터 점출입 조회 페이지 
...
<script id="dataListTemplate" type="text/x-jquery-tmpl">
	<tr class="r1">
		...
		<td align="center"><c:out value="\${strioQty }"/></td>
		<td align="center"><c:out value="\${strioSaleAmt }"/></td>
	</tr>
</script>
<script>
	...
	function doSearch() {
		...
		if(dateValid()){
			$.ajax({
				...
				dataType : 'json',
				async : false,
				url : '<c:url value="/edi/inventory/NEDMINV0030Select.json"/>',
	...
	function doExcel(tbName1, tbName2){		
		...
		var bodyValue = "";
		bodyValue += "<CAPTION><spring:message code='text.inv.field.invCtrCdList'/><br>";
	...
	function printText(){
		...
		if(dateValid()){
			$("#searchForm").attr("action", "<c:url value='/edi/inventory/NEDMINV0030Text.do'/>");
			$("#searchForm").submit();
		}	
			...
			<div class="wrap_con">
				...
				<div class="bbs_list">
					...
					<tr>
						...
						<th><spring:message	code="epc.inv.header.strioSaleAmt" /></th>
						<th>&nbsp;</th>
					</tr>
					</table>
		...
		<div id="footbox">
			...
			<div class="location">
				<ul>
					...
					<li class="last"><spring:message code="epc.inv.menu.ctrCd" /></li>
				</ul>
			</div>
		</div>
	</div>
