...
<script>
	...
	function doSearch() {
		...
		$.ajax({
			...
			url : '<c:url value="/edi/payment/NEDMPAY0060Select.json"/>',
		...
		function _setTbodyMasterValue(json) { 
			...
			if (data.length > 0) {
				...
				for (var i = 0; i < data.length; i++) {
					alert(data[i]["buyBuyAmt"]);
					var buyBuyAmt = data[i]["buyBuyAmt"];
					var pbBuyAmt = data[i]["pbBuyAmt"];
					var expromoBuyAmt = data[i]["expromoBuyAmt"];
					var promoAdjAmt = data[i]["promoAdjAmt"];
					var promoAmt = data[i]["promoAmt"];
					var promoAmtRt = data[i]["promoAmtRt"];
					var manualPromoAmt= data[i]["manualPromoAmt"]; 
					var sumAmt = data[i]["sumAmt"];
					var manualRt = data[i]["manualRt"];
					
					// Comma 설정
					data[i]["buyBuyAmt"]= setComma(buyBuyAmt);
					data[i]["pbBuyAmt"]= setComma(pbBuyAmt);
					data[i]["expromoBuyAmt"]= setComma(expromoBuyAmt);
					data[i]["promoAdjAmt"]= setComma(promoAdjAmt);
					data[i]["promoAmt"]= setComma(promoAmt);
					data[i]["promoAmtRt"]= setComma(promoAmtRt);
					data[i]["manualPromoAmt"]= setComma(manualPromoAmt); 
					data[i]["sumAmt"]= setComma(sumAmt);
					data[i]["manualRt"]= setComma(manualRt);
...
<tr class="r1">
	<td align="center"><c:out value="\${subYm}"/></td>
	<td align="center"><c:out value="\${strNm}"/></td>
	<td align=right><c:out value="\${buyBuyAmt}"/></td>
	<td align=right><c:out value="\${pbBuyAmt}"/></td>
	<td align=right><c:out value="\${expromoBuyAmt}"/></td>
	<td align=right><c:out value="\${promoAdjAmt}"/></td>
	<td align=right><c:out value="\${promoAmt}"/></td>
	<td align=right><c:out value="\${promoAmtRt}"/>%</td>
	<td align=right><c:out value="\${manualPromoAmt}"/></td>
	<td align=right><c:out value="\${sumAmt}"/></td>
	<td align=right><c:out value="\${manualRt}"/>%</td>
</tr>
</script> 
		...
		<div id="wrap_menu">
			...
			<div class="wrap_search">
				...
				<div class="bbs_search">
					<ul class="tit">
						<li class="tit"><spring:message code='epc.pay.search.condition'/> &nbsp;<font color="red"><spring:message code='epc.pay.search.oldPage'/></font></li>
						<li class="btn">
			...
			<div class="wrap_con">
				...
				<div class="bbs_list">
					...
					<tr>
						<th rowspan="2"><spring:message code='epc.pay.header.subMm'/></th>
						<th rowspan="2"><spring:message code='epc.pay.header.strNm'/></th>
						<th colspan="4"><spring:message code='epc.pay.header.thisMonthBuy'/></th>
						<th colspan="2"><spring:message code='epc.pay.header.junsanIncentive'/></th>
						<th><spring:message code='epc.pay.header.sugiIncentive'/></th>
						<th colspan="2"><spring:message code='epc.pay.header.totalSum'/></th>
					</tr>
					<tr>
						<th><spring:message code='epc.pay.header.hCode1'/><br><spring:message code='epc.pay.header.hCode2'/></th>
						<th><spring:message code='epc.pay.header.hCode3'/><br><spring:message code='epc.pay.header.hCode4'/></th>
						<th><spring:message code='epc.pay.header.hCode5'/><br><spring:message code='epc.pay.header.hCode4'/></th>
						<th><spring:message code='epc.pay.header.hCode6'/></th>
						<th><spring:message code='epc.pay.header.hCode7'/></th>
						<th><spring:message code='epc.pay.header.hCode8'/></th>
						<th><spring:message code='epc.pay.header.hCode7'/></th>
						<th><spring:message code='epc.pay.header.hCode7'/></th>
						<th><spring:message code='epc.pay.header.hCode9'/><br><spring:message code='epc.pay.header.hCode8'/></th>
					</tr>
					</table>
