...
<script>
	...
	function popupSearch(tbName1, tbName2, tbName3){		
		...
		form.staticTableBodyValue.value = "<CAPTION>매입정보 전표별 현황표<br>"+
					...
					<div class="bbs_list">
						...
						<c:if test="${not empty buyList }">
							...
							<c:forEach items="${buyList}" var="list" varStatus="index" >
								<tr class="r1">
									<td align="center">${list.BUY_DY }</td>
									<td align="center"><a href="javascript:forwarding('${list.PROD_CD }','${list.BUY_DY }');">${list.STR_NM }</a></td>
						...
						<table class="bbs_list" cellpadding="0" cellspacing="0" border="0" id="testTable3">
							...
							<c:if test="${empty buyList }"><tr><td colspan=9>&nbsp;</td><th width=17>&nbsp;</th></tr></c:if>
						</table>
					</div>
				</div>
			</div>
