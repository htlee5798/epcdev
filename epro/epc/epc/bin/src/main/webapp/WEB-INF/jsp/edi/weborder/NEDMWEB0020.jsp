...
<script  type="text/javascript" >
	$(function() {
		...
		$('#areaCd,, #strCd, #entpCd, #prodCd, #prodNm, input[name=workGb], input[name=pageRowCount]').unbind().keydown(function(e) {
			switch(e.which) {
	...
	/* 초기 설정 */
	function _init(){
		_setTbodyNoResult($("#datalistProd"), 	7, '<spring:message	code="text.web.field.srchInit" />');	// prod tBody 설정
		_setTbodyNoResult($("#datalistStr"),    7, '<spring:message	code="text.web.field.srchProd" />');						// store VEN tBody 설정
	}
	...
	function _eventSelectVendorException(){
		...
		$.post(
				"<c:url value='/edi/weborder/NEDMWEB0020vendorExceptionSelect0020.json'/>",
				JSON.stringify(str),
				function(data){
					if(data == null || data.state !="0"){
						hideLoadingMask();
						//자료조회중 오류가 발생하였습니다.[CODE:data.state]
						alert('<spring:message	code="msg.web.error.vendorExceptionSelet" />');		
					}
					else {
						hideLoadingMask();
	...
	function _eventSelectDelete(){
		...
		$.ajaxSetup({
	 	...
	 	$.post("<c:url value='/edi/weborder/NEDMWEB0020tedProdOrdDelete.json'/>"
	...
	function _eventTempSave(){
		...
		if(!chkValidte){
			alert('<spring:message code="msg.web.error.chkQty"/>');
			return;
		}
		...
		$.ajaxSetup({
	 	...
	 	$.post("<c:url value='/edi/weborder/NEDMWEB0020tedProdOrdTempSave.json'/>"
	  			,JSON.stringify({'tedOrdList' : strDataList})
	  			,function(json){				
					if(jQuery.trim(json) == ""){//처리성공
						hideLoadingMask();
	...
	function _doDetailSearch(prodCd){
		...
		$.post(
				"<c:url value='/edi/weborder/NEDMWEB0020tedProdOrdDetailSelect.json'/>",
				JSON.stringify(str),
				function(data){
	...
	function _eventSearch(){
		...
		$.post(
				"<c:url value='/edi/weborder/NEDMWEB0020tedProdOrdSelect.json'/>",
				JSON.stringify(str),
				function(data){
	...
	function _setTbodyProdOrdValue(json) {
		...
		if(data != null){
			...
			if (sz > 0) {
				...
				_setTbodyNoResult($("#datalistStr"), 7, "<spring:message code='text.web.field.srchProd'/>" );
			}
			...
			<div class="bbs_search">
				<ul class="tit">
					<li class="tit"><spring:message code='text.web.field.searchCondition'/>
						<span style="font-weight: normal; color: #414fbb; margin-left: 25px;"><img src="/images/epc/btn/icon_04.png" alt="Notice" />&nbsp; <span style="color: red;"><spring:message code="msg.web.send.time"/> </span></span>
					</li>
					<li class="btn">
				...
				<tr>
					<th><spring:message code='text.web.field.prodCdNprodNm'/></th>
					<td>
						<input type="text" id="prodCd" style="width: 100px;" name="prodCd" value="<c:out value='${paramMap.prodCd}'/>"> / <input type="text" id="prodNm" name="prodNm" value="<c:out value='${paramMap.prodNm}'/>">
					</td>
				...
				<tr>
					<td valign="top">
						...
						<td valign="top">
							<div class="bbs_list" style="margin-top: 2px;">	
								<ul class="tit">
									<li class="tit"><spring:message code='text.web.field.venOrdList'/></li>
								</ul>
								<table id="dataTable" width="100%" cellpadding="1" cellspacing="1" border="0" bgcolor=efefef>
			                        ...
			                        <tr bgcolor="#e4e4e4" align=center> 
			                          <th><spring:message code='epc.web.header.No'/></th>
			                          <th><spring:message code='epc.web.header.delete'/><br><input type="checkbox" id="allCheckStr" name="allCheckStr"></th>
			                          <th><spring:message code='epc.web.header.strNm'/></th>
			                          <th><spring:message code='epc.web.header.unit'/><br><spring:message code='epc.web.header.qty'/></th>
			                          <th><spring:message code='epc.web.header.ea'/></th>
			                          <th><spring:message code='epc.web.header.amt'/></th>
			                          <th rowspan="2"></th>
			                        </tr>
			                         <tr bgcolor="87CEFA">
										<td align="center" colspan="3"><spring:message code='epc.web.header.prodTotSum'/></td>
		...
		<div id="footbox">
			...
			<div class="location">
				<ul>
					...
					<li class="last"><spring:message code='epc.web.menu.prodCd'/></li>
				</ul>
			</div>
		</div>
	</div>
