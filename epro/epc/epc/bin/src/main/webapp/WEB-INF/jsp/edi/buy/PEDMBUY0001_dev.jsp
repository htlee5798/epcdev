<%--
	Page Name 	: PEDMBUYO0001_Dev.jsp
	Description : 매입정보 일자별 조회 페이지 
	Modification Information
	
	  수정일 			  수정자 				수정내용
	---------- 		---------    		-------------------------------------
	2015.11.05 		KWON O YEUN	 		최초생성
...
<head>
	...
	$(document).ready(function($) {
		...
		//Button Event
		//$('#btnSave').unbind().click(null, 				_eventSave); 		  			// 저장버튼
		
		//매입정보 일자별 조회 Event
		//_eventSearch();
		
		//전체점포
		$("#searchForm #strCdTypeAll").click(function(){
			storeClear();
		});
		
		//점포선택
		$("#searchForm #strCdTypeCho").click(function(){
			storePopUp()();
		});
	...
	function _eventSearch () {
		...
		searchInfo["srchStartDate"]		=	$("#searchForm input[name='startDate']").val().replaceAll("-", "");
		searchInfo["srchEndDate"]		=	$("#searchForm input[name='endDate']").val().replaceAll("-", "");
		searchInfo["srchStoreVal"]		=	($("#searchForm input[name='storeVal']").val() != "" ) ? $("#searchForm input[name='storeVal']").val().split("-") : null;
		searchInfo["srchEntpCode"]		=	$("#searchForm select[name='entp_cd']").val();
		searchInfo["srchProductVal"]	=	$("#searchForm input[name='productVal']").val()
		searchInfo["srchBuying"]		=	$("#searchForm select[name='buying']").val();
		...
		$.ajax({
			...
			dataType : 'json',
			async : false,
			url : '<c:url value="/edi/buy/PEDMBUY0001Select.json"/>',
				...
				<div class="wrap_search">
					...
					<div class="bbs_search">
						...
						<tr>
							...
							<td>
								<input type="Radio" name="strCdType" id="strCdTypeAll" <c:if test="${empty param.storeVal }"> Checked</c:if>/>전점조회
								<input type="Radio" name="strCdType" id="strCdTypeCho" <c:if test="${not empty param.storeVal }"> Checked</c:if>/>점포선택
							</td>
						...
						<tr>
							...
							<th>매입구분</th>
							<td >
									<html:codeTag objId="buying" objName="buying" comType="SELECT" formName="form" selectParam="${param.buying}" parentCode="SPP01" defName="전체"></html:codeTag>
