...
<script>
	...
	$(document).ready(function($) {		
		...
		$("select[name='ordering']").val("<c:out value='${param.ordering}'/>");
	...
	function doSearch() {
		...
		if(dateValid(form)){
			loadingMaskFixPos();
			form.action  = "<c:url value='/edi/order/PEDMORD0005Select.do'/>";
			form.submit();	
		}	
	}
	...
	function popupSearch(tbName1){		
		...
		form.staticTableBodyValue.value = "<CAPTION>발주정보 PDC전표상세별 현황표<br>"+
	...
	function printText(){
		...
		if(dateValid(form)){
			loadingMaskFixPos();
			form.action  = "<c:url value='/edi/order/PEDMORD0005Text.do'/>";
			form.submit();	
						...
						<c:if test="${not empty orderList }">
							...
							<c:forEach items="${orderList}" var="list" varStatus="index" >
								<c:if test="${list.ORD_SLIP_NO != tmp }">
									...
									<tr class="r1">
										<td align=center>${list.CTN_NM }</td>
										...
										<td align=center>${list.TOT_QTY }</td>
