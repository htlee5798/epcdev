...
<%
	/**
	 * @Class Name : PSCPPRD002101.jsp
	 * @Description : 통계 > 업체상품수정관리 이미지탭
	 * 
	 *   수정일                  수정자                                         수정내용
	 *  -------    --------    ---------------------------
	 * 2014.02.25	drkang 
...
$(document).ready(function(){
	...
	ibdata.Cols = [
	  ...
	  , {Header:"SEQ", 	        		Type:"Text", 		SaveName:"SEQ", 	          		Align:"Center", Width:40, Edit:0, Hidden:true}
	  ...
	  , {Header:"대표이미지순번", 		Type:"Text", 		SaveName:"PROD_IMG_NO",  Align:"Center",  Width:70, Edit:0}
	  , {Header:"대표이미지", 				Type:"Text", 		SaveName:"BOS_IMG",        	Align:"Center", Width:60, Edit:0, Cursor:'pointer', FontUnderline:true}
	  ...
	  , {Header:"확인관리자",     	Type:"Text", 		SaveName:"APRV_ADMIN", 	Align:"Center", Width:80, Edit:0}
	  , {Header:"협력업체명",        	Type:"Text", 		SaveName:"VENDOR_NM", 	Align:"Center",     Width:120, Edit:0}
...
function goPage(currentPage){
	...
	var url = '<c:url value="/product/selectProdImgChgHist.do"/>';
...
function viewImg(seq,prodCd) {
	var targetUrl = '<c:url value="/product/imageDetailForm.do"/>?seq='+seq+'&prodCd='+prodCd;	
	Common.centerPopupWindow(targetUrl, 'imgDetail', {width : 1100, height : 650 , scrollBars : "YES"});
}
...
function mySheet_OnClick(Row, Col, Value, CellX, CellY, CellW, CellH) {
	...
	if (mySheet.ColSaveName(Col) == 'BOS_IMG') {
		viewImg(mySheet.GetCellValue(Row, 'SEQ'),mySheet.GetCellValue(Row, 'PROD_CD'));
	}
...
function mySheet_OnSearchEnd(cd,msg) {
	...
	if( mySheet.GetTotalRows() > 0 ){
		for(i = 1; i <= mySheet.GetTotalRows(); i++) {
			...
			mySheet.SetCellFontColor(i, "BOS_IMG", '#1E90FF');
		}
	}
