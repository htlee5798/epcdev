...
<script>
	...
	function popupSearch(tbName1, tbName2){		
		...
		form.staticTableBodyValue.value = "<CAPTION>발주정보 점포별 현황표<br>"+
	...
	function printText(){
		...
		if(dateValid(form)){
			loadingMaskFixPos();
			form.action  = "<c:url value='/edi/order/PEDMORD0004Text.do'/>";
			form.submit();	
		}	
					...
					<div class="bbs_list">
						...
						<c:if test="${not empty orderList }">
							...
							<c:forEach items="${orderList}" var="list" varStatus="index" >
							     <c:if test="${list.PROD_CD != tmp and index.count > 1}">
									<th colspan="8" class="fst" align=center>당일 총 합계</th>
								...
								<tr class="r1">
									...
									<td align="right"><fmt:formatNumber value="${list.BUY_DAN }" type="number" currencySymbol="" /></td>
								...
								<c:set var="tmp" value="${list.PROD_CD }" />
							</c:forEach>
						</c:if>
