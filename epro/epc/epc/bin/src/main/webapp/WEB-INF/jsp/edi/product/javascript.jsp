...
var uploadErrorMsg =
{
	...
	function makeOnlineRepresentProductCode1(currentNodeValue) {
		...
		$.getJSON("${ctx}/edi/product/onlineRepresentProduct.do",{entpCode: currentNodeValue}, function(j){
	...
	function checkBlackListVendor() {
		...
		*/
		$.getJSON("${ctx}/edi/product/checkCountVendorStopTrading.do",{vendorCode: selectedVendor}, vendorCountResult);
	}

	//거래중지된 업체인지 체크. 실제 결과값은 vendorCountResult 함수에서 처리됨.
	function checkBlackListVendor1(selectedVendor) {
		$.getJSON("${ctx}/edi/product/checkCountVendorStopTrading.do",{vendorCode: selectedVendor}, vendorCountResult1);
	}
	...
	function checkBlackListVendor4Online() {
		...
		var isThisBlackListVendor = false;
		*/
		$.getJSON("${ctx}/edi/product/checkCountVendorStopTrading.do",{vendorCode: selectedVendor}, vendorCountResult4Online);
	}
	...
	function sizeCategorySetup () {
		$.getJSON("${ctx}/edi/product/sizeByCategory.do",{sizeCategoryCode: $("select[name=sizeCategory]").val()}, function(j){
			var options = '';
			for (var i = 0; i < j.length; i++) {
				if(i == 0) {
...
//	}
	...
	function openBrandPopup() {
		...
		openDetail("${ctx}/edi/product/brand.do");
	}

	//메이커 선택팝업
	function openMakerPopup() {
		//window.open("${ctx}/edi/product/maker.do");
		openDetail("${ctx}/edi/product/maker.do");
	}
	...
	function isUniqueSell88Code(userInputSellObj) {
		...
		$.getJSON("${ctx}/edi/product/checkUserSellCode.do",{sellCode: userInputSellObj.val()}, function(j){
			if( j > 0 ) {
	...
	function selectL1List() {
		$.getJSON("${ctx}/edi/product/selectL1List.do",
			{
	...
	function selectL4List() {
		$.getJSON("${ctx}/edi/product/selectL4List.do",
			{
				groupCode: $("#teamGroupCode").val(),
	...
	function selectSeasonList(productDivnCode) {
		$.getJSON("${ctx}/edi/product/selectSeasonList.do",
			{
	...
	//	alert("selectProdTemplateList");
		$.getJSON("${ctx}/edi/product/selectProdAddTemplateList.do",
		{
			lCode: lcd,
			flag: flag
		}, function(j){
	...
	//	alert("selectProdTemplateList");
		$.getJSON("${ctx}/edi/product/selectProdCertTemplateList.do",
		{
			lCode: lcd,
			flag: flag
		}, function(j){
	...
	function selectCertTemplateDtlList(infoCd) {
		...
		$.getJSON("${ctx}/edi/product/selectProdCertTemplateDetailList.do",
		{
			infoGrpCd: infoCd
		}, function(j){
			var tempList = ''; //리스트 html
	...
	*/
	...
	function selectBoxProdTemplateDetailList(infoCd) {
		...
		//$.getJSON("${ctx}/edi/product/selectProdAddTemplateList.do",
		$.getJSON("${ctx}/edi/product/selectProdAddTemplateDetailList.do",
		{
			infoGrpCd: infoCd
		}, function(j){
			var tempList = '';	 //리스트 html
	...
	function selectProdTemplateUpdateViewList(prodCd,lcd,infoGrpCd,flag){
		...
		$.getJSON("${ctx}/edi/product/selectProdAddTemplateUpdateList.do",
		{
			newProdCd: prodCd,
			lCode: lcd,
			flag: flag
	...
	function selectBoxProdTemplateUpdateDetailList(prodCd,infoGrpCd) {
		...
		$.getJSON("${ctx}/edi/product/selectProdAddTemplateUpdateDetailList.do",
		{
			newProdCd: prodCd,
			infoGrpCd: infoGrpCd
		}, function(j){
		...
		//selectBoxProdTemplateUpdateDetailList("${tmpProduct.newProductCode}","${tmpProduct.infoGrpCd}");
	...
	function selectBoxProdCertTemplateUpdateDetailList(prodCd,infoGrpCd) {
		$.getJSON("${ctx}/edi/product/selectProdCertTemplateUpdateDetailList.do",
		{
			newProdCd: prodCd,
			infoGrpCd: infoGrpCd
		}, function(j){
