...
<script language="javascript">
	...
	function img_viewNew(imgurl, imgpath) {
		  ...
		  window.open('${ctx}/edi/consult/step3ImagePopupNew.do?img=' + imgurl + '&imgpath=' + imgpath + '&h=' + h, 'imgwin', 'width=' + openWidth + ', height=' + openHeight + ', scrollbars=yes, resizable=yes');
		}	
					...
					<table cellpadding="0" cellspacing="0" border="0" width="100%">						
							...
							<tbody>
								<tr>
									...
									<th><p>사업자등록번호</p></th>
									<td colspan="3"><p class="td_p">${ vendorSession.vendor.bmanNo }</p></td>
								</tr>
								<tr>
								<th><p>희망부서</p></th>
									<td><p class="td_p">${vendor.teamNm }</p></td>
								<th><p>대분류</p></th>
									<td><p class="td_p">${vendor.l1Nm }</p></td>
								</tr>
			...
			<div class="s_title mt20">
					...
					<table cellpadding="0" cellspacing="0" border="0" width="100%">						
							...
							<tbody>		
								...
								<tr>
									<th rowspan="3"><p>상품명</p></th>
									<c:if test="${fn:length(vendorProductList) == 0 }">
									</c:if>
									<c:if test="${fn:length(vendorProductList) > 0 }">
										<c:forEach var="vendorProduct" items="${vendorProductList}"	varStatus="status">
										
											<td ><p class="td_p">${vendorProduct.productName }</p></td>
											<td colspan="2">
												<c:if	test="${not empty vendorProduct.attachFileCode }">
												<a	href="javascript:img_view('${vendorProduct.attachFileCode}', '${attachFileFolder}')" class="btn">
										<!-- 		${vendorProduct.attachFileCode}
										 -->
										 		이미지 보기</a>
											<!-- 	${attachFileFolder}
											-->
