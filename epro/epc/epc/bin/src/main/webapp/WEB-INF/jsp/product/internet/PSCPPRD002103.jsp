...
$(document).ready(function(){
	...
	$("#vendorId, #vendorNm").attr("readonly","readonly");
	...
	ibdata.Cols = [
	  ...
	  , {Header:"전상법", 				Type:"Text", 		SaveName:"ADD_INFO",  		Align:"Center",  Width:70, Edit:0, Cursor:'pointer', FontUnderline:true}
...
function goPage(currentPage){
	...
	var url = '<c:url value="/product/selectProdAddChgHist.do"/>';
...
function viewAddInfo(seq,prodCd) {
	var targetUrl = '<c:url value="/product/addInfoDetailForm.do"/>?seq='+seq+'&prodCd='+prodCd;
	Common.centerPopupWindow(targetUrl, 'imgDetail', {width : 1100, height : 650});
}
...
function mySheet_OnClick(Row, Col, Value, CellX, CellY, CellW, CellH) {
	...
	if (mySheet.ColSaveName(Col) == 'ADD_INFO') {
		viewAddInfo(mySheet.GetCellValue(Row, 'SEQ'),mySheet.GetCellValue(Row, 'PROD_CD'));
	}
...
function mySheet_OnSearchEnd(cd,msg) {
	...
	if( mySheet.GetTotalRows() > 0 ){
		for(i = 1; i <= mySheet.GetTotalRows(); i++) {
			...
			mySheet.SetCellFontColor(i, "ADD_INFO", '#1E90FF');
		}
	}
