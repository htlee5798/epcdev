<%--
	Page Name 	: NEDMBUYO0030.jsp
	Description : 매입정보 상품별 조회 페이지 
...
<script id="dataListTemplate" type="text/x-jquery-tmpl">
	<tr class="r1">
		...
		<td align="center"><a href="javascript:forwarding2('<c:out value="\${prodCd }"/>','<c:out value="\${buyDy }"/>');"><c:out value="\${prodCd }"/></a></td>
...
<script>
	...
	function doSearch() {
		...
		if (dateValid()) {
			$.ajax({
				...
				dataType : 'json',
				async : false,
				url : '<c:url value="/edi/buy/NEDMBUY0030Select.json"/>',
	...
	function doExcel(tbName1, tbName2, tbName3){		
		...
		var bodyValue = "";
		bodyValue += "<CAPTION><spring:message code='text.buy.field.buyPrdCdList'/><br>";
	...
	function forwarding2(val,val2) {
		...
		$("#searchForm #productVal").val(val);
		$("#searchForm #srchStartDate").val(val2);
		$("#searchForm #srchEndDate").val(val2);
		$("#searchForm #isForwarding").val("Y");
		
		$("#searchForm").attr("action", "<c:url value='/edi/buy/NEDMBUY0060.do'/>");
		$("#searchForm").submit();
	}
	...
	function printText() {
		...
		$("#searchForm").attr("action", "<c:url value='/edi/buy/NEDMBUY0030Text.do'/>");
		$("#searchForm").submit();			
	}
</script>
				...
				<div class="wrap_search">
					...
					<div class="bbs_search">
						...
						<tr>
							...
							<td >
									<html:codeTag objId="srchBuying" objName="srchBuying" comType="SELECT" formName="form" selectParam="<c:out value='${param.srchBuying}'/>" parentCode="SPP01" defName="전체"></html:codeTag>
					...
					<div class="bbs_list">
						...
						<table class="bbs_list" cellpadding="0" cellspacing="0" border="0" id="sumTbl">	
							...
							<tr id="sumRow" style="display: none;">
								<th colspan="6" class="fst" align=center><b><spring:message code="epc.buy.header.total" arguments="" /></b></th>
			...
			<div id="footbox">
				...
				<div class="location">
					<ul>
						...
						<li class="last"><spring:message code='epc.buy.menu.prdCd' /></li>
					</ul>
				</div>
			</div>
		</div>
