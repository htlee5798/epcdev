...
<script>
	...
	function doSearch() {
		...
		if(dateValid()){
			$.ajax({
				...
				dataType : 'json',
				async : false,
				url : '<c:url value="/edi/usply/NEDMUSP0060Select.json"/>',
		...
		function _setTbodyMasterValue(json) { 
			...
			if (data.length > 0) {
				...
				// 금액 및 수량 comma 및 합계ORD_IPSU USPLY_BUY_AMT
				for (var i = 0; i < data.length; i++) {
	...
	function popupSearch(tbName1, tbName2){		
		...
		var bodyValue = "";
		bodyValue += "<CAPTION>미납정보 페널티확정 현황표<br>";
	...
	function printText() {
		...
		$("#searchForm").attr("action", "<c:url value='/edi/usply/NEDMUSP0060Text.do'/>");
		$("#searchForm").submit();	
	}
...
<tr class="r1">
	...
	<td align="center"><c:out value="\${minorCd }"/></td>
</tr>
</script>
<!-- Data List Templete -------------------------------------------------------------------------------------------------------------->
...
<body>
	...
	<div>
		...
		<input type="hidden"	name="searchProductVal"		id="srchProductVal"			/>
			...
			<div class="wrap_con">
				...
				<div class="bbs_list">
					...
					<tr>
						...
						<th><spring:message code='epc.usp.header.std'/></th>
		...
		<div id="footbox">
			...
			<div class="location">
				<ul>
					...
					<li class="last"><spring:message code='epc.usp.penalty'/></li>
				</ul>
			</div>
		</div>
	</div>
