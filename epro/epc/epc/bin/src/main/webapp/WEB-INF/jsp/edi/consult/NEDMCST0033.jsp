...
function ckBmanValue(){
	...
	var url = "<c:url value='/edi/consult/ckBmanValue.json'/>";

	$.getJSON(url
			,{
				ajax_bman: $("#searchForm select[name=bmanNo]").val(),
				ajax_seq: $("#searchForm input[name=svcSeq]").val(),
				ajax_up_anx: $("#searchForm select[name=anxInfoCd]").val()
			}
			, function(j){
			var tmp = j.split(";");
			$('#searchForm input[name=venCds]').each(function(){
...
function Nextfocus(lenth,name,a)  
{
	...
	var url ="<c:url value='/edi/consult/getCellCountUP.json'/>";
		...
		$.getJSON(url,
				{
					...
					ajax_ck_cell1: $("input[name=oldCell1]").val(),
					ajax_ck_cell2: $("input[name=oldCell2]").val(),
					ajax_ck_cell3: $("input[name=oldCell3]").val(),
					ajax_up_anx: $("input[name=oldAnxInfoCd]").val()
				}, function(j){
...
var $venCheckBox = $('#searchForm input[name=venCds]:checked');
	...
	if($('#searchForm input[name=valid_tel]').val()=="no"){
		alert("<spring:message code='epc.cst.alert.msg18'/>");
		return;
	}
	if($('#searchForm input[name=valid_email]').val() =="no"){
		alert("<spring:message code='epc.cst.alert.msg19'/>");
		return;
	}

	if(confirm("<spring:message code='epc.cst.alert.msg21'/>")){
		loadingMaskFixPos();
		$('#searchForm').attr('action',"<c:url value='/edi/consult/NEDMCST0032Update.do'/>");
		$('#searchForm').submit();
	}
}
...
function emailCheck(){
	...
	if( $("input[name=email]").val() != null && $("input[name=email]").val() != "" ) {
		...
		var url ="<c:url value='/edi/consult/getEmailCountUP.json'/>";
		$.getJSON(url,
				{
...
function doDelete(){
	if(confirm("<spring:message code='epc.cst.alert.msg22'/>")){
		loadingMaskFixPos();
		$('#searchForm').attr('action',"<c:url value='/edi/consult/NEDMCST0032delete.do'/>");
		$('#searchForm').submit();
	}
}
...
<body>
	...
	<div>
		...
		<input type="hidden" id="svcSeq" name="svcSeq" value="${alertInfo.svcSeq }"/>
		<input type="hidden" id="oldBmanNo" name="oldBmanNo" value="${alertInfo.bmanNo }"/>
		<input type="hidden" id="oldAnxInfoCd" name="oldAnxInfoCd" value="${alertInfo.anxInfoCd }"/>
		
		<input type="hidden" id="oldCell1" name="oldCell1" value="${alertInfo.cell1 }"/>
		<input type="hidden" id="oldCell2" name="oldCell2" value="${alertInfo.cell2 }"/>
		<input type="hidden" id="oldCell3" name="oldCell3" value="${alertInfo.cell3 }"/>
		<input type="hidden" id="oldEmail" name="oldEmail" value="${alertInfo.email }"/>
			...
			<div class="wrap_search">
				...
				<div class="bbs_search">
					<ul class="tit">
						...
						<li class="btn">
							<a href="#" class="btn" onclick="javascript:doDelete();"><span><spring:message code='epc.cst.header.hCode9'/></span></a>
							<a href="#" class="btn" onclick="javascript:valid();"><span><spring:message code='epc.cst.header.hCode00315'/></span></a>
						</li>
					</ul>
					<ul>
					...
					<div style="width:100%; height:400px; overflow-x:hidden; overflow-y:hidden; table-layout:fixed;" >
						<table class="bbs_grid3" cellpadding="0" cellspacing="0" border="0" id="addTable">
							...
							<tr class="r1">
								...
								<td>
									<select name="anxInfoCd">
										<option ${alertInfo.anxInfoCd eq 'ALM' ? 'selected' : '' } value="ALM"><spring:message code='epc.cst.codeNm1'/></option>
										<option ${alertInfo.anxInfoCd eq 'PAY' ? 'selected' : '' } value="PAY"><spring:message code='epc.cst.codeNm2'/></option>
									</select>
								</td>
							</tr>
							<tr class="r1">
								...
								<td align="center"> 
									<select name="cell1">
										<option ${alertInfo.cell1 eq '010' ? 'selected' : '' } value="010">010</option>
										<option ${alertInfo.cell1 eq '011' ? 'selected' : '' } value="011">011</option>
										<option ${alertInfo.cell1 eq '016' ? 'selected' : '' } value="016">016</option>
										<option ${alertInfo.cell1 eq '017' ? 'selected' : '' } value="017">017</option>
										<option ${alertInfo.cell1 eq '018' ? 'selected' : '' } value="018">018</option>
										<option ${alertInfo.cell1 eq '019' ? 'selected' : '' } value="019">019</option>
									</select>&nbsp;-&nbsp;
									<input type="text" name="cell2" value="${alertInfo.cell2}"  size="4" maxlength="4" onblur="Nextfocus(this.value.length,this.name,this);"/>&nbsp;-&nbsp;
									<input type="text" name="cell3" value="${alertInfo.cell3}" size="4" maxlength="4" onblur="Nextfocus(this.value.length,this.name,this);"/>
							...
							<tr class="r1">
								...
								<td align="center">
									<input type="text" name="email" value="${alertInfo.email}" maxlength="30" style="width:345px;" onblur="javascript:emailCheck();"/>
