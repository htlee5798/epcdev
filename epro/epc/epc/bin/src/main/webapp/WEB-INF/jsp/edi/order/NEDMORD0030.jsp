...
<script>
	...
	function doSearch() {
		...
		if (dateValid()){
			$.ajax({
				...
				url : '<c:url value="/edi/order/NEDMORD0030Select.json"/>',
	...
	function doExcel(tbName1) {
		...
		var bodyValue = "";
		bodyValue += "<CAPTION><spring:message code='epc.ord.orderDashBoardOrdSlipNoDetail'/><br>";
	...
	function printText() {
		...
		if (dateValid()) {
			...
			$("#searchForm").attr("action", "<c:url value='/edi/order/NEDMORD0030Text.do'/>");
			$("#searchForm").submit();
								...
								<c:forEach items="${orderList}" var="list" varStatus="index" >
									<c:if test="${list.ordSlipNo != tmp }">
										<tr class="r1">
											...
											<th style="display:none;"><spring:message code='epc.ord.supplyNm'/></th>	<!-- supplyNm 납품처 -->
										...
										<tr class="r1">
											...
											<td align=center style="display:none;"><c:out value ="${list.supplyNm }" /></td>
											<td align=center style="display:none;"><c:out value ="${list.strCd }" /></td>
											<td align=center><c:out value ="${list.strNm }" /></td>
											<td align=center><c:out value ="${list.ctrNm }" /></td>
			...
			<div id="footbox">
				...
				<div class="location">
					<ul>
						...
						<li class="last"><spring:message code='epc.ord.perOrdSlipNo'/></li>
					</ul>
				</div>
			</div>
		</div>
