...
<c:choose>
	...
	<input type="hidden" id="back_e_text" value="${back_e_text }"/>
		<script>
			...
			$(document).ready(function($){
				for(var i=0;i<first_split.length-1;i++){
					addContents();
				}
				valueSetting();
			});
	...
	<c:otherwise>
		<input type="hidden" id="vals_b" value="${vals_b }"/>
		<input type="hidden" id="pid_b" value="${pid_b }"/>
		<input type="hidden" id="etext_b" value="${etext_b }"/>
			<script>

				var vals_b = document.getElementById("vals_b");
				var pid_b = document.getElementById("pid_b");
				var etext_b = document.getElementById("etext_b");

				var first_split_b = vals_b.value.split('@');

				$(document).ready(function($){
					for(var i=0;i<first_split_b.length-1;i++){
						addContents();
					}
					valueSettingB();
				});
				...
				function valueSettingB(){
					

					var form = document.forms[0];
					var return_values = document.getElementById("vals_b");
					var return_val_split = return_values.value.split('@');
					
					form.pid.value      = pid_b.value;
					form.e_text.value = etext_b.value;
...
function doUpdate(){
	...
	var tmpFile = document.getElementById("deleteFile");
	...
	if(tmpFile.style.display=="none" && form.file.value==""){
		alert("기존에 등록된 첨부파일이 없습니다. 첨부파일을 선택하세요.");
		return;
	}

	if(form.file.value != ""){
		alert("기존파일은 새 파일로 교체됩니다.");
	...
	form.action  = "<c:url value='/edi/consult/PEDMCST0004Update.do'/>";
	form.submit();
}
...
<div id="content_wrap" <c:if test="${not empty param.widthSize }">style=width:${param.widthSize }</c:if>  >
	<div>
		...
		<input type="hidden" name="del_file_path" value="${estListTop.FILE_SEQ }"/>
		<input type="hidden" name="del_file_nm" value="${estListTop.FILE_NM }" />
			...
			<div class="wrap_search">
				...
				<div class="bbs_search">
					...
					<tr>
						<th>협력업체 코드</th>
						<td>	
							<html:codeTag objId="entp_cd" objName="entp_cd" width="150px;" selectParam="${estListTop.VEN_CD}" dataType="CP" comType="SELECT" formName="form"  />
						</td>
						<th>문서명</th>
						<td>
							<input type="text"  name="e_text" size="20" style="width:200px;"/>
						</td>
					</tr>
					</table>
				</div>
			...
			<div class="wrap_con">
				...
				<div class="bbs_list">
					...
					<table class="bbs_list" cellpadding="0" cellspacing="0" border="0" >
						<tr id="deleteFile">
							<th align="left">&nbsp;&nbsp;&nbsp;기존 첨부파일 :&nbsp;&nbsp;
							<a href="javascript:fileDownlord('${estListTop.PID }');">
							 <input type="hidden" name="hidden_file" value="${estListTop.FILE_SEQ }">${estListTop.FILE_NM }</a>
							 <a href="#" class="btn" onclick="javascript:fileDelete('${estListTop.FILE_SEQ }','${estListTop.FILE_NM }','${estListTop.PID }');"><span><spring:message code="button.common.delete"/></span></a>
							 </th>
						</tr>
						<tr>
							<th align="left">&nbsp;&nbsp;&nbsp;첨부 파일 : <input type="file" class="imageUpload" value="" name="file" size="50"></th>
						</tr>
					</table>
