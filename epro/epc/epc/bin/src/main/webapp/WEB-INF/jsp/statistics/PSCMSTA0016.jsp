...
function goPage(pageIndex){
	...
	if(startDate>endDate){
		alert('종료일자가 시작일자보다 클수 없습니다.');
		return;
	}
	
	var url = '<c:url value="/statistics/selectGasStationPickUp.do"/>';
...
function pickUpListExcel(){
	...
	f.pageIndex.value = 1;
 	var url = '<c:url value="/statistics/pickUpListExcelByGas.do"/>';
	f.action = url;
	f.submit();	  
}
...
function doExcel(){
	...
	if(!confirm("엑셀 다운로드 하시겠습니까?")){
 	...
 	var url = '<c:url value="/statistics/gasStationPickUpExcel.do"/>';
	f.pageIndex.value = 1;
	f.action = url;
	f.submit();	  
}
		...
		<div class="bbs_search">
			...
			<td>
				<select id="pickUpTime" name="pickUpTime" class="select">
					<option value="">전체</option>
					<c:forEach items="${pickUpTimeList}" var="timeList" begin="0" >
						<option value="${timeList.ORD_SEQNC}" ${selPtime == timeList.ORD_SEQNC ? "selected='selected'" : "" }>${timeList.PICKUP_TIME}</option>
					</c:forEach>
				</select>
			</td>
			<th>주문구분</th>
