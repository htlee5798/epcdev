...
<script>
 	...
 	function doSearch() {
		...
		if (dateValid()) {
			$.ajax({
				...
				dataType : 'json',
				async : false,
				url : '<c:url value="/edi/buy/NEDMBUY0010Select.json"/>',
	...
	function doExcel(tbName1, tbName2, tbName3) {
		...
		var bodyValue = "";
		bodyValue += "<CAPTION><spring:message code='text.buy.field.buyPeriodList'/><br>";
	...
	function forwarding(val) {
		...
		$("#searchForm").attr("action", "<c:url value='/edi/buy/NEDMBUY0020.do'/>");
		$("#searchForm").submit();
	}
	...
	function printText() {
		...
		$("#searchForm").attr("action", "<c:url value='/edi/buy/NEDMBUY0010Text.do'/>");
		$("#searchForm").submit();
					...
					<div class="bbs_list">
						...
						<table class="bbs_list" cellpadding="0" cellspacing="0" border="0" id="sumTbl">
							...
							<tr class="r1" id="sumRow" style="display: none;">
								<th class="fst" align=center><b><spring:message code="epc.buy.header.total" arguments="" /></b></th>
			...
			<div id="footbox">
				...
				<div class="location">
					<ul>
						...
						<li class="last"><spring:message code='epc.buy.menu.buyDy' /></li>
					</ul>
				</div>
			</div>
		</div>
