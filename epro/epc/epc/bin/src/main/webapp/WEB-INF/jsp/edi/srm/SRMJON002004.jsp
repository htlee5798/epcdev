...
<%--
	Page Name 	: SRMSJON02004.jsp
	Description : 임시비밀번호 변경 팝업창
	Modification Information

	수정일      			수정자           		수정내용
	-----------    	------------    ------------------
	2023.09.14  	PARK IL YOUNG	 최초 생성
--%>
...
<head>
	...
	<title>임시비밀번호 초기화</title><%--비밀번호 변경 --%>
	...
	<script language="JavaScript">
		...
		function func_Ok() {
			...
			if ($.trim($("#MyForm input[name='tempPw1']").val()) != $.trim($("#MyForm input[name='tempPw2']").val())) {
			...
			$.ajax({
				...
				dataType : 'json',
				async : false,
				url : '<c:url value="/edi/srm/updateTempPasswdChange.json"/>',
				data : JSON.stringify(searchInfo),
				success : function(data) {
					if (data.msg == "success") {
						alert("<spring:message code='msg.srm.alert.changeTempPw' />");<%-- 비밀번호가 변경되었습니다. --%>
						opener.location.href = "<c:url value='/edi/srm/SRMJON002001.do'/>";
						window.close();
					...
					} else if (data.msg == "GET_SUBMIT") {
						alert("임시비밀번호 초기화창은 새로고침이 안됩니다. 다시 로그인해 변경 부탁드립니다.");
						window.close();
					}
				}
			});
...
<div id="popup_wrap">
	...
	<h2 class="tit_star">임시비밀번호 초기화</h2>	<%--비밀번호 변경 --%>

	<!-- 알림 -->
	<div class="noti_box">
		<ul class="noti_list">
			<li class="txt_l">임시비밀번호 발급시, 비밀번호 변경해주셔야 됩니다.</li>
	...
	<form id="MyForm" name="MyForm">
		<table class="tbl_st1 form_style">
			...
			<tbody id="pwdChg">
			<tr>
				<th><label for="tempPw1">새 비밀번호</label></th>	<%--비밀번호 --%>
				<td><input type="password" id="tempPw1" name="tempPw1" style="width:97%"/></td>
			</tr>
			<tr>
				<th><label for="tempPw2">입력 비밀번호 확인</th>	<%--비밀번호 확인 --%>
