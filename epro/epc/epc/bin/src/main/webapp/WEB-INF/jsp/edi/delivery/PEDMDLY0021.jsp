...
<script>
	...
	function doSearch() {
		...
		if(dateValid(form)){
			loadingMaskFixPos();
			form.action  = "<c:url value='/edi/delivery/PEDMDLY0021Select.do'/>";
			form.submit();
		}	
	...
	function popupSearch(tbName1, tbName2){		
		...
		form.staticTableBodyValue.value = "<CAPTION>TOY배달정보  현황표<br>"+
					...
					<c:if test="${not empty deliveryList }">
						<c:forEach items="${deliveryList}" var="list" varStatus="index" >
							<tr class="r1">
								...
								<td align="center">${list.ACCEPT_FG0 }</td>
								<td align="center">${list.ACCEPT_FG1 }</td>
								<td align="center">${list.ACCEPT_FG2 }</td>
								<td align="center">${list.ACCEPT_FG3 }</td>
								<td align="center">${list.ACCEPT_FG4 }</td>
							</tr>
						</c:forEach>
					</c:if>
