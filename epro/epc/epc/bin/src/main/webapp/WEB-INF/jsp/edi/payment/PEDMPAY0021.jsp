...
<script>
	...
	function doSearch() {
		...
		if(dateValid(form)){
			loadingMaskFixPos();
			form.action  = "<c:url value='/edi/payment/PEDMPAY0021Select.do'/>";
			form.submit();
		}	
	...
	function popupSearch(tbName1, tbName2, tbName3){		
		...
		form.staticTableBodyValue.value = "<CAPTION>결산정보 거래실적정보 현황표<br>"+
	...
	function printText(){
		...
		form.action  = "<c:url value='/edi/payment/PEDMPAY0021Text.do'/>";
		form.submit();	
		...
		<div id="wrap_menu">
			...
			<div class="wrap_con">
				...
				<div class="bbs_list">
					...
					<c:if test="${not empty paymentList }">
						...
						<c:forEach items="${paymentList}" var="list" varStatus="index" >
							<tr class="r1">
								...
								<td align="right"><fmt:formatNumber value="${list.TRANS_BUY_AMT_TOTAL }" type="number" currencySymbol="" /></td>
								<td align="right"><fmt:formatNumber value="${list.BUY_AMT_TOTAL }" type="number" currencySymbol="" /></td>
								<td align="right"><fmt:formatNumber value="${list.PAY_BUY_AMT_TOTAL }" type="number" currencySymbol="" /></td>
								<td align="right"><fmt:formatNumber value="${list.REM_BUY_AMT_TOTAL }" type="number" currencySymbol="" /></td>
								<td rowspan="2" align="right"><fmt:formatNumber value="${list.REM_SUM_TOTAL }" type="number" currencySymbol="" /></td>
							</tr>
							<tr class="r1">
								<td align="center">세액</td>
								<td align="right"><fmt:formatNumber value="${list.TRANS_VAT_TOTAL }" type="number" currencySymbol="" /></td>
								<td align="right"><fmt:formatNumber value="${list.VAT_TOTAL }" type="number" currencySymbol="" /></td>
								<td align="right"><fmt:formatNumber value="${list.PAY_VAT_TOTAL }" type="number" currencySymbol="" /></td>
								<td align="right"><fmt:formatNumber value="${list.REM_VAT_TOTAL }" type="number" currencySymbol="" /></td>
							</tr>
							
							<c:set var="tot_top1"  value="${tot_top1 + list.TRANS_BUY_AMT_TOTAL}" />
							<c:set var="tot_top2"  value="${tot_top2 + list.BUY_AMT_TOTAL}" />
							<c:set var="tot_top3"  value="${tot_top3 + list.PAY_BUY_AMT_TOTAL}" />
							<c:set var="tot_top4"  value="${tot_top4 + list.REM_BUY_AMT_TOTAL}" />
							<c:set var="tot_bottom1"  value="${tot_bottom1 + list.TRANS_VAT_TOTAL}" />
							<c:set var="tot_bottom2"  value="${tot_bottom2 + list.VAT_TOTAL}" />
							<c:set var="tot_bottom3"  value="${tot_bottom3 + list.PAY_VAT_TOTAL}" />
							<c:set var="tot_bottom4"  value="${tot_bottom4 + list.REM_VAT_TOTAL}" />
							<c:set var="tot_sum"  value="${tot_sum + list.REM_SUM_TOTAL}" />
						</c:forEach>
					</c:if>
					...
					<c:if test="${not empty paymentList }">
						<tr class="r1">
							<th rowspan="2" align="center"><b>합계</b></td>
							<th align="center"><b>공급가액</b></td>
					...
					<c:if test="${empty paymentList }">
						<tr><td colspan=7>&nbsp;</td><th rowspan=2 width=17>&nbsp;</th></tr>
