<%--
	Page Name 	: NEDMPRO0100.jsp
	Description : 코리안넷 상품등록 페이지
...
<head>
	...
	<!-- 개발스크립트_에서만 이용하는 스크립트 
	<c:if test="${empty param.mode}">
	  <script type="text/javascript" src="/js/epc/edi/product/updateKorNetInfoForTest.js"></script>
	</c:if>
	-->
	...
	<script type="text/javascript" >
		...
		var entpCdCnt	=	"<c:out value='${entpCdCnt}'/>";				//코리안넷을 통해 들어온 사업자번호로 해당협력업체 존재유무 확인
		var entpCd		=	"<c:out value='${newProdDetailInfo.entpCd}'/>";	//상품등록 이후 저장되어 있는 협력업체 코드
		...
		$(document).ready(function() {
			...
			if (entpCdCnt <= 0 && entpCd.replace(/\s/gi, '') == "") {
				alert("해당사업자번호의 협력사가 존재하지 않아 상품을 등록 할 수 없습니다.");
				//history.back(-1);
			} else {
				_evnetSetEntpCdList();
			}
			...
			$("#prodTabs li").click(function() {
				...
				//물류바코드 탭
					...
					$("#hiddenForm").attr("action", "<c:url value='/edi/product/NEDMPRO0103.do'/>");
					$("#hiddenForm").attr("method", "post").submit();
			...
			$("#newProduct select[name=l1Cd]").change(function() {
				...
				//$("#l3Cd").addClass("default");	//200306 EC전용 카테코리 추가
				$("#grpCd option").not("[value='']").remove();
				$("li[id=productCertDtlSelectBox]").hide(); //--20170520  추가
				//품질안정인증 기존행 삭제
				//fnRemoveCert_List();
			...
			//-----단일상품일때만 88코드 입력 가능(묶음일때는 변형속성 탭에서 88입력가능)
			_eventSetSellCdStat($(":radio[name='prodAttTypFg']:checked").val());
			...
			*/
			...
			//----- 해당년도의 계절리스트 조회
			//_eventSelectSesnList($("#newProduct select[name='sesnYearCd']").val().replace(/\s/gi, ''));
		...
		/* 협력사 리스트 조회 */
		function _evnetSetEntpCdList() {
			var paramInfo = {};
			paramInfo["bmanNo"] = $("input[name='bman_no']").val().replace(/\s/gi, '');
			...
			$.ajax({
				...
				dataType : 'json',
				async : false,
				url : '<c:url value="/edi/product/selectKoreanNetEntpCdList.json"/>',
				data : JSON.stringify(paramInfo),
				success : function(data) {
					var resultList	=	data.entpCdList;
					if (resultList.length > 0) {

						var eleHtml = [];
						for (var i = 0; i < resultList.length; i++) {
							eleHtml[i] = "<option value='"+resultList[i].venCd+"'>"+resultList[i].venCd+"</option>"+"\n";
						}

						$("#entpCd option").not("[value='']").remove();	//콤보박스 초기화
						$("#entpCd").append(eleHtml.join(''));
					} else {
						$("#entpCd option").not("[value='']").remove();	//콤보박스 초기화
					}
				}
		...
		function _eventSetSellCdStat(val) {
			...
			if (val == "00") {
				$("input[name='sellCd']").parent().prev().find("span.star").show();
		...
		function initIBSheetGrid3(){
 		...
 		function addCategory() {
 			...
 			if (rowCount < 3) {
 				//var targetUrl = '${ctx}/edi/product/onlineDisplayCategory.do?closeFlag=1&catCd='
 				//		+ $("#l3Cd").val();//01:상품

 				// 20180920 전산정보팀 이상구 카테고리 링크에 코리안넷 파라미터 추가.
 				var targetUrl = '${ctx}/edi/product/onlineDisplayCategory.do?closeFlag=1&catCd='
 	 						+ $("#l3Cd").val() + '&koreanNet=1';
		...
		function delEcDisplayCategory(isMart){
 		...
 		/* 180919카테고리 검색창으로 부터 받은 카테고리 정보 입력 */
		...
		function selectL4ListForOnline(onlineDisplayCategory) {
			$.getJSON("${ctx}/edi/product/selectL4ListForOnline.do",
					{
		...
		function _eventSetnewProdDetailInfoDetailValue() {
			...
			$("#newProduct select[name='sType']").val(sType); // 20190704 전산정보팀 이상구 소싱유형 추가.
		...
		function commerceChange(groupCode, val, infoGrpCd, infoGrpCd2){
			...
			setupLcdDivnCodeProdAdd(val,'1', infoGrpCd);
...
// 						}
				...
				paramInfo["productDy"]			=	paramInfo["productDy"].replace(/-/g, '').replace(/^\s*(\S*(\s+\S+)*)\s*$/, "$1");
	    		...
	    		paramInfo["admFg"] = "1";	    //무조건 업체는 1로 등록
	    		//----- 상품 수정일때만
				if ($("#hiddenForm input[name='pgmId']").val().replace(/\s/gi, '') != "") {
	    	    	...
	    	    	if (prodAttTypFg == "01") {
	    	    		var varAttCnt = _eventChkVarAttCnt(l3Cd);
	    	    	...
	    	    	/*************************************** 묶음상품에서 단일상품으로 변경시 속성정보 / 이미지 모두 삭제 ***************************************/
	    	    	//----- 상품등록이후 저장되어 있던 상품범주값이 묶음 상품이면
	 //   	    	if (oldProdAttTypFg == "01") {
	 //  	    		//----- 현재 상품범주값이 단일 상품으로 변경되거나 소분류 값이 변경되거나 협력업체 코드가 변경이 되면
	 //   	    		if (prodAttTypFg == "00" || (oldL3Cd != l3Cd) || (oldGrpCd != grpCd)) {
	 //   	    			if (!confirm("<spring:message code='msg.product.modify.prodAttTypFg'/>")) {
	 //   	    				return;
	 //   	    			}
	 //   	    			paramInfo["delGbn"] = "Y";
	 ...
	 //   	    	}
				...
				$.ajax({
					...
					success : function(data) {
						if (data.msg == "SUCCESS") {
							...
							alert("이미지를 가져오는 방식이 변경되었습니다.\n\r이미지 탭으로 이동하여 직접 이미지를 가져오세요.");
		...
		function _eventDoFix() {
			var paramInfo		=	{};
			var imgNcntyn		=	$("input[name='imgNcntyn']").val();		//온라인 이미지 첨부유무
			var prodcommerce	=	$("input[name='prodcommerce']").val();	//전상법 입력 유무
			var prodAttTypFg	=	"<c:out value='${newProdDetailInfo.prodAttTypFg}'/>";
			var varAttCnt		=	"<c:out value='${newProdDetailInfo.varAttCnt}'/>";
			var inputVarAttCnt	=	"<c:out value='${newProdDetailInfo.inputVarAttCnt}'/>";

			var vicOnlineCd 	=   "<c:out value='${newProdDetailInfo.vicOnlineCd}'/>";
			var matCd 			=   "<c:out value='${newProdDetailInfo.matCd}'/>";
			var prodDivnCd		=	"<c:out value='${newProdDetailInfo.prodDivnCd}'/>";				//상품구분[1:규격, 5:패션]
			var ecAttrRegYn		=	"<c:out value='${newProdDetailInfo.ecAttrRegYn}'/>";
			var nutAttrRegYn	=   "<c:out value='${newProdDetailInfo.nutAttrRegYn}'/>"; //선택된 상품의 영양 속성 등록 유무
			var canRegNutAttrYn =   "<c:out value='${newProdDetailInfo.canRegNutAttrYn}'/>"; //선택된 상품의 등록할 수 있는 영양 속성 존재 유무
			

			//전상법 등록 여부
			if (prodcommerce == "N") {
				alert("전상법정보를 등록해 주세요.");
				return;
			}

			//전상법 등록 여부
			if (imgNcntyn == "N") {
				alert("온라인이미지 정보가 누락되었습니다.");
				return;
			}


			if (prodDivnCd == "5" && ecAttrRegYn == "N") {
				alert("패션상품 EC 속성을 입력해주세요.");
				return;
			}

			//---영양성분 필수 입력값
			if (nutAttrRegYn == "N" && canRegNutAttrYn == "Y") {
				alert("영양성분탭에 영양성분속성이 입력되지 않았습니다. ");
				return;
			}		
			...
			if (prodAttTypFg == "01") {
				...
				if (varAttCnt > 0) {
					...
					if (inputVarAttCnt <= 0) {
						alert("묶음 상품으로 등록할 경우 변형속성은 하나 이상 반드시 입력해주세요.");
						return;
					}
				}
			}
			...
			arrPgmId[0]			=	$("input[name='pgmId']").val();
			arrOnOffGubun[0]	=	"0";
			arrImgId[0]			=	$("input[name='prodImgId']").val();
			arrTrdTypeDivnCd[0]	=	$("input[name='trdTypeDivnCd']").val();

			arrMatCd[0]			=	$("select[name='matCd']").val();
			arrVicOnlineCd[0]	=	$(":radio[name='vicOnlineCd']:checked").val();
			arrAdmFg[0]   		=	"1";

			//-----직매입, 특약1 경우 분석속성 입력 필수 체크
			var grpAttrCnt		=	"<c:out value='${newProdDetailInfo.grpAttrCnt}'/>";
			var inputGrpAttrCnt	=	"<c:out value='${newProdDetailInfo.inputGrpAttrCnt}'/>";

			/* 2016.05.12  직매입, 특약1 경우 분석속성이 있을때만 필수 입력 체크 차도록 변경 */
			/* if (arrTrdTypeDivnCd[0] == "1" || arrTrdTypeDivnCd[0] == "2") {
				if (grpAttrCnt <= 0 || (inputGrpAttrCnt < grpAttrCnt) ) {
					alert("분석속성 입력은 필수 입니다. 상품속성 탭에서 분석속성을 입력해주세요.");
					return;
				}
			} */
			...
			if (!confirm("이미지를 가져오는 방식이 변경되었습니다.\n\r이미지 탭에서 이미지를 가져오셨습니까?")) {
				return;
			}
		...
		function checkValuesIsValidatedByRep(){
			...
			if(!validationByRep(hrzn,"prdSize") || !validationByRep(vctl,"prdSize") || !validationByRep(higt,"prdSize")){
		...
		function validationByRep(str,opt)
	    {
	        ...
	        else if (opt == "prdSize"){ // 숫자만 입력 ( 중량 )
	            rst = /^[0-9]+\b/g.exec(str);
	        }
	        else if (opt == "weight"){ // 숫자만 입력 ( 중량 )
...
<body>
			...
			<div id="wrap_menu">
				...
				<div class="wrap_search">
					...
					<div id="prodTabs" class="tabs" style="padding-top:10px;">
						<ul>
							<li id="pro01" class="on"><spring:message code="msg.product.onOff.default.notice"/></li>
							<li id="pro02" style="cursor: pointer;"><spring:message code='msg.product.onOff.default.itemProperty'/></li>
							<li id="pro03" style="cursor: pointer;"><spring:message code='msg.product.tab.img'/></li>

							<!-- 단일 상품일 경우 -->
							<c:if test="${newProdDetailInfo.prodAttTypFg eq '00'}">
								<!-- 추가정보 입력란의 혼재여부가 단일일 경우에만 물류바코드 탭 disply 표기 -->
								<c:if test="${newProdDetailInfo.mixYn eq '0'}">
									<li id="pro04" style="cursor: pointer;">물류바코드</li>
								</c:if>
							</c:if>

							<!-- 묶음 상품일 경우 -->
							<c:if test="${newProdDetailInfo.prodAttTypFg eq '01'}">
								<c:if test="${newProdDetailInfo.inputVarAttCnt > 0}">
									<li id="pro04" style="cursor: pointer;">물류바코드</li>
								</c:if>
							</c:if>
							
							<li id="pro05" style="cursor: pointer;">영양성분</li>
							<!-- 물류바코드 탭 (상품 기본정보가 저장되고 혼재여부가 단일상품일때만 나면 활성화) -->
							<%-- <c:if test="${not empty newProdDetailInfo.pgmId && newProdDetailInfo.mixYn eq '0' && newProdDetailInfo.prodAttTypFg eq '00'}">
								<li id="pro04" style="cursor: pointer;">물류바코드</li>
							</c:if> --%>
						</ul>
					</div>
					...
					<div class="bbs_search">
						<ul class="tit">
							<li class="tit">* <spring:message code="msg.product.onOff.default.footerRegNewItem"/> : <spring:message code="msg.product.onOff.register.notice"/></li>
							<!------------------------------------------------------------------------------------------------->

							<!-- 확정요청이  아닐 때만 버튼 활성화 -->
							<c:if test="${empty newProdDetailInfo.mdSendDivnCd}">

								<li class="btn">
									<a href="#" class="btn" onclick="_eventSave();"><span><spring:message code="button.common.save" /></span></a>

									<!-- 상품정보 저장 후 확정요청 버튼 활성화 -->
									<c:if test="${not empty newProdDetailInfo.pgmId}">
										<a href="#" class="btn" onclick="_eventDoFix();"><span>확정요청</span></a>
									</c:if>
					...
					<form name="newProduct"		id="newProduct">
						...
						<input type="hidden" 	name="onOffDivnCd"						id="onOffDivnCd" 		value="0" 	/>		<!-- 온오프상품구분[0:온오프, 1:온라인, 2:소셜] -->
						<input type="hidden" 	name="maxKeepDdCnt"						id="maxKeepDdCnt" 		value="0" 	/>		<!-- 최대보관일수 -->
						<input type="hidden"	name="brandCode" 						id="brandCode"			value="<c:out value='${newProdDetailInfo.brandCode}'/>"			/>		<!-- 브랜드코드 -->
						<input type="hidden" 	name="makerCode" 						id="makerCode"  		value="<c:out value='${newProdDetailInfo.makerCode}'/>"			/> 		<!-- 메이커코드 -->
						<input type="hidden" 	name="prodImgId" 						id="prodImgId"  		value="<c:out value='${newProdDetailInfo.prodImgId}'/>"			/> 		<!-- 상품 가등록 코드 -->
						<input type="hidden" 	name="newProdGenDivnCd" 				id="newProdGenDivnCd"  	value="KOR"	/> 		<!-- 새상품 구분 -->
						<input type="hidden" 	name="categoryId"  		id="categoryId" value="${newProdDetailInfo.categoryId}" />
						<!-- 200306 EC전용 카테고리 -->
						...
						<div class="bbs_list">
							...
							<table class="bbs_grid3" cellpadding="0" cellspacing="0" border="0">
								...
								<tr>
									<th colspan="4" align="left" style="padding-left:15px;">
										<strong><font color="red">* 판매코드는 입력란 옆에 있는 판매코드를 참조하여 직접 입력해주세요.(묶음상품으로 등록 할 경우 '상품속성' 탭에서 속성별로 직접 입력요망)</font></strong><br/>
										<strong><font color="red">* 최종 기본정보 탭에서 '확정요청'을 반드시 해주세요.</font></strong><br/>
										<strong><font color="red">* 추가정보 입력란의 혼재여부가 '혼재상품'일 경우에는 물류바코드를 입력 하실 수 없습니다.</font></strong><br/>
										<strong><font color="red">* 상품범주 를 '묶음상품'으로 등록하실 경우 '상품속성' 탭에서 해당 소분류의 변형속성을 입력 저장 하신 후에 '물류바코드' 탭이 활성화 됩니다.</font></strong><br/>
										<strong><font color="red">* 상품범주 를 '묶음상품'으로 등록하실 경우 '물류바코드' 탭에서 속성별 물류바코드 등록이 가능합니다.</font></strong><br/>
									</th>
								</tr>
								...
								<tr>
									...
									<td>
										<div>
											<b><spring:message code="msg.product.onOff.default.onOffDivnCode"/></b>
										</div>
									</td>
									<th><span class="star">*</span> 수기등록 여부</th>
									<td>
										<div>
											<font color="blue" style="font-size: 11px">※안내사항: 수기등록 이미지 상품 일 경우 Y 선택 필수</font><br>
											<input type="radio" class="required" name="mnlProdReg"
												id="mnlProdReg" value="0" />N <input type="radio"
												class="onOffField" name="mnlProdReg" id="mnlProdReg"
												value="1" />Y
										</div>
									</td>
								</tr>
								...
								<tr>
									...
									<th><span class="star">*</span><spring:message code="msg.product.onOff.default.prodAttTypFg"/></th>
									<td>
										<div>
											<input type="radio"		name="prodAttTypFg"		id="prodAttTypFg"		class="required"	 value="00"> <spring:message code="msg.product.onOff.default.danItem"/>
											<input type="radio"		name="prodAttTypFg"		id="prodAttTypFg"		class="required"	 value="01"> <spring:message code="msg.product.prodAttTypFg.txt2"/>
										</div>
									</td>
								</tr>
								...
								<tr>
									<th>
										...
										<spring:message code="msg.product.onOff.default.l2Cd"/>
									</th>
									<td >
									...
									<th><span class="star">*</span><spring:message code="msg.product.onOff.default.detailClass"/></th>
									<td >
								...
								<tr>
									...
									<td colspan="1">
										<%-- <c:choose>
											<c:when test="${empty newProdDetailInfo.sellCd}">
												<input type="text" name="sellCd" 		id="sellCd"		 maxlength="13" 	style="width:150px;" 	class="sell88Code" 		maxlength="13"		value="<c:out value='${param.srcmk_cd}'/>"/>
											</c:when>
											<c:otherwise>
												<input type="text" name="sellCd" 		id="sellCd"		 maxlength="13" 	style="width:150px;" 	class="sell88Code" 		maxlength="13"		value="<c:out value='${newProdDetailInfo.sellCd}'/>"/>
											</c:otherwise>
										</c:choose> --%>
										<input type="text" name="sellCd" 		id="sellCd"		 maxlength="13" 	style="width:150px;" 	class="sell88Code" 		maxlength="13"		value="<c:out value='${newProdDetailInfo.sellCd}'/>"/>
										<c:choose>
											<c:when test="${empty newProdDetailInfo.sellCd}">
												(<c:out value="${param.srcmk_cd}"/>)
											</c:when>
											<c:otherwise>
												(<c:out value="${newProdDetailInfo.sellCd}"/>)
											</c:otherwise>
										</c:choose>
									</td>
									<th>
										<span class="star">*</span>
										<spring:message code="msg.product.onOff.default.grpClass"/>
									</th>
									<td >
										<select id="grpCd" name="grpCd"	 class="required">
											<option value=""><spring:message code="button.common.choice"/></option>
										</select>
									</td>
								</tr>
								...
								<tr>
									...
									<td>
										<c:choose>
											<c:when test="${empty newProdDetailInfo.prodStandardNm}">
												<input type="text" name="prodStandardNm"	id="prodStandardNm"		 maxlength="15" style="width:150px;" 	value="<c:out value='${param.prod_std}'/>"/>
											</c:when>
											<c:otherwise>
												<input type="text" name="prodStandardNm"	id="prodStandardNm"		 maxlength="15" style="width:150px;" 	value="<c:out value='${newProdDetailInfo.prodStandardNm}'/>"/>
											</c:otherwise>
										</c:choose>
									...
									<td>
										<html:codeTag attr="class=\"required\"" objId="taxatDivnCd" objName="taxatDivnCd" parentCode="PRD06" width="150px;" comType="SELECT" formName="form" defName="선택"  />
									</td>
								</tr>
								...
								<tr>
									...
									<td>
										...
										<div name="productSize">
											<c:choose>
												<c:when test="${empty newProdDetailInfo.prodHrznLeng}">
													<font style="font-size: 10px">
											        <spring:message code="msg.product.onOff.default.width"/>
											        </font>
													<input type="text" name="prodHrznLeng" 		id="prodHrznLeng"		style="width:40px" 		class="required number default0 size"  maxlength="4"	value="<c:out value='${param.prod_width}'/>"	/>
												</c:when>
												<c:otherwise>
												    <font style="font-size: 10px">
											        <spring:message code="msg.product.onOff.default.width"/>
											        </font>
													<input type="text" name="prodHrznLeng" 		id="prodHrznLeng"		style="width:40px" 		class="required number default0 size"  maxlength="4"	value="<c:out value='${newProdDetailInfo.prodHrznLeng}'/>"	/>
												</c:otherwise>
											</c:choose>

											<c:choose>
												<c:when test="${empty newProdDetailInfo.prodVtclLeng}">
													<font style="font-size: 10px">
											        <spring:message code="msg.product.onOff.default.length"/>
											        </font>
												    <input type="text" name="prodVtclLeng" 		id="prodVtclLeng"		style="width:40px"   	class="required number default0 size"  maxlength="4"	value="<c:out value='${param.prod_length}'/>"	/>
												</c:when>
												<c:otherwise>
													<font style="font-size: 10px">
											        <spring:message code="msg.product.onOff.default.length"/>
											        </font>
													<input type="text" name="prodVtclLeng" 		id="prodVtclLeng"		style="width:40px"   	class="required number default0 size"  maxlength="4"	value="<c:out value='${newProdDetailInfo.prodVtclLeng}'/>"	/>
												</c:otherwise>
											</c:choose>

											<c:choose>
												<c:when test="${empty newProdDetailInfo.prodHigt}">
											        <font style="font-size: 10px">
											        <spring:message code="msg.product.onOff.default.height"/>
											        </font> 	
													<input type="text" name="prodHigt" 			id="prodHigt"			style="width:40px"      class="required number default0 size"  maxlength="4"	value="<c:out value='${param.prod_height}'/>"	/>
												</c:when>
												<c:otherwise>
											        <font style="font-size: 10px">
											        <spring:message code="msg.product.onOff.default.height"/>
											        </font>
													<input type="text" name="prodHigt" 			id="prodHigt"			style="width:40px"      class="required number default0 size"  maxlength="4"	value="<c:out value='${newProdDetailInfo.prodHigt}'/>"	/>
												</c:otherwise>
											</c:choose>
									...
									<th><span class="star">*</span><spring:message code="msg.product.sizeUnit"/></th>
									<td>
										<html:codeTag objId="sizeUnit" objName="sizeUnit" parentCode="PRD42" comType="SELECT" formName="form" selectParam="MM" attr="class=\"required\""/>
										<!-- <select name="sizeUnit"		id="sizeUnit"	class="required"	style="width:50px;">
											<option value="">선택</option>
								...
								<tr>
									<th><span class="star">*</span> <spring:message
											code="msg.product.totWeight" /></th>
									<td>
										<div name="productTotWeight">
											<font color="blue" style="font-size: 11px">※총중량: 패키지를 포함한 상품의 전체 무게</font>
											<input type="text" name="totWeight" id="totWeight"
												class="required rate" style="width: 85px;" maxlength="5"
									...
									<th><span class="star">*</span> 순중량</th>
									<td>
										<div name="productNetWeight">
											<font color="blue" style="font-size: 11px">※순중량: 패키지가 포함되지 않은 순수 상품의 무게</font>
											<input type="text" name="netWeight" id="netWeight"
												...
												&nbsp;
											<font color="red" style="font-size: 11px">(총중량, 순중량 단위 같음.)</font>
										</div>
									</td>
								...
								<tr>
									...
									<td colspan=3>
										<c:choose>
											<c:when test="${empty newProdDetailInfo.prodNm}">
												<input type="text" name="prodNm"		id="prodNm" 	class="required"  style="width:390px;" 	maxlength="50" 		value="<c:out value='${param.prod_nm}'/>"/>
											</c:when>
											<c:otherwise>
												<input type="text" name="prodNm"		id="prodNm" 	class="required"  style="width:390px;" 	maxlength="50" 		value="<c:out value='${newProdDetailInfo.prodNm}'/>"/>
											</c:otherwise>
										</c:choose>
									</td>
								</tr>
								...
								<tr>
									...
									<td colspan=3>
										<input type="text" name="prodEnNm"		id="prodEnNm" 	style="width:390px;ime-mode:disabled;" 	maxlength="50" 		value="<c:out value='${newProdDetailInfo.prodEnNm}'/>"		/>
										<font color="red"><spring:message code="msg.product.itemNmEn2"/></font>
									</td>
								</tr>
								...
								<tr>
									...
									<td colspan=3>
										<c:choose>
											<c:when test="${empty newProdDetailInfo.prodShortNm}">
												<input type="text" name="prodShortNm" 	id="prodShortNm""	class="required" style="width:390px;" maxlength="30" 	value="<c:out value='${param.short_nm}'/>"/>
											</c:when>
											<c:otherwise>
												<input type="text" name="prodShortNm" 	id="prodShortNm""	class="required" style="width:390px;" maxlength="30" 	value="<c:out value='${newProdDetailInfo.prodShortNm}'/>"/>
											</c:otherwise>
										</c:choose>
								...
								<tr>
									...
									<td>
										<html:codeTag   objId="dpUnitCd" objName="dpUnitCd" parentCode="PRD17" width="150px;" comType="SELECT" formName="form" defName="선택"  />
									</td>
									<th><span class="star"></span><spring:message code="msg.product.onOff.default.displayStandardQuantity"/></th>
									<td>
										 <input type="text" name="dpBaseQty"	id="dpBaseQty"	maxlength="5" style="width:150px;" 	class="requiredIf number default0"		value="<c:out value='${newProdDetailInfo.dpBaseQty}'/>"	/>
									</td>
								</tr>
								...
								<tr>
									...
									<td>
										 <input type="text" 	name="dpTotQty"		id="dpTotQty" 	maxlength="5"  style="width:150px;" 	class="requiredIf number default0"	value="<c:out value='${newProdDetailInfo.dpTotQty}'/>"/>
									</td>
									...
									<td>
										<div>
											...
											<!-- <select name="norProdSaleCurr"		id="norProdSaleCurr"	class="required"	style="width:50px;">
												<option value="">선택</option>
								...
								<tr>
									...
									<td >
										<input type="text" name="prftRate"		id="prftRate"	value="<c:out value='${newProdDetailInfo.prftRate}'/>"  	class="requiredIf rate"  maxlength="5" style="width:150px;"		readonly/>
									</td>

									<th><span class="star">*</span><spring:message code="msg.product.onOff.default.norProdPcost"/></th>
									<td>
										<div>
											<input type="text" name="norProdPcost"		id="norProdPcost"		value="<c:out value='${newProdDetailInfo.norProdPcost}'/>" 			class="required number range cost" maxlength="9" style="width:150px;" 	/>
											...
											<!-- <select name="norProdCurr"		id="norProdCurr"	class="required"	style="width:50px;">
												<option value="">선택</option>
												<option value="KRW">KRW</option>
											</select> -->
										</div>
								...
								<tr>
									<th colspan='4'>
										<b><spring:message code="msg.product.onOff.default.normalItemTxt"/><b>
										<br/>
								...
								<tr>
									...
									<th><spring:message code="msg.product.onOff.default.vicOnlineCd"/></th>
									<td>
										<div class="matchVicOnlinoption" style="display:none">
											<input type="radio" class="onOffField" name='vicOnlineCd' id="vicOnlineCd"	value=""><spring:message code="msg.product.onOff.default.notApply"/>
											<input type="radio" class="onOffField" name='vicOnlineCd' id="vicOnlineCd" value="X"><spring:message code="msg.product.onOff.default.apply"/>
										</div>
										<div class="nomatchVicOnlinoption" ><spring:message code="msg.product.onOff.default.notUse"/></div>
									</td>
								</tr>
								<tr>
								<th>
								...
								<th><span class="star">*</span> <spring:message code="msg.product.onOff.default.vicNormalItemPrice"/></th>
									<td>
										<div>
											...
											<!-- <select name="wnorProdSaleCurr"		id="wnorProdSaleCurr"	class="required"	style="width:50px;">
												<option value="">선택</option>
												<option value="KRW">KRW</option>
											</select> -->
										</div>
								...
								<tr>
									...
									<td>
										<input type="text" name="wprftRate"		id="wprftRate"  	value="<c:out value='${newProdDetailInfo.wprftRate}'/>"	class="requiredIf vicrate"  maxlength="5" style="width:150px;" readonly		/>
									</td>
									<th><span class="star">*</span><spring:message code="msg.product.onOff.default.vicNormalCost"/></th>
									<td>
										<div>
											<input type="text" name="wnorProdPcost"		id="wnorProdPcost"	 class="required number vicrange viccost" 	maxlength="9" 	style="width:150px;" 	value="<c:out value='${newProdDetailInfo.wnorProdPcost}'/>"	/>
						...
						<div class="bbs_list">
							<ul class="tit">
								<li class="tit"><spring:message code="msg.product.onOff.default.AdditionalInfo"/>
							...
							<table class="bbs_grid3" cellpadding="0" cellspacing="0" border="0"		id="tblNewProdAdd">
								<colgroup>
									<col style="width:15%" />
									<col style="width:30%" />
									<col style="width:17%" />
								...
								<tr>
									...
									<th><span class="star">*</span> <spring:message code="msg.product.onOff.default.sType" /></th>
									<td>
								...
								<tr>
									<th><spring:message code="msg.product.wasteFg"/></th>
									<td>
								...
								<tr>
									<th><spring:message code="msg.product.recycleFg1"/></th>
									<td>
								...
								<tr>
									<th><spring:message code="msg.product.recycleFg2"/></th>
									<td>
								...
								<tr>
									<th><spring:message code="msg.product.recycleFg3"/></th>
									<td>
								...
								<tr>
									...
									<td>
										...
										<input type="radio"	class="onOffField" name="ownStokFg" id="ownStokFg" value="X" /> 대상
									</td>
									<th>개별계획량</th>
									<td>
									<font color="blue" style="font-size: 11px">개별계획량 : 계약량(최저납품량)</font>
										<br/>
										<input tupe="text" class="onOffField requiredIf number" name = "planRecvQty" id ="planRecvQty"
											value="<c:out value='${newProdDetailInfo.planRecvQty}'/>" />
									</td>
								</tr>
								<tr>
									<th>계획종료일</th>
									<td>
										...
										<input type="text" maxlength="8" class="requiredIf"
										name="ordDeadline" id="ordDeadline" style="width: 80px;"
										value="<c:out value='${newProdDetailInfo.ordDeadline}'/>"
										readonly /> <img src="/images/epc/layout/btn_cal.gif"
										class="middle" onClick="openCal('newProduct.ordDeadline');"
										style="cursor: hand;" id="ordDeadlineImg"/></td>
								</tr>
								...
								<tr>
									...
									<td>
										<div class="jangoption">
											<input type="radio"  class="onOffField" name='newProdPromoFg'		id="newProdPromoFg"		value="">	<spring:message code="msg.product.onOff.default.notApply"/>
											<input type="radio"  class="onOffField" name='newProdPromoFg' 		id="newProdPromoFg"		value="X">	<spring:message code="msg.product.onOff.default.apply"/>
										</div>
								...
								<tr>
									...
									<td>
										<div class="jangoption">
											<input type="radio"  	name='overPromoFg'		id="overPromoFg"	class="onOffField"  value=""><spring:message code="msg.product.onOff.default.notApply"/>
											<input type="radio" 	name='overPromoFg'		id="overPromoFg"	class="onOffField"  value="X"><spring:message code="msg.product.onOff.default.apply"/>
										</div>
										<div class="nojangoption" style="display:none"><spring:message code="msg.product.onOff.default.notUse"/></div>
									</td>
									<th><span class="star">*</span><spring:message code="msg.product.onOff.default.expireDt"/></th>
									<td>
										 <html:codeTag  attr="class=\"required\"" objId="flowDdMgrCd" objName="flowDdMgrCd" parentCode="PRD29" width="150px;" comType="SELECT" formName="form" defName="선택" />
									</td>
								</tr>


								<tr>
									<th><spring:message code="msg.product.flowDD"/></th>
									<td>
										<div class="flowDate">
											<input type="text"  class="requiredIf number flowDate"  maxlength="4" name='flowDd'		id="flowDd"		value="<c:out value='${newProdDetailInfo.flowDd}'/>"/>일
										</div>
									...
									<td>
										<div class="flowDate">
											<input type="text"  class="requiredIf number flowDate"  maxlength="3" name='stgePsbtDd'	id="stgePsbtDd"		value="<c:out value='${newProdDetailInfo.stgePsbtDd}'/>"/>일
										</div>
										<div class="noFlowDate" style="display:none"><spring:message code="msg.product.onOff.default.notUse"/></div>
									</td>
								</tr>
								...
								<tr>
									...
									<td><div>
										<input type="radio"  	name='npurBuyPsbtDivnCd'		id="npurBuyPsbtDivnCd"	class="onOffField required"  value=""><spring:message code="msg.product.onOff.default.notApply"/>
										<input type="radio" 	name='npurBuyPsbtDivnCd'		id="npurBuyPsbtDivnCd"	class="onOffField required"  value="X"><spring:message code="msg.product.onOff.default.apply"/>
										</div>
									</td>

									<th><spring:message code="msg.product.onOff.default.releasePossibleDt"/></th>
									<td><div class="flowDate">
										 <input type="text" name="pickPsbtDd" 	id="pickPsbtDd"		maxlength="3"  class="requiredIf number flowDate" 	value="<c:out value='${newProdDetailInfo.pickPsbtDd}'/>"/><spring:message code="msg.product.date"/>
										 </div>
										 <div class="noFlowDate" style="display:none"><spring:message code="msg.product.onOff.default.notUse"/></div>
									</td>
								</tr>
								...
								<tr>
									...
									<td>
									<div>
									<input type="radio" 	name="newProdFirstPurDivnCd"		id="newProdFirstPurDivnCd"	class="onOffField required" 	value="0" /><spring:message code="msg.product.onOff.default.notApply"/>
									</div>
									</td>
								...
								<tr>
									...
									<td>
										<input type="radio" 	class="onOffField"  name="totInspYn" 	id="totInspYn" 		value="" /><spring:message code="msg.product.onOff.default.notTarget"/>
										<input type="radio"  	class="onOffField" 	name="totInspYn" 	id="totInspYn"		value="X"/><spring:message code="msg.product.onOff.default.target"/>
									</td>
									<th><spring:message code="msg.product.onOff.default.balhangDt"/></th>
									<td>
										<c:choose>
											<c:when test="${empty newProdDetailInfo.productDy}">
												<input type="text" maxlength="8" class="requiredIf"  name="productDy"		id="productDy"	 	style="width:80px;" 	value="<c:out value='${issueDate}'/>"		readonly/>
											</c:when>
											<c:otherwise>
												<input type="text" maxlength="8" class="requiredIf"  name="productDy"		id="productDy"	 	style="width:80px;" 	value="<c:out value='${newProdDetailInfo.productDy}'/>"		readonly/>
											</c:otherwise>
										</c:choose>

										<img src="/images/epc/layout/btn_cal.gif" class="middle" onClick="openCal('newProduct.productDy');" style="cursor:hand;" />
									</td>
								</tr>
								...
								<tr>
									...
									<td>
										<html:codeTag  attr="class=\"onOffField required\"" objId="protectTagDivnCd" objName="protectTagDivnCd" 	parentCode="ALL14" width="150px;" comType="SELECT" formName="form" 	defName="선택"  />
									</td>
									<th><span class="star">*</span><spring:message code="msg.product.onOff.default.safeTagTypeGbn"/></th>
									<td >
										<html:codeTag  attr="class=\"onOffField required\"" objId="protectTagTypeCd" objName="protectTagTypeCd" parentCode="PRD01" width="150px;" comType="SELECT" formName="form"   	defName="선택"/>
									</td>
								</tr>
								...
								<tr>
									...
									<td>
										<html:codeTag attr="class=\"onOffField required\"" objId="ctrTypeDivnCd" objName="ctrTypeDivnCd" parentCode="PRD12" width="150px;" comType="SELECT" formName="form" defName="선택"  />
									</td>
									<th><span class="star">*</span><spring:message code="msg.product.onOff.default.temperature"/></th>
									<td >
										<html:codeTag attr="class=\"required\"" objId="tmprtDivnCd" objName="tmprtDivnCd" parentCode="PRD30" width="150px;" comType="SELECT" formName="form" defName="선택"  />
									</td>
								</tr>
								...
								<tr>
									...
									<td>
										 <input type="text" maxlength="30" class="requiredIf" id="modelNm" name="modelNm" style="width:150px;" 		value="<c:out value='${newProdDetailInfo.modelNm}'/>"/>

									</td>
									<th><span class="star">*</span><spring:message code="msg.product.onOff.default.country"/></th>
									<td>
										<html:codeTag attr="class=\"required\"" objId="homeCd" objName="homeCd" parentCode="PRD16" width="150px;" comType="SELECT" formName="form" defName="선택"  />
									</td>
								</tr>
									<th><spring:message code="msg.product.onOff.seasonGbn"/></th>
									<td>
										<div>
											<select name="sesnYearCd"		id="sesnYearCd"		class="required">
												<option value=""><spring:message code="button.common.choice"/></option>
											...
											<select id="sesnDivnCd"		name="sesnDivnCd"		class="requiredIf">
												<option value="">선택</option>
											</select>
										</div>
									</td>
								</tr>
								<!-- --20170701 수입신고번호 추가 start -->
								<tr>
									<th><spring:message code="msg.product.onOff.default.decno"/></th>
									<td>
										 <input type="text" maxlength="14" class="requiredIf decno" id="decno" name="decno" style="width:150px;" 		value="<c:out value='${newProdDetailInfo.decno}'/>"/>

									</td>
									<th><spring:message code="msg.product.onOff.default.tarrate"/></th>
									<td>
										 <input type="text" maxlength="30" class="requiredIf rate" id="tarrate" name="tarrate" style="width:150px;" 		value="<c:out value='${newProdDetailInfo.tarrate}'/>"/>
								...
								<tr>
									...
									<td>
										 <input type="text" maxlength="10" class="requiredIf hscd" id="hscd" name="hscd" style="width:150px;" 		value="<c:out value='${newProdDetailInfo.hscd}'/>"/>

									</td>
								</tr>
								<!-- --20170701 수입신고번호 추가 end -->

								<tr>
									<th><spring:message code="msg.product.onOff.default.brand"/></th>
									<td>
										 <input type="text" maxlength="30" class="requiredIf" id="brandName" name="brandName" style="width:150px;" 		value="<c:out value='${newProdDetailInfo.brandName}'/>"		disabled="disabled"/>
									...
									<td >
										 <input type="text" maxlength="30" class="requiredIf"  id="makerName" name="makerName" style="width:150px;"		value="<c:out value='${newProdDetailInfo.makerName}'/>" 	disabled="disabled"/>
										 <a href="javascript:openMakerPopup();" class="btn" ><span><spring:message code="button.common.choice"/></span></a>
									</td>
								</tr>
								...
								<tr>
									...
									<td colspan="3">
										 <input type="text" id="dlvDt" name="dlvDt" style="width:90%;" maxlength="50"	value="<c:out value='${newProdDetailInfo.dlvDt}'/>"/>
									</td>
								</tr>
								...
								<tr>
									...
									<td colspan="3">
										 <input type="text" id="ispDtlDesc" name="ispDtlDesc" style="width:100%;" maxlength="200"		value="<c:out value='${newProdDetailInfo.ispDtlDesc}'/>"/>
									</td>
								</tr>
								...
								※ ,(커머) ;(세미콜론) |(버티컬 바) 와 같은 특수문자는 사용하실 수 없으며 1행당 검색어 1개를 입력해 주세요.<br>
							 	※ 띄어쓰기, 영문 대문자는 사용하실 수 없습니다.	<br>
										</font> <c:if test="${empty tpcPrdKeywordList}">
										...
										<table id='keywordSubTable' style='width: 700px'>
											...
											<c:forEach var="keywordList" items="${tpcPrdKeywordList}"
												...
												<tr id="kwRow${index.count}">
													<td style='text-align: center'>${keywordList.num}<input
														type='hidden' name='seq' id='seq${index.count}'
														value='${keywordList.seq}' />
													</td>
													<td><input type='text'
														style='text-align: left; width: 98%;' name='searchKywrd'
														id='searchKywrd${index.count}'
														value='${keywordList.searchKywrd}' maxlength="39"
														onblur="limitPushString(${index.count});" /></td>
													...
													<c:choose>
														...
														<c:otherwise>
															<td><a
																href='javascript:fnKeywordDelete("${index.count}", "${keywordList.seq}")'
																id='deleteNewKeyword${index.count}' class='btn'><span>삭제</span></a>
															</td>
														</c:otherwise>
													</c:choose>
								...
								<div class="bbs_list">
									<ul class="tit">
										...
										<li class="tit" id="productCertDtlSelectBox" style="display:none">
											<select id="productCertSelectDtlTitle" name="productCertSelectDtlTitle" style="display:none">
												<option value="">선택</option>
											</select>
										</li>
									</ul>
						...
						<div class="bbs_list">
							<ul class="tit">
									<li class="tit">*<spring:message code="msg.product.descrTxt"/> : <font color="red"><spring:message code="msg.product.descrTxt2"/></font></li>
									<%-- <li class="btn">
							...
							<table class="bbs_grid3" cellpadding="0" cellspacing="0" border="0">
								<tr>
									<td>
										<c:choose>
											<c:when test="${empty newProdDetailInfo.prodDesc}">
												<input type="hidden"	name="viewProdDesc"		id="viewProdDesc"	value="<c:out value='${param.prod_detail}'/>">
											</c:when>
											<c:otherwise>
												<input type="hidden"	name="viewProdDesc"		id="viewProdDesc"	value="<c:out value='${newProdDetailInfo.prodDesc}'/>">
											</c:otherwise>
										</c:choose>
	...
	<form name="hiddenForm"	id="hiddenForm">
		...
		<input type="hidden"	name="imgNcntyn"			   id="imgNcntyn"			   value="<c:out value='${newProdDetailInfo.imgNcntyn }'/>"	/>	<!-- 온라인 이미지 첨부유무 -->
		<input type="hidden"	name="prodcommerce"			 id="prodcommerce"	   value="<c:out value='${newProdDetailInfo.prodcommerce }'/>"	/>	<!-- 전상법 첨부유무 -->
		<input type="hidden"	name="trdTypeDivnCd"		 id="trdTypeDivnCd"	   value="<c:out value='${newProdDetailInfo.trdTypeDivnCd }'/>"/>	<!-- 거래형태 구분 -->
		<input type="hidden"	name="cfmFg"				     id="cfmFg"				     value="<c:out value='${newProdDetailInfo.cfmFg }'/>"/>			<!-- 상품확정구분 -->
		...
		<input type="text"	name="bman_no"				     id="bman_no"				   value="<c:out value='${param.bman_no}'/>"					/>	<!-- 코리안넷에서 넘어온 사업자등록번호 -->
	</form>
</body>
</html>
