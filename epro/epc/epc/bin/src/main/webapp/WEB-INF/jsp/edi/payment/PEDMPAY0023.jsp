...
<script>
	...
	function printText(){
		...
		form.action  = "<c:url value='/edi/payment/PEDMPAY0023Text.do'/>";
		form.submit();	
		...
		<div id="wrap_menu">
			...
			<div class="wrap_con">
				...
				<div class="bbs_list">
					...
					<c:set var="tot3"  value="0" />
					
					<c:forEach items="${paymentList}" var="list" varStatus="index" >
					<tr class="r1">
						<td align="center"><a href="javascript:forwarding('${list.BUY_YM }');">${list.BUY_YM }</a></td>
						...
						<td align="right"><fmt:formatNumber value="${list.REM_SUM }" type="number" currencySymbol="" /></td>
					</tr>
						<c:set var="tot1"  value="${tot1 + list.REM_BUY_AMT}" />
						<c:set var="tot2"  value="${tot2 + list.REM_VAT}" />
						<c:set var="tot3"  value="${tot3 + list.REM_SUM}" />
					</c:forEach>
					</c:if>
					...
					<c:if test="${not empty paymentList }">
					<tr class="r1">
						<th align="center" colspan="3"><b>합계</b></th>
						<th align="right"><b><fmt:formatNumber value="${tot1 }" type="number" currencySymbol="" /></b></th>
						<th align="right"><b><fmt:formatNumber value="${tot2 }" type="number" currencySymbol="" /></b></th>
						<th align="right"><b><fmt:formatNumber value="${tot3 }" type="number" currencySymbol="" /></b></th>
						<th>&nbsp;</th>
					</tr>
					</c:if>
					<c:if test="${empty paymentList }">
						<tr><td colspan=6>&nbsp;</td><th width=17>&nbsp;</th></tr>
					</c:if>
					</table>
				</div>
			</div>
