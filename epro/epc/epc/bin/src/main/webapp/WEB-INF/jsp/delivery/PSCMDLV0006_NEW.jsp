...
function doubleSubmitCheck(){
    if(doubleSubmitFlag){
    	 alert("이미 실행중입니다.");
    	 setTimeout(function doubleSubmitCheck(){ doubleSubmitFlag = false;  },3000);
    }else{
...
//    		return;
	...
	var url = '<c:url value="/delivery/selectPartnerFirmsOrderListNew.do"/>';
...
//    		return;
	...
	//alert(gubun);
  	if(gubun=='up'){
		var url = '<c:url value="/delivery/selectPartnerFirmsOrderListExcelFormNew.do"/>';
  	}else{
		var url = '<c:url value="/delivery/selectPartnerFirmsOrderListExcelNew.do"/>';
  	}
	//f.pageIndex.value = pageIndex;
	f.action = url;
	f.submit();	
...
// 		//document.oncontextmenu = function(){ // 컨택스트 메뉴금지			  
// 		//	 return false;			  
// 		//	 }; 
			
// 	window.document.ondragstart = new Function("return false")	
// 	//    document.ondragstart = function(){ // 컨택스트 메뉴금지			  
// 	//			 return false;			  
// 	//			 }; 
...
// //	alert("6");
//   //오른쪽 버튼 방지
//   document.oncontextmenu=ctlEvt_RightClick;
// }

// //마우스 드레그 막기 
// function NoBodyDrag() {
//   document.ondragstart=ctlEvt_RightClick;
// }

// //블럭 선택금지 
// function NoBodySelect() {
// //	alert("블록");
//   document.onselectstart=ctlEvt_RightClick;
// }
...
<%-- 						<input type="hidden" id="strVendorId" name="strVendorId" value="${searchVO.strVendorId}"> --%>
            ...
            ③ 배송중 단계에서 택배사/송장번호가 올바르다면 배송완료 단계로 전환 및 매출 확정</br>2. 설치/직배송/무형</br>① 미발송 → 배송예정/배송중 수기 전환</br>② 배송중 단계에서 자동 Batch에 의한 매출확정 </b></font>
...
<%--                         <col style="width:70px" /> --%>
					...
					<tr>
						<th rowspan="2">주문번호<br/><font color='red'>(원주문번호)</font></th>
						<th>상품코드</th>
						<th>판매코드</th>
						<th colspan="3">상품명</th>
						<th>상품가격</th>
			...
			<c:choose>			
				<c:when test="${list.ROWSPAN1<2}">
					<tr >
						...
						<td rowspan="2" style="background: ${divColor}"><c:out value="${list.ORDER_ID}"/><br/><font color='red'><c:out value="${list.FIRST_ORDER_ID}"/></font></td>
						...
						<td style="background: ${divColor}" colspan="2" rowspan="<c:out value="${list.ROWSPAN2}"/>" id="chk2_${tr_cnt}">
							...
							<br/><br/>
							<!-- 지연/불가상세사유 -->
								<input type="text" name="dlayUnavlDtlReason" style="width:70%;" disabled="disabled" value="<c:if test="${list.VEN_DELI_STATUS_CD == '02' || list.VEN_DELI_STATUS_CD == '07'}">${list.DLAY_UNAVL_DTL_REASON }</c:if>"/>
