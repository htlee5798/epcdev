<%--
	Page Name 	: NEDMINVO0010.jsp
	Description : 재고정보 현재고(점포) 조회 페이지 
...
<script id="dataListTemplate" type="text/x-jquery-tmpl">
	<tr class="r1">
		<td rowspan="2" align="center"><a href="javascript:forwarding('<c:out value="\${strCd }"/>');"><c:out value="\${strNm }"/></a></td>
		...
		<!-- <td align="right"><c:out value="\${nowStkQty }"/></td> -->
	...
	<tr class="r1">
		...
		<!-- <td align="right"><c:out value="\${nowStkAmt }"/></td> -->
...
<script>
	...
	function doSearch() {
		...
		searchInfo["srchEndDate"]		=	"";       // 조회기간 to
		...
		//if(dateValid()){
			$.ajax({
				...
				dataType : 'json',
				async : false,
				url : '<c:url value="/edi/inventory/NEDMINV0010Select.json"/>',
	...
	function doExcel(tbName1, tbName2){		
		...
		var bodyValue = "";
		bodyValue += "<CAPTION><spring:message code='text.inv.field.invStrCdList'/><br>";
	...
	function forwarding(val) {
		...
		loadingMaskFixPos();
		$("#searchForm input[name='storeVal']").val(val);
		$("#searchForm input[name='isForwarding']").val("Y");
		
		$("#searchForm").attr("action", "<c:url value='/edi/inventory/NEDMINV0020.do'/>");
		$("#searchForm").submit();
	}
	...
	function printText(){
		...
		//if(dateValid()){
			$("#searchForm").attr("action", "<c:url value='/edi/inventory/NEDMINV0010Text.do'/>");
			$("#searchForm").submit();
		//}	
...
<body>
	...
	<div>
		...
		<input type="hidden" name="isForwarding" id="isForwarding" />
		...
		<div id="wrap_menu">
			...
			<div class="wrap_con">
				...
				<div class="bbs_list">
					...
					<tr>
						<th rowspan="2"><spring:message	code="epc.inv.header.strNm" /></th>
		...
		<div id="footbox">
			...
			<div class="location">
				<ul>
					...
					<li class="last"><spring:message code="epc.inv.menu.strCd" /></li>
				</ul>
			</div>
		</div>
	</div>
