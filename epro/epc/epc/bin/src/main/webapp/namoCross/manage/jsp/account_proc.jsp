...
<%
	...
	if((encrypt.equals(u_pass) || encrypt2.equals(u_pass)) && login_id.equals(enc_id))
	{
			...
			{
				session.invalidate(); 
				result_sc = "<script>alert(NamoSELang.pe_qq);window.document.location.href='../index.html';</script>";
			}
			else
			{
				result_sc = "<script>alert(NamoSELang.pe_lR+'" + fileName + "');history.back();</script> ";
			}
	}
	else{
		result_sc = "<script>alert(NamoSELang.pe_qa);location.href='account_setting.jsp';</script>";
	}
