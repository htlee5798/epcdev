 ...
 */
	...
	public String detectXSSEx2(String s) {
		if (s != null) {
			...
			// 확장자에 XSS 문자열이 있을 경우 추가 검열
			if (s.indexOf(".") != -1) {
				String fileName = s.substring(0, s.lastIndexOf("."));
				String fileExt = s.substring(s.lastIndexOf(".") + 1);
				// 파일 확장자는 1글자 이상이여야하며, 파일 확장자의 XSS 문자열 치환을 진행
				if (fileExt.length() > 0) {
					...
					fileExt = fileExt.replaceAll("\"", "");
					fileExt = fileExt.replaceAll("\'","");
					fileExt = fileExt.replaceAll("%00","");
					s = fileName + "." + fileExt;
					// System.out.println("fileName :" + fileName + " " + "fileExt :" + fileExt + " " + fileExt.length());
				}
			} else {
				// 확장자가 없는 파일의 경우에는 XSS 문자열을 전체적으로 치환하는 과정을 진행
				s = s.replaceAll("<", "&lt;");
				s = s.replaceAll(">", "&gt;");
				s = s.replaceAll("&", "&amp;");
				s = s.replaceAll("\"", "&quot;");
	...
	public String detectXSSEx3(String s) {
