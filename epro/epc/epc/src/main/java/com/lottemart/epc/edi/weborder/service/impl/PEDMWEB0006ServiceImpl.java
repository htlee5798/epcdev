...
import com.lottemart.epc.edi.weborder.dao.PEDMWEB0006Dao;
...
@Service("pEDMWEB0006Service")
public class PEDMWEB0006ServiceImpl implements PEDMWEB0006Service {
	...
	@Autowired
	private PEDMWEB0006Dao pedmweb0006dao;
    ...
    */
	public HashMap<String, String> insertReturnProdData(EdiRtnProdVO vo, HttpServletRequest request) throws Exception{
		...
		int rtnProCnt = pedmweb0006dao.selectRtnProdCnt(vo);
		...
		/* ==[ 반품저장 전 점포별상품 중복  검사 ] ============================ --*/
		int rtnDupCnt = pedmweb0006dao.selectRtnProdDuplicateCnt(vo);	//점포별 상품 등록 중복확인 Count
		...
		pedmweb0006dao.saveRtnProdMst(vo);
		/*-- ----------------------------------------------------*/

		/*-- 반품상품 점별 상품목록 생성 (INSERT TED_PO_RRL_PROD) ---*/
		int prodCnt = pedmweb0006dao.saveRtnProdList(vo);
		if(prodCnt <= 0){
		...
		pedmweb0006dao.saveRtnrodSum(vo);
		/*-- ----------------------------------------------------*/
	...
	public Map<String,Object> selectDayRtnProdList(SearchWebOrder vo, HttpServletRequest request)  throws Exception {
		...
		sumData 	= pedmweb0006dao.selectDayRtnProdSum(vo); 		// 합계
		listData	= pedmweb0006dao.selectDayRtnProdList(vo);		// 목록
	...
	public HashMap<String, String> deleteReturnProdData(EdiRtnProdProcessVO vo, HttpServletRequest request)  throws Exception{
		...
		pedmweb0006dao.deleteRtnrodListBatch(rtnDeleteList);
		/*-----------------------------------------------------------------------*/
		...
		pedmweb0006dao.deleteRtnrodList(param);
		rtnData.put("state","0");
		//-----------------------------------------------------------------------*/

		/*-- 반품상품 점별 마스터 합계   (UPDATE TED_PO_RRL_MST) -------------------*/
		pedmweb0006dao.saveRtnrodSum(param);
		//-----------------------------------------------------------------------*/
	...
	public List<EdiRtnProdVO> selectDayRtnProdSumList(SearchWebOrder vo, HttpServletRequest request) throws Exception {
		return pedmweb0006dao.selectDayRtnProdSumList(vo);
	}
	...
	public HashMap<String, String> insertSendReturnProdData(SearchWebOrder vo, HttpServletRequest request)  throws Exception{
		...
		List<EdiRtnProdVO> prodList = pedmweb0006dao.selectDayRtnProdListSend(vo); //
		...
		for ( EdiRtnProdVO  prams : prodList ) {
			if(Integer.parseInt(prams.getRrlQty()) > Integer.parseInt(prams.getRrlStkQty())){
				...
				prams = pedmweb0006dao.saveCallSendReturnProd(prams);	// SP 호출, 결과 리턴
		...
		/*반품결과 UPDATE ===================================================================*/
		pedmweb0006dao.saveTedPoRrlProdBatch(rtnList);
		/*==================================================================================*/


		/*반품결과 Master UPDATE ===================================================================*/
		pedmweb0006dao.saveTedPoRrlMst(vo);
		/*==================================================================================*/
	...
	public Map<String,Object> selectDayRtnList(SearchWebOrder vo, HttpServletRequest request)  throws Exception {
		...
		result.put("state","-1");
			...
			totalCount = pedmweb0006dao.selectDayRtnListTotCnt(vo);
			...
			if (totalCount > 0){
				listData	= pedmweb0006dao.selectDayRtnList(vo);		// 목록
			}

			sumData 	= pedmweb0006dao.selectDayRtnSum(vo); 		// 합계
	...
	public Map<String,Object> selectRtnPackInfo(SearchWebOrder vo) {
		...
		totalCount = pedmweb0006dao.selectRtnPackListTotCnt(vo);
		Paging paging = PagingFactory.makePagingObject(totalCount, pageRowCount, pagePerCount, page);
		int startRowNum = paging.getStartRowNum();
		int endRowNum = paging.getEndRowNum();
		vo.setStartRowNo(startRowNum);
		vo.setEndRowNo(endRowNum);
		if (totalCount > 0){
			list = pedmweb0006dao.selectRtnPackList(vo);
		}
		// 전점 합계 조회
		rtnCntVo = pedmweb0006dao.selectRtnPackCntSum(vo);
	...
	public String insertRtnPackInfo(EdiRtnPackListVO vo, HttpServletRequest request, String ordVenCd) throws SQLException, Exception{
		...
		// 파일 구분 코드 생성
		packDivnCd = pedmweb0006dao.selectNewRtnPackDivnCd();

		// 파일 그룹 코드 생성
		fileGrpCd = pedmweb0006dao.selectNewRtnFileGrpCd(ordVenCd);
		...
		// 업체발주요청상품정보 저장
		pedmweb0006dao.insertRtnPackInfo(vo);

		// 업체발주요청상품정보 수정
		pakVO.setPackDivnCd(packDivnCd);
		pedmweb0006dao.updateRtnPackInfo(pakVO);

		// 오류 코드 업데이트 ( TED_ORD_SUPP_INFO와 Excel 정보가 일치 하지 않은 경우)
		pedmweb0006dao.updateRtnPackState(packDivnCd);
	...
	public String deleteExcelRtnInfo(EdiRtnPackVO vo) throws Exception{
		...
		// 업체발주요청상품정보 삭제
		pedmweb0006dao.deleteExcelRtnInfo(vo);
	...
	public String insertExcelRtnInfo(SearchWebOrder scVo, HttpServletRequest request) throws Exception{
		...
		excelErrorCnt = pedmweb0006dao.selectRtnExcelErrorCnt(vo);
		if(excelErrorCnt > 0){
			return "fail-001";
		}

		// 반품 엑셀 데이터중 중복 건이 있는지 확인. 정상 : suc, 오류( COUNT > 0 ) : fail-002
		excelDuplCnt = pedmweb0006dao.selectRtnExcelDuplCnt(vo);
		if(excelDuplCnt > 0){
			return "fail-002";
		}

		// 기존에 저장된 데이터와 비교하여 중복 건이 있는지 확인. 정상 : suc, 오류( COUNT > 0 ) : fail-003
		rrlDuplCnt = pedmweb0006dao.selectRtnDuplCnt(vo);
		if(rrlDuplCnt > 0){
			return "fail-003";
		}

		// 오류건과 중복건이 없을 경우 점포 코드 조회 후( ven_cd, ord_dy ) TED_PO_ORD_MST 점포별로 생성
		packList = pedmweb0006dao.selectRtnPackStrInfo(vo);
		rrlList = pedmweb0006dao.selectRtnPackInfoList(vo);
		...
		pedmweb0006dao.insertRtnExcelProdInfo(rrlList);
		//-----------------------------------------
		...
		// 업체반품정보마스터 생성 -------------------
		pedmweb0006dao.insertRtnExcelMstInfo(mst);
		//-----------------------------------------

		// 업체반품 요청 마스터 의 발주 총 수량, 발주총금액, 발주전체 총 수량 점포별 일괄 업데이트
		pedmweb0006dao.updateExcelRtnMstSum(mst);
		//-----------------------------------------

		// 저장이 성공하면 TED_PO_ORD_PACK 정보 삭제.
		pedmweb0006dao.deleteRtnPackInfo(vo);
