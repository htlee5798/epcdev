...
import com.lottemart.epc.delivery.model.PSCMDLV0010VO;
import com.lottemart.epc.delivery.service.PSCMDLV0010Service;
import com.lottemart.epc.util.Utils;

@Controller
public class PSCMDLV0010Controller {
	private static final Logger logger = LoggerFactory.getLogger(PSCMDLV0010Controller.class);
    ...
    @Autowired
    private PSCMDLV0010Service pscmdlv0010Service;
	...
	@RequestMapping("delivery/pscmdlv0010/init.do")
	public String noticeForm(@ModelAttribute("searchVO") PSCMDLV0010VO searchVO, HttpServletRequest request, ModelMap model) throws Exception {
		...
		return "delivery/PSCMDLV0010";
	}

	@RequestMapping("delivery/pscmdlv0010/search.do")
	public @ResponseBody Map selectVendorDeliDyStatus(@ModelAttribute("searchVO") PSCMDLV0010VO searchVO, HttpServletRequest request, ModelMap model) throws Exception {
		...
		try {
			...
			String soutYn =param.getString("soutYn");
			String dispYn =param.getString("dispYn");

			param.put("prodCd", prodCd);
			param.put("prodNm", prodNm);
			param.put("penaltyCnt", penaltyCnt);
			param.put("soutYn", soutYn);
			param.put("dispYn", dispYn);
			...
			List<DataMap> list = pscmdlv0010Service.selectVendorPenalty(param);
			rtnMap = JsonUtils.convertList2Json((List) list, null, param.getString("currentPage"));
	...
	@RequestMapping("delivery/pscmdlv0010/excel.do")
	public void excelVendorDeliDyStatus(@ModelAttribute("searchVO") PSCMDLV0010VO searchVO, HttpServletRequest request, HttpServletResponse response, ModelMap model) throws Exception {
		...
		paramMap.put("prodCd", searchVO.getProdCd());
		paramMap.put("prodNm", searchVO.getProdNm());
		paramMap.put("penaltyCnt", searchVO.getPenaltyCnt());
		paramMap.put("soutYn", searchVO.getSoutYn());
		paramMap.put("dispYn", searchVO.getDispYn());
		...
		List<DataMap> list = pscmdlv0010Service.selectVendorPenalty(paramMap);
	...
	private void setHeader(HttpServletRequest request, HttpServletResponse response) {
		...
		String fileName = "Penalty_Product_"+time+".xls"; //실제 파일명
	...
	private WritableSheet createWritableSheet(WritableWorkbook wWorkbook) {
		return wWorkbook.createSheet("업체배송발송일현황", 0);
	}
