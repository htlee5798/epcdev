...
/**
 * 결산정보 - > 거래실적조회  - > 점포별 거래실적 Controller
 * 
 * @author SUN GIL CHOI
 * @since 2015.11.04
 * @version 1.0
 ...
 */
@Controller
public class NEDMPAY0120Controller {

	@Autowired
	private NEDMPAY0120Service nedmpay0120Service;
	 ...
	 * @return
	 */
		@RequestMapping(value = "/edi/payment/NEDMPAY0120.do", method = {RequestMethod.GET,RequestMethod.POST})
		public String credLedStoreDetail(Locale locale,  ModelMap model,HttpServletRequest request) {
			...
			return "/edi/payment/NEDMPAY0120";
		}
		 ...
		 * @return
		 */
		@RequestMapping(value = "/edi/payment/NEDMPAY0120Select.json", method = RequestMethod.POST)
		public @ResponseBody Map<String, Object> selectCredLedStoreDetail(@RequestBody NEDMPAY0120VO map,HttpServletRequest request) throws Exception {
			
			// session 접근 N개의 협력사코드(VEN_CD) -------------------------------
			EpcLoginVO epcLoginVO = (EpcLoginVO) request.getSession().getAttribute(config.getString("lottemart.epc.session.key"));
			map.setVenCds(epcLoginVO.getVendorId());
			Map<String, Object> resultMap = new HashMap<String, Object>();
			List<NEDMPAY0120VO> paymentList = nedmpay0120Service.selectCredLedStoreDetail(map);
		 ...
		 * @return
		 */
		@RequestMapping(value = "/edi/payment/NEDMPAY0120Text.do", method = RequestMethod.POST)
		public void createTextCredLedStoreDetail(NEDMPAY0120VO map,HttpServletRequest request, HttpServletResponse response) throws Exception {
			...
			nedmpay0120Service.createTextCredLedStoreDetail(map,request,response);
