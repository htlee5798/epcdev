 ...
 * </pre>
 */
public class EdiImageUploadInterceptor001 implements HandlerInterceptor {
...
//	}
	...
	public void setImageUploadProperty(FileUploadProperty imageUploadProperty) {
		...
		this.imageUploadProperty.setMaximumSize(600000L);// 크기가 500k 미만인 이미지만 업로드 가능
		this.imageUploadProperty.setMinimumSize(300000L);// 크기가 300k 이상인 이미지만 업로드 가능
	...
	public boolean preHandle(HttpServletRequest request,
		...
		while (fileIter.hasNext()) {
			...
			if (!mFile.isEmpty()) {
				...
				if (this.imageUploadProperty.getMinimumSize() > mFile.getSize()) {
					error = "minsize";
					resultFlag = false;
					modelAndView.addObject("message", error);
					break;
