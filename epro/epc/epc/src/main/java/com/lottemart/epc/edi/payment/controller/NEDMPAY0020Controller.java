...
/**
 * 결산정보 - > 기간별 결산정보  - > 대금결제 정보 Controller
 * 
 * @author SUN GIL CHOI
 * @since 2015.11.04
 * @version 1.0
 ...
 */
@Controller
public class NEDMPAY0020Controller {

	@Autowired
	private NEDMPAY0020Service NEDMPAY0020Service;
	 ...
	 * @return
	 */
		@RequestMapping(value = "/edi/payment/NEDMPAY0020.do", method = {RequestMethod.GET,RequestMethod.POST})
		public String paymentDay(Locale locale,  ModelMap model,HttpServletRequest request) {
			...
			return "/edi/payment/NEDMPAY0020";
		}
		 ...
		 * @return
		 */
		@RequestMapping(value = "/edi/payment/NEDMPAY0020Select.json", method = RequestMethod.POST)
		public @ResponseBody Map<String, Object> selectPaymentDay(@RequestBody NEDMPAY0020VO map,HttpServletRequest request) throws Exception {
					
			// session 접근 N개의 협력사코드(VEN_CD) -------------------------------
			EpcLoginVO epcLoginVO = (EpcLoginVO) request.getSession().getAttribute(config.getString("lottemart.epc.session.key"));
			map.setVenCds(epcLoginVO.getVendorId());
			Map<String, Object> resultMap = new HashMap<String, Object>();
			List<NEDMPAY0020VO> paymentList = NEDMPAY0020Service.selectPaymentDayInfo(map);
		 ...
		 * @return
		 */
		@RequestMapping(value = "/edi/payment/NEDMPAY0020Text.do", method = RequestMethod.POST)
		public void createTextPaymentDay(NEDMPAY0020VO map,HttpServletRequest request, HttpServletResponse response) throws Exception {
			...
			NEDMPAY0020Service.createTextPaymentDay(map,request,response);
