 ...
 */
@Controller
public class PSCMBRD0008Controller 
{
	private static final Logger logger = LoggerFactory.getLogger(PSCMBRD0008Controller.class);
	@Autowired
	private ConfigurationService config;
	@Autowired
	private PSCMBRD0008Service pscmbrd0008Service;

	/**
	 * Desc : 콜센터 목록을 조회하는 메소드
	 ...
	 * @exception Exception
	 */
	@RequestMapping(value = "/board/selectCallCenterList.do")
	public String selectCallCenterList(@ModelAttribute("searchVO") PSCMBRD0008SearchVO searchVO, ModelMap model, HttpServletRequest request) throws Exception
	{
		...
		if (StringUtil.isNVL(searchVO.getStartDate()) || StringUtil.isNVL(searchVO.getEndDate()))
		{
			...
			paramMap.put("startDate", getOnlyNumber(searchVO.getStartDate()));
			paramMap.put("endDate",   getOnlyNumber(searchVO.getEndDate())  );
		}
		else
		{
		...
		// 데이터 조회
		List<DataMap> list = pscmbrd0008Service.selectCallCenterList(paramMap);
