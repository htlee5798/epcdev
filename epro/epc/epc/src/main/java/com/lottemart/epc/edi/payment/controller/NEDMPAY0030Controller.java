...
/**
 * 결산정보 - > 기간별 결산정보  - > 점포별 대금결제 Controller
 * 
 * @author SUN GIL CHOI
 * @since 2015.11.04
 * @version 1.0
 ...
 */
@Controller
public class NEDMPAY0030Controller {

	@Autowired
	private NEDMPAY0030Service NEDMPAY0030Service;
	...
	private ConfigurationService config;
		 ...
		 * @return
		 */
		@RequestMapping(value = "/edi/payment/NEDMPAY0030.do", method = {RequestMethod.GET,RequestMethod.POST})
		public String paymentStore(Locale locale,  ModelMap model,HttpServletRequest request) {
			...
			return "/edi/payment/NEDMPAY0030";
		}
		 ...
		 * @return
		 */
		@RequestMapping(value = "/edi/payment/NEDMPAY0030Select.json", method = RequestMethod.POST)
		public @ResponseBody Map<String, Object> selectPaymentStore(@RequestBody NEDMPAY0030VO map,HttpServletRequest request) throws Exception {
			
			// session 접근 N개의 협력사코드(VEN_CD) -------------------------------
			EpcLoginVO epcLoginVO = (EpcLoginVO) request.getSession().getAttribute(config.getString("lottemart.epc.session.key"));
			map.setVenCds(epcLoginVO.getVendorId());
			Map<String, Object> resultMap = new HashMap<String, Object>();
			List<NEDMPAY0030VO> paymentList = NEDMPAY0030Service.selectPaymentStoreInfo(map);
		 ...
		 * @return
		 */
		@RequestMapping(value = "/edi/payment/NEDMPAY0030Text.do", method = RequestMethod.POST)
		public void createTextPaymentStore(NEDMPAY0030VO map,HttpServletRequest request, HttpServletResponse response) throws Exception {
			...
			NEDMPAY0030Service.createTextPaymentStore(map,request,response);
