...
/**
 * 결산정보 - > 기간별 결산정보  - > 물류비정보 Controller
 * 
 * @author SUN GIL CHOI
 * @since 2015.11.04
 * @version 1.0
 ...
 */
@Controller
public class NEDMPAY0050Controller {

	@Autowired
	private NEDMPAY0050Service NEDMPAY0050Service;
	...
	private ConfigurationService config;
		 ...
		 * @return
		 */
		@RequestMapping(value = "/edi/payment/NEDMPAY0050.do", method = RequestMethod.GET)
		public String logiAmt(Locale locale,  ModelMap model,HttpServletRequest request) {
			...
			return "/edi/payment/NEDMPAY0050";
		}
		 ...
		 * @return
		 */
		@RequestMapping(value = "/edi/payment/NEDMPAY0050Select.json", method = RequestMethod.POST)
		public @ResponseBody Map<String, Object> selectLogiAmt(@RequestBody NEDMPAY0050VO map,HttpServletRequest request) throws Exception {
			
			// session 접근 N개의 협력사코드(VEN_CD) -------------------------------
			EpcLoginVO epcLoginVO = (EpcLoginVO) request.getSession().getAttribute(config.getString("lottemart.epc.session.key"));
			map.setVenCds(epcLoginVO.getVendorId());
			Map<String, Object> resultMap = new HashMap<String, Object>();
			List<NEDMPAY0050VO> paymentList = NEDMPAY0050Service.selectLogiAmtInfo(map);
		 ...
		 * @return
		 */
		@RequestMapping(value = "/edi/payment/NEDMPAY0050Text.do", method = RequestMethod.POST)
		public void createTextLogiAmt(NEDMPAY0050VO map,HttpServletRequest request, HttpServletResponse response) throws Exception {
			// session 접근 N개의 협력사코드(VEN_CD) -------------------------------
			EpcLoginVO epcLoginVO = (EpcLoginVO) request.getSession().getAttribute(config.getString("lottemart.epc.session.key"));
			map.setVenCds(epcLoginVO.getVendorId());
			
			NEDMPAY0050Service.createTextLogiAmt(map,request,response);
