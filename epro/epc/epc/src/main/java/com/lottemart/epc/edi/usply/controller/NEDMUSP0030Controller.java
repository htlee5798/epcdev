...
import com.lottemart.epc.edi.usply.service.NEDMUSP0030Service;

/**
 * 미납정보 - > 기간정보  - > 상품별 Controller
 *
 * @author SUN GIL CHOI
 * @since 2015.11.04
 * @version 1.0
 ...
 */
@Controller
public class NEDMUSP0030Controller {
	//private final String version = "new"; //new old
	@Autowired
	private NEDMUSP0030Service nedmusp0030Service;
	 ...
	 * @return
	 */
	@RequestMapping(value = "/edi/usply/NEDMUSP0030.do", method = {RequestMethod.GET,RequestMethod.POST})
	public String product(Locale locale,  ModelMap model,HttpServletRequest request) {
		...
		String returnPage = "/edi/usply/NEDMUSP0030";
	 ...
	 * @return
	 */
	@RequestMapping(value = "/edi/usply/NEDMUSP0030Select.json", method = RequestMethod.POST)
	public @ResponseBody Map<String, Object> selectProduct(@RequestBody NEDMUSP0030VO map, HttpServletRequest request) throws Exception {
		// session 접근 N개의 협력사코드(VEN_CD) -------------------------------
		EpcLoginVO epcLoginVO = (EpcLoginVO)request.getSession().getAttribute(config.getString("lottemart.epc.session.key"));
		...
		List<NEDMUSP0030VO> 	usplyList 	=  nedmusp0030Service.selectProductInfo(map);
	 ...
	 * @throws Exception
	 */
	@RequestMapping(value = "/edi/usply/NEDMUSP0030Text.do", method=RequestMethod.POST)
    public void createTextOrdProdList(NEDMUSP0030VO vo, HttpServletRequest request, HttpServletResponse response) throws Exception {
		...
		nedmusp0030Service.createTextOrdProdList(vo, request, response);
	}
}
