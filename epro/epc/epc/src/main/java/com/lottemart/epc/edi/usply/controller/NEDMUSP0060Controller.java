 ...
 */
@Controller
public class NEDMUSP0060Controller {
	@Autowired
	private NEDMUSP0060Service nedmusp0060Service;
	
	@Resource(name = "configurationService")
	private ConfigurationService config;
	
	/**
	 * 미납정보 - > 기간정보  - > 패널티 확정 첫페이지
	 * @param locale
	 * @param model
	 * @param request
	 * @return
	 */
	@RequestMapping(value = "/edi/usply/NEDMUSP0060.do", method = RequestMethod.GET)
	public String penalty(Locale locale,  ModelMap model,HttpServletRequest request) {
		String returnPage = "/edi/usply/NEDMUSP0060";
	 ...
	 * @return
	 */
	@RequestMapping(value = "/edi/usply/NEDMUSP0060Select.json", method = RequestMethod.POST)
	public @ResponseBody Map<String, Object> selectPenalty(@RequestBody NEDMUSP0060VO map, HttpServletRequest request) throws Exception {
		// session 접근 N개의 협력사코드(VEN_CD) -------------------------------
		EpcLoginVO epcLoginVO = (EpcLoginVO)request.getSession().getAttribute(config.getString("lottemart.epc.session.key"));
		...
		List<NEDMUSP0060VO> 	usplyList 	=  nedmusp0060Service.selectPenaltyInfo(map);
	 ...
	 * @throws Exception
	 */
	@RequestMapping(value = "/edi/usply/NEDMUSP0060Text.do", method=RequestMethod.POST)
    public void createTextOrdProdList(NEDMUSP0060VO vo, HttpServletRequest request, HttpServletResponse response) throws Exception {
		...
		nedmusp0060Service.createTextOrdProdList(vo, request, response);
	}
