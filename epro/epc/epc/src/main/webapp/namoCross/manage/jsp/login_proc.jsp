...
<%
	...
	String passwd2 = encrypt("SHA1", pwval);
	...
	if(id.equalsIgnoreCase(u_id) || id.equalsIgnoreCase(encrypt("SHA-256", u_id))){
		if(passwd.equalsIgnoreCase(u_pass) || passwd2.equalsIgnoreCase(u_pass)){
			...
			result_sc = "<script>alert(NamoSELang.pe_sv);history.back();</script>";
		}
	}
	else{
		result_sc = "<script>alert(NamoSELang.pe_Ds);history.back();</script>";
	}
