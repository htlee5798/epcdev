...
<%
	...
	String encrypt2 = encrypt("SHA1", detectXSSEx(request.getParameter("passwd")));
	...
	if((encrypt.equals(u_pass) || encrypt2.equals(u_pass)) && login_id.equals(enc_id))
	{
			...
			{
				session.invalidate(); 
				result_sc = "<script>alert(NamoSELang.pe_pZ);window.document.location.href='../index.html';</script>";
			}
			else
			{
				result_sc = "<script>alert(NamoSELang.AdminPageFileWritableErr+'" + fileName + "');history.back();</script> ";
			}
	}
	else{
		result_sc = "<script>alert(NamoSELang.pe_sv);location.href='account_setting.jsp';</script>";
	}
