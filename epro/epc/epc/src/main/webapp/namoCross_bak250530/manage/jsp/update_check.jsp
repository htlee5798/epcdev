...
<%
	...
	if(update_xml.exists()){
		...
		if(update_check_value){
			result_pop = "<script language=\"javascript\">alert(NamoSELang.pe_Dh)</script>";
			String saveCheck = update_xml(update_xml_url,before_xml_url);
			if(saveCheck.equals("sucess")){
				result_sc = resultStrSc;
			}
			else{
				result_sc = "<script language=\"javascript\">alert(NamoSELang.pe_tZ+NamoSELang.pe_lR);window.document.location.href='manager_setting.jsp';</script>";
			}
		}
		else{
			result_sc = resultStrSc;
