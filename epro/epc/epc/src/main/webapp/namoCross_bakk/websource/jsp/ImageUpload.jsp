...
<%
	Logger logger = LoggerFactory.getLogger("ImageUpload");
...
<%
	...
	String imageDomain = detectXSSEx(request.getParameter("imageDomain"));
	...
	String checkPlugin = detectXSSEx(request.getParameter("checkPlugin"));
	//if (detectXSSEx(request.getParameter("checkPlugin")) != null)
	//	checkPlugin = detectXSSEx(request.getParameter("checkPlugin"));

	String fileType = detectXSSEx(request.getParameter("fileType"));
	...
	if (!"".equals(imageUPath)) {
		if (imageUPath.length() > 7) {
			if ("http://".equals(imageUPath.substring(0, 7))) {
			...
			if (!"/".equals(imageUPath.substring(0, 1))) {
				scriptValue = executeScript(response, "invalid_path", "" , useExternalServer, imageDomain, imageEditorFlag, checkPlugin);
				response.getWriter().println(scriptValue);
				return;
			}
		...
		if (defaultUPath.length() > 7) {
			if ("http://".equals(defaultUPath.substring(0, 7))) {
				imageTemp = defaultUPath.substring(7);
				imageUPath = imageTemp.substring(imageTemp.indexOf("/"));
			} else if ("https://".equals(defaultUPath.substring(0, 8))) {
				imageTemp = defaultUPath.substring(8);
				imageUPath = imageTemp.substring(imageTemp.indexOf("/"));
			} else if ("/".equals(defaultUPath.substring(0, 1))) {
			...
			if ("/".equals(defaultUPath.substring(0, 1))) {
				imageUPath = defaultUPath;
			} else {
				scriptValue = executeScript(response, "invalid_path", "", useExternalServer, imageDomain, imageEditorFlag, checkPlugin);
				response.getWriter().println(scriptValue);
	...
	if ("".equals(imagePhysicalPath)) {
		String DompaserValue = Dompaser(imageUPath);
		if ("".equals(DompaserValue)) {
	...
	try {
		...
		if ("false".equals(uploadFileSubDir) && !"".equals(imageUPath)) {
		...
		if (isMultipart) {
			...
			*/
			...
			while(iter.hasNext()) {
				...
				if (fileItem.isFormField()) {
					...
					if ("imageMaxCount".equals(fieldName)) { imageMaxCount = toString(fileItem.getString("utf-8")); }
					if ("imageTitle".equals(fieldName)) { imageTitle = toString(fileItem.getString("utf-8")); }
					if ("imageAlt".equals(fieldName)) { imageAlt = toString(fileItem.getString("utf-8")); }
					if ("imageWidth".equals(fieldName)) { imageWidth = toString(fileItem.getString("utf-8")); }
					if ("imageWidthUnit".equals(fieldName)) { imageWidthUnit = toString(fileItem.getString("utf-8")); }
					if ("imageHeight".equals(fieldName)) { imageHeight = toString(fileItem.getString("utf-8")); }
					if ("imageHeightUnit".equals(fieldName)) { imageHeightUnit = toString(fileItem.getString("utf-8")); }

					/*
					* 2013.03.28 [3.0] mwHong
					* imageMarginSet
					*/
					if ("imageMaginLeft".equals(fieldName)) { imageMarginLeft = toString(fileItem.getString("utf-8")); }
					if ("imageMaginLeftUnit".equals(fieldName)) { imageMarginLeftUnit = toString(fileItem.getString("utf-8")); }
					if ("imageMaginRight".equals(fieldName)) { imageMarginRight = toString(fileItem.getString("utf-8")); }
					if ("imageMaginRightUnit".equals(fieldName)) { imageMarginRightUnit = toString(fileItem.getString("utf-8")); }
					if ("imageMaginTop".equals(fieldName)) { imageMarginTop = toString(fileItem.getString("utf-8")); }
					if ("imageMaginTopUnit".equals(fieldName)) { imageMarginTopUnit = toString(fileItem.getString("utf-8")); }
					if ("imageMaginBottom".equals(fieldName)) { imageMarginBottom = toString(fileItem.getString("utf-8")); }
					if ("imageMaginBottomUnit".equals(fieldName)) { imageMarginBottomUnit = toString(fileItem.getString("utf-8")); }

					if ("imageAlign".equals(fieldName)) { imageAlign = toString(fileItem.getString("utf-8")); }
					if ("imageId".equals(fieldName)) { imageId = toString(fileItem.getString("utf-8")); }
					if ("imageClass".equals(fieldName)) { imageClass = toString(fileItem.getString("utf-8")); }
					if ("imageBorder".equals(fieldName)) { imageBorder = toString(fileItem.getString("utf-8")); }
					if ("imageKind".equals(fieldName)) { imageKind = toString(fileItem.getString("utf-8")); }
					if ("imageTempFName".equals(fieldName)) { imageTempFName = toString(fileItem.getString("utf-8")); }
					if ("imageUNameType".equals(fieldName)) { imageUNameType = toString(fileItem.getString("utf-8")); }
					if ("imageUNameEncode".equals(fieldName)) { imageUNameEncode = toString(fileItem.getString("utf-8")); }
					if ("imageViewerPlay".equals(fieldName)) { imageViewerPlay = toString(fileItem.getString("utf-8")); }
					if ("imageOrgPath".equals(fieldName)) { imageOrgPath = toString(fileItem.getString("utf-8")); }
					if ("editorFrame".equals(fieldName)) { editorFrame = toString(fileItem.getString("utf-8")); }
			...
			if ("real".equals(imageUNameType)) {
				fileTempName = filename.substring(0, filename.lastIndexOf("."));
			} else if ("random".equals(imageUNameType)) {
			...
			if ("false".equals(uploadFileSubDir)) {
				if ("".equals(imageUPath)) {
			...
			if ("true".equals(imageViewerPlay)) {
			...
			if (imageOrgPath != null && !"".equals(imageOrgPath)) {
			...
			if (imageWidthUnit == null) { imageWidthUnit = ""; }
			if (imageHeight == null) { imageHeight = ""; }
			if (imageHeightUnit == null) { imageHeightUnit = ""; }

			/*
			* 2013.03.28 [3.0] mwHong
			* imageMarginSet
			*/
			if (imageMarginLeft == null) { imageMarginLeft = ""; }
			if (imageMarginLeftUnit == null) { imageMarginLeftUnit = ""; }
			if (imageMarginRight == null) { imageMarginRight = ""; }
			if (imageMarginRightUnit == null) { imageMarginRightUnit = ""; }
			if (imageMarginTop == null) { imageMarginTop = ""; }
			if (imageMarginTopUnit == null) { imageMarginTopUnit = ""; }
			if (imageMarginBottom == null) { imageMarginBottom = ""; }
			if (imageMarginBottomUnit == null) { imageMarginBottomUnit = ""; }

			if (imageAlign == null) { imageAlign = ""; }
			if (imageId == null) { imageId = ""; }
			if (imageClass == null) { imageClass = ""; }
			if (imageBorder == null) { imageBorder = ""; }
			if (imageOrgPath == null) { imageOrgPath = ""; }
			if (editorFrame == null) { editorFrame = ""; }
			...
			*/
			...
			if (check == 1) {
				...
				if (SaveSubFolder.exists()) { SaveSubFolder.delete(); }

				if ("flashPhoto".equals(imageEditorFlag)) {
					scriptValue = "{";
		...
		logger.error("ImageUpload_catch-01 IOException ioe :: " + sw.toString());
		...
		logger.error("ImageUpload_catch-02 SizeLimitExceededException e :: " + sw.toString());
		...
		StringWriter sw = new StringWriter();
		e.printStackTrace(new PrintWriter(sw));
		logger.error("ImageUpload_catch-03 FileUploadException e :: " + sw.toString());

		scriptValue = executeScript(response, "invalid_size (Upload Exception)", Integer.toString(maxSize), useExternalServer, imageDomain, imageEditorFlag, checkPlugin);
		response.getWriter().println(scriptValue);
		return;
	} catch (RuntimeException e) {
		StringWriter sw = new StringWriter();
		e.printStackTrace(new PrintWriter(sw));
		logger.error("ImageUpload_catch-04 RuntimeException e :: " + sw.toString());
		...
		StringWriter sw = new StringWriter();
		e.printStackTrace(new PrintWriter(sw));
		logger.error("ImageUpload_catch-05 Exception e :: " + sw.toString());

		String messageText = "Exception";
		messageText = "<System Error.>" + messageText;
		scriptValue = executeScript(response, "", messageText, useExternalServer, imageDomain, imageEditorFlag, checkPlugin);
		response.getWriter().println(scriptValue);
		return;
