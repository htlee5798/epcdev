...
<script>
	...
	function doSearch() {
		...
		for(var i=0; i<TAB2.length;i++)
		{
			if(i==0){
				...
				$("#openDataClicked input[name='formEntpCd']").val(TAB2[i].VEN_CD);
			}
			else{
				var tagAppending = document.createElement("input");
				tagAppending.type ="hidden";
				...
				tagAppending.value = TAB2[i].VEN_CD;
				document.openDataClicked.appendChild(tagAppending);
			}
		}
		...
		for(var i=0; i<TAB3.length;i++)
		{
			if(i==0){
				...
				$("#openDataClicked input[name='formStrCd']").val(TAB3[i].STR_CD);
			}
			else{
				var tagAppending = document.createElement("input");
				tagAppending.type ="hidden";
				...
				tagAppending.value = TAB3[i].STR_CD;
				document.openDataClicked.appendChild(tagAppending);
			}
		}
		...
		rfcCallForSearch("INV0640" , dataInfo);
	}
	...
	function clickSlipNo(slipNo,srcmkCd){
		...
		var popInfo = window.open("",'_blankPop','top=0, left=0, width=800, height=350, toolbar=no, status=no');

		popInfo.focus();

		console.log($("#formEntpCd").val());

		$("#openDataClicked").attr("action", "<c:url value='/edi/delivery/NEDMDLY0121.do'/>");
	...
	/* 저장 */
	function doUpdate(){
		var $checkedArr = $('input[name=accept_check]:checked');
	...
	/*	data-str-cd        		="<c:out value="\${STR_CD     			}" />"
		...
		data-accept-dy     		="<c:out value="\${ACCEPT_DY  			}" />"
	...
	function popupSearch(tbName1, tbName2){
		...
		var bodyValue = "";
		bodyValue += "<CAPTION>TOY배달정보 접수확인 현황표<br>";
	...
	function printText() {
		...
		var date = dataInfo["START_DATE"] + "~" + dataInfo["END_DATE"];
		...
		$("#searchForm input[name='proxyNm']").val("INV0640");
		...
		$("#searchForm").attr("action", "<c:url value='/edi/delivery/NEDMDLY0120Text.do' />");
		$("#searchForm").submit();
	}
	...
	function initParamForProd(){
		var paramNames=["formEntpCd","formHangmok","formStrCd"];

		var lenEntpCd = $('input[name=formEntpCd]').length;
		var lenHangmok = $('input[name=formHangmok]').length;
		var lenStrCd = $('input[name=formStrCd]').length;

	    for(var cnt=0; cnt<lenEntpCd; cnt++)$('#formEntpCd').remove();
	    for(var cnt=0; cnt<lenHangmok; cnt++)$('#formHangmok').remove();
	    for(var cnt=0; cnt<lenStrCd; cnt++)$('#formStrCd').remove();
		...
		for (var idx=0; idx<paramNames.length; idx++)
		{
			...
			var tagAppending = document.createElement("input");
			tagAppending.type ="hidden";
			tagAppending.name = paramName;
			tagAppending.id	  = paramName;
			document.openDataClicked.appendChild(tagAppending);
		}
	}
...
data-lst-chg-emp   		="<c:out value="\${LST_CHG_EMP}" />"
		>
			<input type="checkbox" name="accept_check">
		</td>
		{%else%}
		<td rowspan="3" align="center"><spring:message code='text.dly.field.registed' /></td>
		{%/if%}
	</tr>
	<tr class="r1">
...
<body>
	...
	<div>
		<form id="openDataClicked" name="openDataClicked" method="post"  action="/epc/edi/delivery/NEDMDLY0121.do"  target="iframe1">
			...
			<div class="wrap_search">
				...
				<div class="bbs_search">
					...
					<tr>
						...
						<td>
							<input type="Radio" name="strCdType" <c:if test="${empty param.storeVal }"> Checked</c:if>  onclick="javascript:storeClear();" onclick="javascript:storeClear();"/><spring:message code='text.dly.field.allStore' />
					...
					<tr>
						...
						<th><spring:message code='text.dly.field.hangmok' /></th>
						<td>
							<select id="hangmok" name="hangmok">
								...
								<option value="1" <c:if test="${param.hangmok == '1' }"> selected </c:if>><spring:message code='text.dly.field.registed' /></option>
								<option value="0" <c:if test="${param.hangmok == '0' }"> selected </c:if>><spring:message code='text.dly.field.notRegisted' /></option>
							</select>
						</td>
					</tr>
					</table>
			...
			<div class="wrap_con">
				...
				<div class="bbs_list">
					...
					<tr>
						...
						<th rowspan="3"><spring:message code='epc.dly.header.h01207' /></th>
					</tr>
					<tr>
						<th><spring:message code='epc.dly.header.h01208' /></th>
						<th><spring:message code='epc.dly.header.h01209' /></th>
						<th><spring:message code='epc.dly.header.h01210' /></th>
						<th><spring:message code='epc.dly.header.h01211' /></th>
						<th><spring:message code='epc.dly.header.h01212' /></th>
					</tr>
					<tr>
						<th><spring:message code='epc.dly.header.h01213' /></th>
						<th><spring:message code='epc.dly.header.h01214' /></th>
						<th><spring:message code='epc.dly.header.h01215' /></th>
						<th><spring:message code='epc.dly.header.h01216' /></th>
						<th><spring:message code='epc.dly.header.h01217' /></th>
					</tr>
					<tr>
						<th colspan="7"><spring:message code='epc.dly.header.h01218' /></th>
					</tr>
					</table>
		...
		<div id="footbox">
			...
			<div class="location">
				<ul>
					...
					<li class="last"><spring:message code='epc.dly.menu.registConfirm' /></li>
				</ul>
			</div>
		</div>
	</div>
