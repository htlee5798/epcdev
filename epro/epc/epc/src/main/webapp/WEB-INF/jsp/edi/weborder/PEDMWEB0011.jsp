...
$(function() {
	...
	$('#startDate,#endDate,#areaCd,#strCd,#vend_nm,#prodCd, input[name=pageRowCount]').unbind().keydown(function(e) {
		switch(e.which) {
...
/* 초기 설정 */
function _init(){
	_setTbodyNoResult($("#datalist"), 	11, '발주요청일자, 협력업체코드, 지역/점포, 상품 코드를 입력 또는 선택 후 [검색] 하세요!');	// prod tBody 설정
}	
...
//업체코드 검색 팝업 호출 
 ...
 function setVendorInto(vendorId,vendorName){
	 ...
	 $("#vend_cd").val(vendorId);
	 $("#vend_nm").val(vendorName);
 }
...
function caldate(day){
 ...
 function doSearch(){
	...
	$.post(
			"<c:url value='/edi/weborder/PEDMWEB0011Select.do'/>",
			JSON.stringify(str),
			function(data){
				if(data == null || data.state != "0"){
			...
			if (sz > 0) {
				...
				for ( var k = 0; k < sz; k++) {
					...
					if(data[k].mdModCd == '00') {
						eleHtml[++h] = "\t" + '<td style="border-bottom-style:double; border-color:#c0c0c0;"  rowspan=2 align="center">정상</td>' + "\n";
					}else if(data[k].mdModCd == '01'){
						eleHtml[++h] = "\t" + '<td align="center" style="border-bottom-style:double; border-color:#c0c0c0;" rowspan=2"><a href="javascript:;" onClick="_viewMdState(this)">조정</a></td>' + "\n";
					}else if(data[k].mdModCd == '02'){
						eleHtml[++h] = "\t" + '<td style="border-bottom-style:double; border-color:#c0c0c0;" rowspan=2 align="center" style="font-weight: bold; text-decoration:underlin"><a href="javascript:;" onClick="_viewMdState(this)">삭제</a></td>' + "\n";
					}else{
						eleHtml[++h] = "\t" + '<td style="border-bottom-style:double; border-color:#c0c0c0;" rowspan=2 align="center" style="font-weight: bold; text-decoration:underlin"><a href="javascript:;" onClick="_viewMdState(this)">중단</a></td>' + "\n";
					}
		...
		<div id="wrap_menu">
			...
			<div class="wrap_search">
				...
				<div class="bbs_search">
					...
					<tr> 
						<th><span class="star">*</span> 발주요청일자 </th>
						<td>
							...
							<th><span class="star">*</span> 협력업체 코드</th>
									<td>
				...
				<div class="bbs_list">
					...
					<table id="dataTable" cellpadding="1" cellspacing="1" border="0" width=100% bgcolor="#EFEFEF">
                        ...
                        <tr bgcolor="#e4e4e4" align=center> 
                          ...
                          <th rowspan=2>조정</br>구분</th>
	                      <th>점포</th>
	                      <th>상품코드</th>
                          <th>판매코드</th>
                          <th>규격</th>
                          <th rowspan=2>단위</th>
                          <th rowspan=2>발주<br>수량</th>
                          <th rowspan=2>EA</th>
                          <th rowspan=2>금액</th>
	                      <th rowspan=2></th>
                        </tr>
