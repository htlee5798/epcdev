...
<script>
	...
	function doSearch() {
		...
		if(dateValid(form)){
			loadingMaskFixPos();
			form.action  = "<c:url value='/edi/order/PEDMORD0001Select.do'/>";
			form.submit();	
		}	
	//	hideLoadingMask();
	}
			...
			<div class="wrap_con">
				...
				<div class="bbs_list">
					...
					<c:if test="${not empty orderList }">
						<c:set var="total_ipsu"  value="0" />
						<c:set var="total_qty"  value="0" />
						<c:set var="total_prc"  value="0" />
						<c:forEach items="${orderList}" var="list" varStatus="index" >
							<tr class="r1">
								<td align="center"><a href="javascript:forwarding('${list.ORD_DY }');">${list.ORD_DY }</a></td>
								<td align="center" style="mso-number-format:'\@'"><a href="javascript:forwarding2('${list.PROD_CD }','${list.ORD_DY }');">${list.PROD_CD }</a></td>
							...
							<c:set var="total_ipsu" value="${total_ipsu + list.ORD_IPSU }" />
							<c:set var="total_qty" value="${total_qty + list.ORD_QTY }" />
							<c:set var="total_prc" value="${total_prc + list.BUY_PRC }" />
						</c:forEach>
					</c:if>
					...
					<table class="bbs_list" cellpadding="0" cellspacing="0" border="0" id="testTable3">
						...
						<c:if test="${not empty orderList }">
						<tr class="r1">
							<th colspan="3" class="fst" align=center><b>합계</b></th>
							<th align=right><b><fmt:formatNumber value="${total_ipsu }" type="number" currencySymbol="" /></b></th>
							<th></th>
						...
						<c:if test="${empty orderList }"><tr><td colspan=7>&nbsp;</td><th width=17>&nbsp;</th></tr></c:if>
					</table>	
				</div>
			</div>
		</div>
