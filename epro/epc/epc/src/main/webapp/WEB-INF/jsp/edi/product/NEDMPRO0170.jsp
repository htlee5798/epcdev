...
<script>
	...
	function doSave() {
		...
		var disconMstate = $("#mstate"+idx).val();
...
// 		info["mstate"]		  = "03";
		info["mstate"]		  = disconMstate;						// 변경상태 (02:임시발주중단, 03:단종)
		...
		if (data.length > 0) {
	        for(var i=0; i<data.length; i++){
	        	...
	        	if(isDisconApply(data[i])){		// DB에 넣기 위해 형식 변환
	        		...
	        		data[i].regDtFmt = $.trim(data[i].regDt).replace(/(\d{4})(\d{2})(\d{2})/g,"$1-$2-$3");
	        	}
	        	else{							// 값이 없을 때, 메시지 
	...
	function _setTbodyMasterValue_old(json) {
		setTbodyInit("dataListbody");		    // dataList 초기화
    ...
    function appendDisconDateRow(){
    	var html = '<td></td>'+
				   '<td></td>'+
				   '<th><span class="star"></span>적용일자 검색</th>'+
				   '<td>' +
...
<tr class="r1 row\${idx}"  bgcolor=ffffff>
	<td><input type="checkbox" name="disconApplyBox" value="<c:out value='\${idx}'/>" onclick="rowFocus(this)"></td>
	...
	<td align="center"><c:out value='\${werksCnt}'/></td>
	<td align="center">
		<html:codeTag objId="mstate\${idx}" objName="mstate" parentCode="DSCON"  comType="SELECT" dataType="NTCPCD" orderSeqYn="Y" defName="선택" formName="form"/>
	</td>
	<td align="center">
		<select id="disconReason\${idx}" name="disconReason">
			<option value="">선택</option>
			<option value="취급중단(단종)">취급중단 (단종)</option>
		</select>
	</td>
	<td align="center">
		<input type="text" id="disconApplyDt\${idx}" name="disconApplyDt" 	value="<c:out value='\${disconApplyDt }'/>" style="width:80px" readonly />
		<img src="/images/epc/layout/btn_cal.gif" class="middle" onClick="openCal('searchForm.disconApplyDt\${idx}');" style="cursor: hand;" />
	</td>
	<td align="center"><c:out value='\${regDtFmt}'/></td>
	{%if viewType != 1 %}
	<td align="center"><c:out value='\${zstat}'/></td>
	{%/if%}
</tr>
						...
						<div style="width:100%; height:446px;overflow-x:hidden; overflow-y:scroll; table-layout:fixed;white-space:nowrap;">
							<table  cellpadding="1" cellspacing="1" border="0" width="820px" bgcolor=efefef>
								...
								<tr bgcolor="#e4e4e4">
									...
									<th rowspan=3>변경상태</th>
									<th rowspan=3>변경사유</th>
									<th rowspan=3>적용일</th>
									<th rowspan=3>입력일자</th>
			...
			<div id="footbox">
				...
				<div class="location">
					<ul>
						...
						<li>상품</li>
						<li>상품현황관리</li>
						<li class="last">상품단종/임시발주중단 신청</li>
					</ul>
				</div>
			</div>
		</div>
