...
<script>
	...
	function doSearch() {
		...
		if(dateValid()){
			$.ajax({
				...
				url : '<c:url value="/edi/usply/NEDMUSP0020Select.json"/>',
	...
	function popupSearch(tbName1, tbName2){		
		...
		var bodyValue = "";
		bodyValue += "<CAPTION>미납정보 점포별 현황표<br>";
	...
	function forwarding(val,val2) {
		...
		$("#searchForm").attr("action", "<c:url value='/edi/usply/NEDMUSP0030.do'/>");
		$("#searchForm").submit();	
		form.submit();	
	}
	...
	function printText() {
		...
		$("#searchForm").attr("action", "<c:url value='/edi/usply/NEDMUSP0020Text.do'/>");
		$("#searchForm").submit();	
	}
</script>
<!-- Data List Templete -------------------------------------------------------------------------------------------------------------->
...
<tr class="r1">
	...
	<td align=center><a href="javascript:forwarding('<c:out value="\${strCd}"/>','<c:out value="\${splyDy}"/>');"><c:out value="\${strNm}"/></a></td>
		...
		<div id="footbox">
			...
			<div class="location">
				<ul>
					...
					<li class="last"><spring:message code='epc.usp.perStore'/></li>
				</ul>
			</div>
		</div>
	</div>
