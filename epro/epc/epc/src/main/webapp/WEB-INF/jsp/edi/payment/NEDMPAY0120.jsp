...
<script>
	...
	function doSearch() {
		...
		if(dateValid()){
			$.ajax({
				...
				url : '<c:url value="/edi/payment/NEDMPAY0120Select.json"/>',
		...
		function _setTbodyMasterValue(json) { 
			...
			if (data.length > 0) {
				...
				var trans_buy_amt_sum = 0;
				var buy_amt_sum =0;
				var pay_buy_amt_sum =0;
				var rem_buy_amt_sum =0;
				var trans_vat_sum =0;
				var vat_sum =0;
				var pay_vat_sum =0;
				var rem_vat_sum =0;
				...
				for (var i = 0; i < data.length; i++) {
					transBuyAmt = data[i]["transBuyAmt"];
					buyAmt = data[i]["buyAmt"];
					payBuyAmt = data[i]["payBuyAmt"];
					remBuyAmt = data[i]["remBuyAmt"];
					transVat = data[i]["transVat"];
					vat = data[i]["vat"];
					payVat = data[i]["payVat"];
					remVat = data[i]["remVat"];
					remSum = data[i]["remSum"];
					
					// 합계
					trans_buy_amt_sum += parseInt(transBuyAmt);
					buy_amt_sum += parseInt(buyAmt);
					pay_buy_amt_sum += parseInt(payBuyAmt);
					rem_buy_amt_sum += parseInt(remBuyAmt);
					trans_vat_sum += parseInt(transVat);
					vat_sum += parseInt(vat);
					pay_vat_sum += parseInt(payVat);
					rem_vat_sum += parseInt(remVat);
					rem_sum_sum += parseInt(remSum);
					
					// Comma 설정
					data[i]["transBuyAmt"] 	= setComma(transBuyAmt);
					data[i]["buyAmt"] 	= setComma(buyAmt);
					data[i]["payBuyAmt"] 	= setComma(payBuyAmt);
					data[i]["remBuyAmt"] 	= setComma(remBuyAmt);
					data[i]["transVat"] 	= setComma(transVat);
					data[i]["vat"] 	= setComma(vat);
					data[i]["payVat"] 	= setComma(payVat);
					data[i]["remVat"] 	= setComma(remVat);
					data[i]["remSum"] 	= setComma(remSum);
				}
				$("#dataListTemplate").tmpl(data).appendTo("#dataListbody"); // setComma(n)
				
				$('#trans_buy_amt_sum').text(setComma(trans_buy_amt_sum));
				$('#buy_amt_sum').text(setComma(buy_amt_sum));
				$('#pay_buy_amt_sum').text(setComma(pay_buy_amt_sum));
				$('#rem_buy_amt_sum').text(setComma(rem_buy_amt_sum));
				$('#trans_vat_sum').text(setComma(trans_vat_sum));
				$('#vat_sum').text(setComma(vat_sum));
				$('#pay_vat_sum').text(setComma(pay_vat_sum));
				$('#rem_vat_sum').text(setComma(rem_vat_sum));
				$('#rem_sum_sum').text(setComma(rem_sum_sum));
	...
	function popupSearch(tbName1, tbName2, tbName3){		
		...
		var bodyValue = "";
		bodyValue += "<CAPTION><spring:message code='epc.pay.dash.perStoreSiljuk'/><br>";
	...
	function forwarding(val) {
		...
		$("#searchForm").attr("action", "<c:url value='/edi/payment/NEDMPAY0130.do'/>");
		$("#searchForm").submit();
	}
	...
	function printText(){
		...
		$("#searchForm").attr("action", "<c:url value='/edi/payment/NEDMPAY0120Text.do'/>");
		$("#searchForm").submit();
...
<tr class="r1">
	...
	<td rowspan="2" align="center"><c:out value="\${strNm}"/></td>
	<td align="center"><spring:message code='epc.pay.header.buyAmt'/></td>
	<td align="right"><c:out value="\${transBuyAmt}"/></td>
	<td align="right"><c:out value="\${buyAmt}"/></td>
	<td align="right"><c:out value="\${payBuyAmt}"/></td>
	<td align="right"><c:out value="\${remBuyAmt}"/></td>
	<td rowspan="2" align="right"><c:out value="\${remSum}"/></td>
	<td rowspan="2" align="center"><c:out value="\${boru}"/></td>
</tr>
<tr class="r1">
	<td align="center">세액</td>
	<td align="right"><c:out value="\${transVat}"/></td>
	<td align="right"><c:out value="\${vat}"/></td>
	<td align="right"><c:out value="\${payVat}"/></td>
	<td align="right"><c:out value="\${remVat}"/></td>
</tr>
</script>
			...
			<div class="wrap_con">
				...
				<div class="bbs_list">
					...
					<tr>
						...
						<th colspan="2"><spring:message code='epc.pay.header.strNm'/></th>
						...
						<th><spring:message code='epc.pay.header.hCode15'/></th>
						<th>&nbsp;</th>
					</tr>
					</table>
					...
					<colgroup>
						...
						<tr style="display:none;" class="r1 sum">
							<th rowspan="2" colspan="2" align="center"><b><spring:message code='epc.pay.header.totalSum'/></b></td>
							<th align="center"><b><spring:message code='epc.pay.header.buyAmt'/></b></td>
							<th align="right"><b id="trans_buy_amt_sum"></b></th>
							<th align="right"><b id="buy_amt_sum"></b></th>
							<th align="right"><b id="pay_buy_amt_sum"></b></th>
							<th align="right"><b id="rem_buy_amt_sum"></b></th>
							<th rowspan="2" align="right"><b id="rem_sum_sum"></b></th>
							<th rowspan="2"></td>
							<th rowspan="2">&nbsp;</td>
						</tr>
						<tr style="display:none;" class="r1 sum">
							<th align="center"><b><spring:message code='epc.pay.header.vat'/></b></td>
							<th align="right"><b id="trans_vat_sum"></b></th>
							<th align="right"><b id="vat_sum"></b></th>
							<th align="right"><b id="pay_vat_sum"></b></th>
							<th align="right"><b id="rem_vat_sum"></b></th>
						</tr>
						<tr style="display:none;" class="empty"><td colspan=9>&nbsp;<br>&nbsp;</td><th width=17 rowspan=2>&nbsp;</th></tr>
		...
		<div id="footbox">
			...
			<div class="location">
				<ul>
					...
					<li class="last"><spring:message code='epc.pay.perStoreSiljuk'/></li>
				</ul>
			</div>
		</div>
	</div>
