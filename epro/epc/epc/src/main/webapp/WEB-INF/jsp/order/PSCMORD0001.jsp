...
var excelConf;
	...
	$(document).ready(function() {
		...
		ibdata.Cols = [
		  ...
		  , {Header:"주문자", 		Type:"Text", 			SaveName:"CUST_NM",  					Align:"Center", Width:100, Edit:0}
		  ...
		  , {Header:"매출일자", 	    Type:"Text", 			SaveName:"SALE_DY", 	      			Align:"Center", Width:70,  Edit:0, Hidden:true}
		];
...
//     	param.excelYn =  "Y";
    ...
    function doExcel()
    {
		...
		if(confirm('엑셀 다운로드 하시겠습니까?')){
			...
			$.ajax({
				...
				dataType : 'json',
				async : false,
				url : '<c:url value="/order/selectCustInfoList.json"/>',
			...
			var xlsUrl = '<c:url value="/order/selectOrderListExcel.do"/>';
			...
			directExcelDown(mySheet, '주문목록조회_' + new Date().yyyymmdd(), xlsUrl, '#searchForm', param, hideCols); // 전체 다운로드
			}
	...
	function mySheet_OnSearchEnd(cd,msg) {
		...
		if ($("#searchType").val() == '2'){
			mySheet.SetColHidden("SALE_DY", 0);
		}else{
			mySheet.SetColHidden("SALE_DY", 1);
		}
