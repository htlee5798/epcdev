...
<c:set var="isFullFrm" value="N"/>			<%-- 풀화면 사용여부 (left 없음) --%>
<c:set var="isLeftFullFrm" value="N"/>		<%-- 풀화면 사용여부 (left 있음)--%>
<c:if test='${not empty paramMap.pgm_code and paramMap.pgm_code eq "MAIN"}'>
	<c:set var="isLeftFullFrm" value="Y"/>
</c:if>
...
<frameset rows="69px,*" frameborder="0" framespacing="0">
	...
	<frameset <c:if test='${isFullFrm ne "Y"}'>cols="190px,*"</c:if> id="widthFram" name="widthFram" frameborder="0" framespacing="0">
		<c:if test='${isFullFrm ne "Y"}'>
		<frame src="<c:url value="/edi/main/viewEdiLeft.do"/>?pgm_code=${paramMap.pgm_code}&pgm_sub=${paramMap.pgm_sub}&isFull=<c:out value='${isLeftFullFrm}'/>" id="lnbFram" name="lnbFram"  border="0" marginheight="0" marginwidth="0" scrolling="no"/>
		</c:if>
		<c:choose>
