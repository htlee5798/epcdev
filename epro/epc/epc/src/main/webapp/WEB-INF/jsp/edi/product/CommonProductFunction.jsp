...
<script type="text/javascript" >
	...
	function _eventChangeFiledByTradeType(tradeType) {
		...
		$("#newProduct input[name='chanCd']").not(":checked").each(function(){
			chanCd = $.trim($(this).val());
			$("#tblNewProdPrc tr[data-chan="+chanCd+"]").find("input, select").removeClass("required inputRead");
		});
		
		//선택된 채널에 대해서 필수체크 영역 컨트롤
		$("#newProduct input[name='chanCd']:checked").each(function(){
			chanCd = $.trim($(this).val());
			//선택된 채널에 대한 원매가 영역별 컨트롤
			$("#tblNewProdPrc tr[data-chan="+chanCd+"]").find("input, select").each(function(){
...
// 	}
	...
	function _eventSetupJangCheck(evntGbn) {
		...
		var isSuper = $("input[name='chanCd'][value='KR04']").is(":checked");		//채널 슈퍼인지 확인
		...
		if (tradeType == "1") {
			...
			if (prodTypeDivnCd == "1" || prodTypeDivnCd == "3"	||	prodTypeDivnCd == "5"	||	prodTypeDivnCd == "9") {
				...
				$("#newProduct input[name=sNewProdStDy]").val('');				//(슈퍼) 신상품 출시일자
		...
		var sNewProdStDy	=	$("#newProduct input[name=sNewProdStDy]").val().replace(/\s/gi, '');
		...
		if (newProdPromoFg == "X") {
			if (newProdStDy == "") {
				...
				$("#newProduct input[name=newProdStDy]").focus();
				return false;
			}
		} else {
...
// 				$("#newProduct input[name=sNewProdStDy]").focus();
// 				return false;
// 			}
		}else{
			$("#newProduct input[name=sNewProdStDy]").val("");
		}
	...
	function _fncCtrlVenZzorgInfo(json){
		...
		if(venPurDepts != null && venPurDepts.length > 0){
			const allChanCds = $("input[name='chanCd']").map(function(){return $(this).val();}).get();	//모든 온오프 채널
			...
			$.each(activeChans, function(i,val){
				//채널 활성화
				$("input[name='chanCd'][value='"+$.trim(val)+"']").prop("disabled", false);
				//선택된 체크박스일 경우, 상품정보 영역 show
				$("#tblNewProdAdd tr[data-chan="+$.trim(val)+"]").show();
			});
			
			//해당하지 않는 채널별 영역 비활성화
			$.each(deactiveChans, function(i,val){
				//채널 비활성화
				$("input[name='chanCd'][value='"+$.trim(val)+"']").prop("checked", false);
				$("input[name='chanCd'][value='"+$.trim(val)+"']").prop("disabled", true);
				//채널별 원매가 정보 영역 비활성화
				fncSetPriceAreaByChan($.trim(val), false);
				//선택되지 않은 것일 경우, 상품정보 영역 hide
				$("#tblNewProdAdd tr[data-chan="+$.trim(val)+"]").hide();
			});
			
			//현재 선택된 채널이 없을 경우, Default 채널 체크 ---------------------------------------------
			var chkedLen = $("input[name='chanCd']:checked").not(":disabled").length;
			if(chkedLen == 0 && activeChans.length > 0){
				//마트가 선택 가능한 채널일 경우,
				if(activeChans.includes("KR02")){
					//마트, 오카도(CFC) 선택
					$("input[name='chanCd'][value='KR02']").not(":disabled").prop("checked", true);
					$("input[name='chanCd'][value='KR09']").not(":disabled").prop("checked", true);
				...
				//마트 선택 불가이면서, 슈퍼 선택가능할 경우,
					//슈퍼 선택
					$("input[name='chanCd'][value='KR04']").not(":disabled").prop("checked", true);
				
					//슈퍼 원매가 영역 활성화
					fncSetPriceAreaByChan("KR04", true);
				}
				
				$("input[name='chanCd']").trigger("change");
			}
			//--------------------------------------------------------------------------------------
	...
	function _fncDeactiveAllChans(){
		...
		//채널선택 해제
		$("input[name='chanCd']").prop("checked", false);
		$("input[name='chanCd']").prop("disabled", true);
	}
</script>
