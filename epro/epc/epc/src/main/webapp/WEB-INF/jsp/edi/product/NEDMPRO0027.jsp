  ...
  var modifyResultHtml = {
    createInputDom : function() { 
      ...
      <c:forEach var="nutAttInfo" items="${nutAttInfo}" varStatus="status">
      	var unit = "<c:out value='${nutAttInfo.unit}' />";
      	var unitStr = "";   
      	if (unit) { 
			unitStr = " (" + unit + ")" 
		}     	 
        var contentHtml = "<th><div style='text-align:center; font-weight: bold;'>" + "<c:out value='${nutAttInfo.nutNm}' />" + unitStr + "</div></th>"
        				+ "<td><div style='text-align:center;'><input id=nutCd_" + "<c:out value='${nutAttInfo.nutCd}' />" + " class='nutValue' type='text' style='text-align:right;' size='24' maxlength='14'/></div></td>"
    ...
    getNutAmtByPgmId : function() { 
      <c:forEach var="nutAttProd" items="${nutAttProd}">
        var nutCd = "<c:out value='${nutAttProd.nutCd}' />";
        $("#nutCd_"+nutCd).val("<c:out value='${nutAttProd.nutAmt}' />");
      </c:forEach> 
    }    
