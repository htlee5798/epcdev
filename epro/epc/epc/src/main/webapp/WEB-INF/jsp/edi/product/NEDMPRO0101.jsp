<%--
	Page Name 	: NEDMPRO0101.jsp
	Description : 신상품등록 [코리안넷 전용 온오프전용-상품속성]
		...
		*/
		function doIsAttReApply() {
			...
			if (tblCnt != 0) {
				for (var i = 0; i < tblCnt; i++) {
					...
					if (orgVariant != "") {
						idxVariant = parseInt(orgVariant);
					}
				}
			}
			//--------------------------------------------------
			...
			for (var i = 0; i < tblCnt; i++) {
				...
				if (orgVariant != "") {		// 기등록된 변형속성일 경우
					$tbl.find("input[name='variant']").val(parseInt(orgVariant));
				} else {					// 새로 추가한 변형속성일 경우(기 등록된 변형속성 MAX값 ++)
					idxVariant++;
					$tbl.find("input[name='variant']").val(parseInt(idxVariant));
						...
						for (var j = 0; j < allValueCnt; j++) {
							if (i != j) {
								...
								if (!$("input[name='rowDel']").eq(j).is(":checked")) {	// 삭제 대상인건은 중복체크 제외
									if (iValue == jValue) {
										alert((parseInt(i) + 1) + "번째 속성과 " + (parseInt(j) + 1) + "번째 속성값이 중복입니다.");
										return;
									}
								}
							}
							...
							if (i != j) {
								...
								if (iValue != "") {
									...
									if (!$("input[name='rowDel']").eq(j).is(":checked")) {	// 삭제 대상인건은 중복체크 제외
										if (jValue != "") {
											if (iValue == jValue) {
												alert((parseInt(i) + 1) + "번째 속성과 " + (parseInt(j) + 1) + "번째 속성의 88코드 값이 중복입니다.");
												return;
											}
										}
									}
		...
		function addEcProductAttr(attrPiType, type) {
			...
			if (type == "change") {
				...
				var resetItemSubTable = '<tr><th style="width:48px">단품코드</th><th style="width:380px">옵션정보</th></tr>';
				$("#itemSubTable").html(resetItemSubTable);
			...
			$.ajax({
				...
				success: function(data) {
					...
					if (attrPiType == "P") {
						...
						if (list.length > 0) {
							if (type == "change") {
								$("#singleProductAttrDiv").html("<br/><font color=blue style='font-size:11px;'>※ 상품 속성</font><br/><input type='radio' name='prodDirectYn' value='N' /> 선택입력 &nbsp;<input type='radio' name='prodDirectYn' value='Y' /> 직접입력<br/>");
								$(":radio[name='prodDirectYn']:radio[value='N']").prop('checked', true); // 선택입력 체크
							}
							for (var i = 0; i < list.length; i++) {
								if (list[i].NUM == 1) {
					...
					success: function(data) {
						...
						if (attrPiType == "P") {
							...
							if (list.length > 0) {
								...
								$("#singleProductAttrDiv").html("<br/><font color=blue style='font-size:11px;'>※ 상품 속성</font>&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;<input type='radio' name='prodDirectYn' value='N' ${prodDirectN} /> 선택입력 &nbsp;<input type='radio' name='prodDirectYn' value='Y' ${prodDirectY} /> 직접입력<br/>");
				...
				<div class="wrap_search">
					...
					<div id="prodTabs" class="tabs" style="padding-top:10px;">
						<ul>
							...
							<!-- 단일 상품일 경우 -->														
							<c:if test="${prodDetailInfo.prodAttTypFg eq '00'}">
								<!-- 추가정보 입력란의 혼재여부가 단일일 경우에만 물류바코드 탭 disply 표기 -->
								<c:if test="${prodDetailInfo.mixYn eq '0'}">
									<li id="pro04" style="cursor: pointer;">물류바코드</li>
								</c:if>
							</c:if>
							
							<!-- 묶음 상품일 경우 -->
							<c:if test="${prodDetailInfo.prodAttTypFg eq '01'}">
								<c:if test="${prodDetailInfo.inputVarAttCnt > 0}">
									<li id="pro04" style="cursor: pointer;">물류바코드</li>
								</c:if>
							</c:if>						
					...
					<div id="divProdVarAtt" name="divProdVarAtt">
						<div class="bbs_list">
							<ul class="tit">
								...
								<!-- 상품 확정요청을 하게 되면 버튼 비활성화 -->
								<c:if test="${empty prodDetailInfo.mdSendDivnCd}">
									<li class="btn">
							...
							<table class="bbs_grid3" cellpadding="0" cellspacing="0" border="0">
								...
								<tr>
									<th colspan="4" align="left" style="padding-left:15px;">
										<!-- 변형속성이 있을때만 판매코드 입력참조를 보여준다 -->
										<c:if test="${not empty classList}">
											<strong><font color="red">* 판매코드는 변형속성별로 입력해주세요.(판매코드 참조:<c:out value="${param.srcmk_cd}"/>)</font></strong><br/>	
										</c:if>										
						...
						<div class="bbs_list">
							...
							<div id="divAtt" name="divAtt">
								...
								<c:forEach items="${varAtt}" var="att" varStatus="status">
									...
									<%--
										<c:if test="${att.variant != preVariant}">
											<th id="rowDelTr" name="rowDelTr" rowspan="<c:out value="${att.rowSpan}" />"><input type="checkbox" id="rowDel" name="rowDel" value="1"></th>
									 ...
									 --%>		
										...
										<c:if test="${att.variant != preVariant}">
											<th rowspan="<c:out value="${att.rowSpan}" />">88코드</th>
					...
					<div id="divGrpAtt">
						<ul name="productCertTemplateTitle"	id="productCertTemplateTitle" class="tit mt10">
							<div class="bbs_list">
								<ul class="tit">
									<li class="tit">세부분류 속성<font color="red">(해당되지 않는 속성은 반드시 '해당없음' 으로 선택해주세요)</font></li> 
						...
						<div class="bbs_list">
							...
							<!-- [200420 EDI상품 범주 및 속성 추가 (칸 맞추기 위해서)] START -->
							...
							<!-- [200326 EDI상품 범주 및 속성 추가 (칸 맞추기 위해서)] END -->
								...
								<tr>
									...
									<br/><font color=blue style="font-size:11px;">※ 단품 정보</font><br/>
										...
										<table id="itemSubTable" style="width: 700px">
											...
											<c:if test="${itemList.itemCode ne null}">
