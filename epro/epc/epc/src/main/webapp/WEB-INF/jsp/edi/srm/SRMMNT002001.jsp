...
//		alert($(".on").html());
		...
		function drawChart(obj){
			...
			$.ajax({
				...
				dataType : 'json',
				async : false,
				url : '<c:url value="/edi/mnt/selectCEOSRMmoniteringDetailChart.json"/>',
...
//						labelList.push(data[i].countYear + "-" + data[i].countMonth);
		...
		function currentValueDrawChart(){
			...
			$.ajax({
				...
				dataType : 'json',
				async : false,
				url : '<c:url value="/edi/mnt/selectSRMmoniteringDetailCurrentValue.json"/>',
				data : JSON.stringify(searchInfo),
				success : function(data) {
		...
		function popup(gugun, evItemNo, evIeSeq, grade){
			...
			if(gugun =="remark"){
				//비고
				PopupWindow(450, 250, "<c:url value="/edi/mnt/selectSRMmoniteringDetailRemarkPopup.do"/>","remarkPopup");
			} else if(gugun =="etc"){
				//특이사항
				PopupWindow(450, 250, "<c:url value="/edi/mnt/selectSRMmoniteringDetailEtcPopup.do"/>","etcPopup");
			} else if(gugun =="grade"){
				PopupWindow(700, 250, "<c:url value="/edi/mnt/selectSRMmoniteringDetailGradeExemplePopup.do"/>","gradePopup");
			} else if(gugun =="plc"){
				...
				PopupWindow(500, 550, "<c:url value="/edi/mnt/selectSRMmoniteringDetailPlcPopup.do"/>","plcPopup");
			}
		}
		...
		function defectivePopup(){
			if("<c:out value="${defective}"/>" == "0"){
				...
				$('#popupForm input[name=catLv2Code]').val("<c:out value="${list[0].catLv2Code}"/>");
				PopupWindow(900, 550, "<c:url value="/edi/mnt/selectSRMmoniteringDetailDefectivePopup.do"/>","defectivePopup");
			}
		}
			...
			<tr>
				<td style="vertical-align: top;">
					<div class="popLeftArea">
						...
						<div class="complain2" style="text-align:center;"><a href='#' onClick='defectivePopup();'><spring:message code="text.srm.field.defective"/></a><span><c:out value="${defective}"/></span></div>
						<div style='height:2px;'></div>
				...
				<td style="vertical-align: top;">
					...
					<div class="popContArea" style="width:100%">
						...
						<div align="right" style="font-size: 15px;font-weight: bold;"><c:out value="${countYear}"/><spring:message code="text.srm.field.year"/> <c:out value="${countMonth}"/><spring:message code="text.srm.field.month"/></div>
							...
							<tr>
								<th class="c_title_2">
									<a href="#" onclick="javascript:doAllDisplay(this);">
										<spring:message code="table.srm.srmmnt002001.colum.title2"/><%--spring:message : 구분--%>
										(<span class="plusMinus">+</span>)
									</a>
								</th>
								<th class="c_title_2"><spring:message code="table.srm.srmmnt002001.colum.title3"/></th><%--spring:message : 등급--%>
								<th class="c_title_2"><spring:message code="table.srm.srmmnt002001.colum.title4"/></th><%--spring:message : 당월--%>
								<th class="c_title_2"><spring:message code="table.srm.srmmnt002001.colum.title5"/></th><%--spring:message : 단위--%>
								<th class="c_title_2"><spring:message code="table.srm.srmmnt002001.colum.title6"/></th><%--spring:message : 개선필요--%>
								<th class="c_title_2"><spring:message code="table.srm.srmmnt002001.colum.title7"/></th><%--spring:message : 비고--%>
								<th class="c_title_2"><spring:message code="table.srm.srmmnt002001.colum.title8"/></th><%--spring:message : 특이사항--%>
							</tr>
							</thead>
