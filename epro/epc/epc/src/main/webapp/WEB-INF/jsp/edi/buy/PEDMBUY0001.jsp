...
<script>
	...
	function doSearch() {
		...
		if(dateValid(form)){
			loadingMaskFixPos();
			form.action  = "<c:url value='/edi/buy/PEDMBUY0001Select.do'/>";
			form.submit();		
		}	
	...
	function popupSearch(tbName1, tbName2, tbName3){		
		...
		form.staticTableBodyValue.value = "<CAPTION>매입정보 일자별 현황표<br>"+
	...
	function forwarding(val) {
		...
		form.action  = "<c:url value='/edi/buy/PEDMBUY0002Select.do'/>";
		form.submit();	
	}
</script>
				...
				<div class="wrap_search">
					...
					<div class="bbs_search">
						...
						<tr>
							...
							<th>매입구분</th>
							<td colspan="3">
									<html:codeTag objId="buying" objName="buying" comType="SELECT" formName="form" selectParam="<c:out value='${param.buying}'/>" parentCode="SPP02" defName="전체"></html:codeTag>
				...
				<div class="wrap_con">
					...
					<div class="bbs_list">
						...
						<c:if test="${not empty buyList }">
							...
							<c:set var="tot_sum5"  value="0" />
							
							<c:forEach items="${buyList}" var="list" varStatus="index" >
								<tr class="r1">
									<td align="center"><a href="javascript:forwarding('${list.BUY_DY }');">${list.BUY_DY }</a></td>
								...
								<c:set var="tot_sum1" value="${tot_sum1 + list.TOT_QTY }" />
								<c:set var="tot_sum2" value="${tot_sum2 + list.TOT_PRC }" />
								<c:set var="tot_sum3" value="${tot_sum3 + list.BUY_BOX_QTY }" />
								<c:set var="tot_sum4" value="${tot_sum4 + list.BUY_QTY }" />
								<c:set var="tot_sum5" value="${tot_sum5 + list.BUY_BUY_PRC }" />
							</c:forEach>
							
							
						</c:if>
						<c:if test="${empty buyList }">
							<tr><td colspan="5" align=center>Data가 없습니다.</td></tr>
						</c:if>
						</table>
						</div>
						<table class="bbs_list" cellpadding="0" cellspacing="0" border="0" id="testTable3">
							...
							<c:if test="${not empty buyList }">
							<tr>
								<th class="fst" align=center><b>합계</b></th>
								<th align=right><b><fmt:formatNumber value="${tot_sum1 }" type="number" currencySymbol="" /></b></th>
								<th align=right><b><fmt:formatNumber value="${tot_sum2 }" type="number" currencySymbol="" /></b></th>
								<th align=right><b><fmt:formatNumber value="${tot_sum3 }" type="number" currencySymbol="" /></b></th>
								<th align=right><b><fmt:formatNumber value="${tot_sum4 }" type="number" currencySymbol="" /></b></th>
								<th align=right><b><fmt:formatNumber value="${tot_sum5 }" type="number" currencySymbol="" /></b></th>
								<th>&nbsp;</th>
							</tr>
							</c:if>
							<c:if test="${empty buyList }"><tr><td colspan=5>&nbsp;</td><th width=17>&nbsp;</th></tr></c:if>
						</table>	
					</div>
				</div>
			</div>
