...
<script>
	...
	function doSearch() {
		...
		loadingMaskFixPos();
		form.action  = "<c:url value='/edi/payment/PEDMPAY0006Select.do'/>";
		form.submit();	
		...
		<div id="wrap_menu">
			...
			<div class="wrap_search">
				...
				<div class="bbs_search">
					<ul class="tit">
						<li class="tit">검색조건 &nbsp;<font color="red">(2014년 4월 이전 조회페이지)</font></li>
						<li class="btn">
			...
			<div class="wrap_con">
				...
				<div class="bbs_list">
					...
					<tr>
						<th rowspan="2">월</th>
						<th rowspan="2">점포</th>
						<th colspan="4">당월매입</th>
						<th colspan="2">전산판매장려금</th>
						<th>수기판매장려금</th>
						<th colspan="2">합계</th>
					</tr>
					<tr>
					...
					<c:if test="${not empty paymentList }">
						<c:forEach items="${paymentList}" var="list" varStatus="index" >
							<tr class="r1">
								<td align="center">${list.SUB_YM }</td>
								<td align="center">${list.STR_NM }</td>
								<td align=right><fmt:formatNumber value="${list.BUY_BUY_AMT }" type="number" currencySymbol="" /></td>
								<td align=right><fmt:formatNumber value="${list.PB_BUY_AMT }" type="number" currencySymbol="" /></td>
								<td align=right><fmt:formatNumber value="${list.EXPROMO_BUY_AMT }" type="number" currencySymbol="" /></td>
								<td align=right><fmt:formatNumber value="${list.PROMO_ADJ_AMT }" type="number" currencySymbol="" /></td>
								<td align=right><fmt:formatNumber value="${list.PROMO_AMT }" type="number" currencySymbol="" /></td>
								<td align=right><fmt:formatNumber value="${list.PROMO_AMT_RT }" type="number" currencySymbol="" />%</td>
								<td align=right><fmt:formatNumber value="${list.MANUAL_PROMO_AMT }" type="number" currencySymbol="" /></td>
								<td align=right><fmt:formatNumber value="${list.SUM_AMT }" type="number" currencySymbol="" /></td>
								<td align=right><fmt:formatNumber value="${list.MANUAL_RT }" type="number" currencySymbol="" />%</td>
							</tr>
						</c:forEach>
					</c:if>
