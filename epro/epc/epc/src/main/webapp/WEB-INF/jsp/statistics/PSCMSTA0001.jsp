...
function goPage(pageIndex){
	...
	var url = '<c:url value="/statistics/selectNaverEdmSummaryList.do"/>';
...
function doExcel(){
	...
	var url = '<c:url value="/statistics/selectNaverEdmSummaryListExcel.do"/>';
	f.pageIndex.value = 1;
	f.action = url;
	f.submit();	
}
			...
			<tr>
				...
				<td>
					<input type="hidden" name="rootAffiliateLinkNo" value="${searchVO.rootAffiliateLinkNo}" />
						<c:if test="${searchVO.rootAffiliateLinkNo eq '0015'}">
						<select name="affiliateLinkNo" class="select">
							<c:forEach items="${affiliateLinkNoList}" var="affiliateLinkNo" begin="0">
							<c:if test="${fn:indexOf(affiliateLinkNo.AFFILIATE_LINK_NM, '지식쇼핑')>=0}">
						...
						<c:if test="${searchVO.rootAffiliateLinkNo != '0015'}">
						<select name="affiliateLinkNo" class="select">
							<c:forEach items="${affiliateLinkNoList}" var="affiliateLinkNo" begin="0">
								<option value="${affiliateLinkNo.AFFILIATE_LINK_NO}"
									<c:if test="${affiliateLinkNo.AFFILIATE_LINK_NO == searchVO.affiliateLinkNo}">selected</c:if>
