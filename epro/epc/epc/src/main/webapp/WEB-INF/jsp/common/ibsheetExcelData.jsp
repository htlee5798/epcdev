...
String error = SecureUtil.stripXSS(request.getParameter("error"));
...
// 	String sheetRemoveAll = (String)request.getAttribute("sheetRemoveAll");	//Sheet 초기화 여부
	...
	String[] colNms 		= (SecureUtil.stripXSS((String)request.getAttribute("colNms"))).split("\\^"); 		//엑셀의 칼럼명 ^로 구분하여 순서대로 처리
	String functionNm 		= SecureUtil.stripXSS((String) request.getAttribute("func"));						//실행 자바스크립트 함수명
	String sheetNm 			= SecureUtil.stripXSS((String) request.getAttribute("sheetNm"));					//IBSheet 변수명
	String sheetRemoveAll 	= SecureUtil.stripXSS((String) request.getAttribute("sheetRemoveAll"));				//Sheet 초기화 여부
