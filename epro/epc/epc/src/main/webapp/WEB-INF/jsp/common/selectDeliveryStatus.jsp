...
function fnSubmit(){
	var hodecoCd = "<c:out value='${hodecoCd}' />";
	var hodecoInvoiceNo = "<c:out value='${hodecoInvoiceNo}' />";
	var actionUrl = "";
	var $form1 = $("#form1");
					...
					<table summary="배송정보 입니다. 각 항목으로는 배송업체, 송장번호, 발송인, 수령인이 있습니다.">
						...
						<tbody>
							<tr>
								...
								<td><c:out value="${tCodeNm}" /></td>
							</tr>
							<tr>
								<th scope="row">송장번호</th>
								<td><c:out value="${invoiceNo}" /></td>
							</tr>
							<tr>
								<th scope="row">발송인</th>
								<td><c:out value="${senderName}" /></td>
							</tr>
							<tr>
								<th scope="row">수령인</th>
								<td><c:out value="${receiverName}" /></td>
							</tr>
						</tbody>
					</table>
				</div>
				<div class="state">
					...
					<div class="inner">
						...
						<c:forEach items="${trackingDetails}" var="nRow" begin="0"  step="1" varStatus="status">
							...
							<div class="s_box">
								<span class="date"><c:out value="${nRow.timeString}" /></span>
								<p>[<c:out value="${nRow.where}" />] <c:out value="${nRow.kind}" /></p>
								...
								<span class="driver"> <c:out value="${manName}" /><em><c:out value="${nRow.telno2}" /></em></span>
								</c:if>
