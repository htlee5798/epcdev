...
<%--
	Page Name 	: SRMEVL0030.jsp
	Description : 품질경영평가 대상
...
//		searchInfo["pageRowCount"] = $('input[name="pageRowCount"]:radio:checked').val();
		...
		$.ajax({
			...
			dataType : 'json',
			async : false,
			url : '<c:url value="/edi/evl/selectQualityEvaluationList.json"/>',
	...
	function doDetail(houseCode, sgNo, evNo, vendorCode, evTplNo, statusNm, sellerNameLoc, evalNoResult, visitSeq, seq, progressCode) {
		...
		$('#hiddenForm input[name="evalFlag"]').val('0');


		if(progressCode == 100) {
			//접수화면
			popupWindow("<c:url value='/edi/evl/receiptPopup.do'/>",880,720);
			return;
	...
	function doExcelDown(){
		...
		$("#excelForm").attr("action", "<c:url value='/edi/evl/selectQualityEvaluationListExcel.do'/>");
		$("#excelForm").submit();
	}
	...
	<div id="container">
		...
		<div class="inner sub_wrap">
			...
			<div class="sub_top">
				...
				<p class="page_path"><spring:message code="evlHeader.menu.text2" /><span><spring:message code="text.srm.field.srmevl0030.sub.title1"/></span></p>
			</div>
			<!-- END 서브상단 -->
			...
			<table class="tbl_st1">
				...
				<thead>
					<tr>
						...
						<th><spring:message code="table.srm.srmevl0030.colum.title4"/></th><%--spring:message : 분류--%>
