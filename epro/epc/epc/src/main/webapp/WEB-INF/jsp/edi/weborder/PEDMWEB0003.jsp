...
<script  type="text/javascript" >
	...
	$(function() {
		...
		_excelSelectCodeOptionTag($('#entp_cd').val(), "#fileGrpCd", "선택");
	});
	...
	/* 초기 설정 */
	function _init(){
		_setTbodyNoResult($("#datalist"), 	14, '지역/점포, 협력업체 코드, 상품코드, 업로드 파일/상태를  선택 또는 입력하여 [조회] 하세요!');	// prod tBody 설정
	}
	...
	function _eventDelete(){
		...
		$.post(
				"<c:url value='/edi/weborder/tedOrdPackDelete.do'/>",
				JSON.stringify(str),
				function(json){				
					if(jQuery.trim(json) == ""){//처리성공
	...
	function _eventExcelUpload(){
		...
		if(fileValueCheck($('#file').val())){
			loadingMaskFixPos();
			form.action  = '<c:url value="/edi/weborder/ordExcelUpload.do"/>';
			form.submit();
		}
	}
	...
	function _eventSearch(){
		...
		$.post(
				"<c:url value='/edi/weborder/tedOrdPackSelect.do'/>",
				JSON.stringify(str),
				function(data){
					hideLoadingMask();
	...
	function _viewMdState(){
		...
		message = "발주 중단 또는 상품(점포코드) 확인이 필요한 데이터 입니다.\n수정 후 다시 업로드 하여 주십시요.";
		alert(message);
	}
		...
		<form id="excel" name="excel" method="post" enctype="multipart/form-data">
			<div class="wrap_search" style="padding-bottom: 10px;">
				...
				<div class="bbs_search">
					<ul class="tit">
						<li class="tit">
							Excel 발주등록<span style="font-weight: normal; color: #414fbb; margin-left: 25px;"><span><img src="/images/epc/btn/icon_04.png" alt="Notice" /></span>&nbsp; 발주서를 Excel File로 업로드 할 경우 <span style="color: red;">점포코드/상품코드/발주량</span> 순으로 작성하여 업로드 하세요!</span>
			...
			<div class="wrap_search">
				...
				<div class="bbs_search">
					<ul class="tit">
						<li class="tit">
							검색조건
							<span style="font-weight: normal; color: #414fbb; margin-left: 60px;"><span><img src="/images/epc/btn/icon_04.png" alt="Notice" /></span>  업로드 파일단위 발주 Data를 삭제 합니다.</span>
				...
				<div class="bbs_list">
					...
					<table id="dataTable" width="100%;" cellpadding="1" cellspacing="1" border="0" width=100% bgcolor=efefef>
                        ...
                        <tr bgcolor="#e4e4e4" align=center> 
                          <th rowspan="2">No</th>
	                      <th rowspan="2">파일<br>구분</th>
	                      <th rowspan="2">등록<br>상태</th>
