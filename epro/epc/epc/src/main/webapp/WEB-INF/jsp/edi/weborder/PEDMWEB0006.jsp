...
<script  type="text/javascript" >
	...
	$(function() {
		...
		$('#btnProdSend').unbind().click(null, 	 _eventProdSendCheck);			// 반품요청
	...
	function _eventProdSearch(){
		...
		var str = {  "prodCd" 	: $("#prodCd"  ).val()	//검색조건 상품코드 
			...
			$.post(  
					"<c:url value="/edi/weborder/PEDMWEB0099SearchProd2.do"/>",  // 공통 [PEDMWEB0099]
					JSON.stringify(str),
					function(data){
						...
						else
						{
			    			...
			    			$("#rtnQty1").focus();	// 수량입력필드로 focus 설정
	...
	function _eventProdSave(){
		...
		for ( var i = 0; i < ($("#rtnCnt").val()); i++) {
			var cnt = i+1;
			//alert("rtncnt"+i);
			//alert($("#rtnQty"+cnt).val());
			//alert("재고수량 : " + $("#rrlStkQty"+cnt).val() + " 반품수량 : " + $("#rtnQty"+cnt).val());
			if( ($("#rtnQty"+cnt).val()) > ($("#rrlStkQty"+cnt).val()) ){
		...
		for ( var i = 0; i < ($("#rtnCnt").val()); i++) {
			...
			if(($("#rtnQty"+cnt).val()) > 0){
				...
				var str = {  "prodCd" 	: $("#prodCd"	).val()	//상품코드 
			    		,"venCd"	: $("#entp_cd"		).val()		//업체코드
			    		,"strCd"	: $("#rtnStrCd"+cnt	).val()		//점포코드
			    		,"rrlQty"	: $("#rtnQty"+cnt	).val()		//반품수량
			      };
				//alert($("#rtnStrCd"+cnt	).val());
				//alert($("#rtnQty"+cnt	).val());
				<%-- ----------- ============================= --%>
	...
	function doSearchReturnList(){
		...
		$.post(
				"<c:url value='/edi/weborder/PEDMWEB0005SearchList2.do'/>",
				JSON.stringify(str),
				function(data){
					if(data == null || data.state != "0"){
	...
	function _setTbodyProdReturnValue(json){
		...
		if (sz > 0) {
			for ( var k = 0; k < sz; k++) {
				...
				if(data[k].regStsCd == '1' || data[k].regStsCd == null){
				...
				if(data[k].regStsCd == '9'){
					...
					eleHtml[++h] = " <td align='right' class='through_tr'>"+amtComma(data[k].rrlStkQty2)+"</td>		\n";
					...
					//eleHtml[++h] = " <td align='center'>"+data[k].rrlStkQty+"</td>				\n";
					//eleHtml[++h] = " <td align='center'>"+data[k].ordUnit+"</td>				\n";
					eleHtml[++h] = " <td align='right' style='color:blue; font-weight: bolder;'>"+amtComma(data[k].rrlStkQty2)+"</td>		\n";
					...
					if(data[k].regStsCd == null){
						eleHtml[++h] = " <td align='right'><input type='text' tabindex='"+(k+1)+"' id='rtnQty"+(k+1)+"' name='rtnQty' maxlength='10' value='' style='text-align: right; width:60px;' onkeypress='onlyNumber();' onblur='amtFormat(this); unNumberFormat(this); sumOrdInfo(this);'/></td>		\n";
					}else{																																													
						eleHtml[++h] = " <td align='right'>"+amtComma(data[k].rrlQty)+"</td>		\n";
					}
				...
				eleHtml[++h] = "<input type='hidden' id='rtnStrCd"+(k+1)+"' name='rtnStrCd"+(k+1)+"' value='"+data[k].strCd+"'>   \n";
				eleHtml[++h] = "<input type='hidden' id='rrlStkQty"+(k+1)+"' name='rrlStkQty"+(k+1)+"' value='"+amtComma(data[k].rrlStkQty2)+"'>   \n";
				eleHtml[++h] = "<input type='hidden' id='regSts"+(k+1)+"' name='regSts"+(k+1)+"' value='"+data[k].regStsCd+"'>   \n";

				eleHtml[++h] = "</td>		\n";
				eleHtml[++h] = "</tr>		\n";
				$("#rtnCnt").val(k+1);
				//jegosum = jegosum + amtComma(data[k].rrlStkQty2); 
			}
			//$("#jegoSum"	).text(jegosum);		//재고SUM
...
<body>
	...
	<div>
		...
		<input type="hidden" name="rtnCnt" id="rtnCnt" />
		<!-- <input type="hidden" name="totRrlStkQty" id="totRrlStkQty" /> -->
		<div id="wrap_menu">
		...
		<!--	@ 검색조건	-->
			...
			<div class="bbs_search">
				<ul class="tit">
					...
					<li class="btn"><span style="font-weight: normal;  color: #414fbb; text-align: right;"><span><img src="/images/epc/btn/icon_04.png" alt="Notice" /></span> 협력업체 선택 후 상품을 검색하여 수량을 등록한 다음 반품요청 버튼을 클릭하세요. </span>
					<span style="font-weight: normal; color: #414fbb; margin-left: 25px;"><img src="/images/epc/btn/icon_04.png" alt="Notice" />&nbsp; 반품가능 시간은 <span style="color: red;"><spring:message code="msg.weborder.vendor.send.time.rtn"/> </span>입니다.</span></li>
				</ul>
					...
					<tr>
						...
						<td colspan=7  style="border-bottom-color: #ffffff;">
							 <table>
								<tr>
									<td><b>업체코드 </b></td>
									<td><html:codeTag objId="entp_cd" objName="entp_cd" width="100px;" formName="form" selectParam="" dataType="CP" comType="SELECT" /></td>
									...
									<td><a href="#" class="btn" id="btnProdSelect" ><span>조회</span></a></td>
									<!--  
									<td>
					...
					<tr>
						...
						<td style="border-bottom-color: #ffffff; ">
							...
							<tr>
								...
								<td><a href="#" class="btn" id="btnProdSave" ><span style="font-weight: bold; font-size:1.1em; color: red;">등록하기</span></a>
								<span style="font-weight: normal; color: #414fbb;">&nbsp; <span><img src="/images/epc/btn/icon_04.png" alt="Notice" /></span> 반품은 단위가 아닌 수량으로  입력하세요.</span>
								</td>
							</tr>	
							</table>
						</td>
			...
			<div class="wrap_con">
				...
				<div class="bbs_list">
					<ul class="tit">
						...
						<li class="btn">
							<!-- <a href="#" class="btn" id="btnProdSave"><span>수량등록</span></a>
