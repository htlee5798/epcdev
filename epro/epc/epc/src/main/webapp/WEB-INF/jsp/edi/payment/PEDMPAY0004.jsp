...
<script>
	...
	$(document).ready(function($) {		
		...
		$("select[name='tax']").val("<c:out value='${param.tax}'/>");	// 협력업체 선택값 세팅
	...
	function doSearch() {
		...
		if(dateValid(form)){
			loadingMaskFixPos();
			form.action  = "<c:url value='/edi/payment/PEDMPAY0004Select.do'/>";
			form.submit();
		}	
	...
	function popupSearch(tbName1, tbName2, tbName3){		
		...
		var tax="";
		for(var i=0;i<form.tax.options.length;i++){
			if(form.tax.options[i].selected == true){
				tax=form.tax.options[i].text;
           }
		}

		form.staticTableBodyValue.value = "<CAPTION>결산정보 세금계산서 현황표<br>"+
		"[조회기간 : "+date+"] [점포 : "+store+"] [과세구분  : "+tax+"] [업체 : "+tmp+"]<br>"+
	...
	function printText(){
		...
		form.text_data.value="[조회기간 : "+date+"] [점포 : "+store+"] [과세구분  : "+tax+"] [업체 : "+tmp+"]";

		form.action  = "<c:url value='/edi/payment/PEDMPAY0004Text.do'/>";
		form.submit();	
			...
			<div class="wrap_search">
				...
				<div class="bbs_search">
					...
					<tr>
						...
						<th>과세구분</th>
						<td >
							<html:codeTag objId="tax" objName="tax" comType="SELECT" selectParam="" formName="form" parentCode="TAX02" subCode="1" defName="전체" ></html:codeTag>
						</td>
					</tr>
					</table>
				</div>
				...
				<div class="bbs_list">
					...
					<c:if test="${not empty paymentList }">
						...
						<c:forEach items="${paymentList}" var="list" varStatus="index" >
							<tr class="r1">
								...
								<td align="center">${list.TAX_FG }</td>
								...
								<td align=right><fmt:formatNumber value="${list.TOTAL }" type="number" currencySymbol="" /></td>
							</tr>
							
							<c:set var="buy_amt_sum" value="${buy_amt_sum + list.BUY_AMT }" />
					...
					<c:if test="${not empty paymentList }">
						<tr class="r1">
							...
							<th align=right><b><fmt:formatNumber value="${buy_amt_sum }" type="number" currencySymbol="" /></b></th>
							<th align=right><b><fmt:formatNumber value="${vat_sum }" type="number" currencySymbol="" /></b></th>
							<th align=right><b><fmt:formatNumber value="${total_sum }" type="number" currencySymbol="" /></b></th>
							<th>&nbsp;</th>
						</tr>
					</c:if>
					<c:if test="${empty paymentList }"><tr><td colspan=5>&nbsp;</td><th width=17>&nbsp;</th></tr></c:if>
					</table>
