...
<script>
	...
	function doSearch() {
		...
		if(dateValid(form)){
			loadingMaskFixPos();
			form.action  = "<c:url value='/edi/delivery/PEDMDLY0002Select.do'/>";
			form.submit();		
		}	
	}
	...
	function doUpdate() {
		...
		if(tmp==""){
			...
			form.action  = "<c:url value='/edi/delivery/PEDMDLY0002Update.do'/>";
			form.submit();
		}
	...
	function popupSearch(tbName1, tbName2){		
		...
		form.staticTableBodyValue.value = "<CAPTION>배달정보 접수확인 현황표<br>"+
	...
	function printText(){
		...
		form.action  = "<c:url value='/edi/delivery/PEDMDLY0002Text.do'/>";
		form.submit();	

	}

	function cancelJyunPyo(val){
		var form = document.forms[0];

		form.cancelJyun.value=val;

		form.target="_blankPop";
		form.action ="<c:url value='/edi/delivery/PEDPDLY0002Select.do'/>";

		var popInfo = window.open('','_blankPop','top=0, left=0, width=1200, height=350, toolbar=no, status=yes, scrollbars=yse');
...
<body>
	...
	<div>
		...
		<input type="hidden" name="cancelJyun"/>
		<div id="wrap_menu">
			<!--	@ 검색조건	-->
					...
					<tr>
						...
						<td>
							<select id="hangmok" name="hangmok">
								...
								<option value="2" <c:if test="${param.hangmok == '2' }"> selected </c:if>>취소</option>
							</select>
						</td>
					</tr>
					</table>
					...
					<input type="hidden" name="firstList" value="exist"/>
						<c:forEach items="${deliveryList}" var="list" varStatus="index" >
							<tr class="r1">
							    ...
							    <c:when test="${list.ACCEPT_FG == 0}">
									<td rowspan="4" align="center">
										<input type="checkbox" name="accept_check" value="${list.SALE_DY};${list.STR_CD};${list.POS_NO};${list.TRD_NO};${list.SALE_PROD_SEQ}@">
									</td>
							    </c:when>
							    <c:when test="${list.ACCEPT_FG == 1}">
							    	<td rowspan="4" align="center">접수</td>
							    </c:when>
							    <c:otherwise>
							    	<td rowspan="4" align="center">취소</td>
							    </c:otherwise>
							    </c:choose>
							</tr>
							<tr class="r1">
								...
								<td rowspan="2" align="left">&nbsp;${list.CUST_ADDR }</td>
							...
							<tr class="r1">
								...
								<c:choose>
							    <c:when test="${list.CANCEL_FG eq '2'}">
							    	<td align="center">취소전표</td>
							    </c:when>
							    <c:otherwise>
							    	<c:choose>
							...
							<tr class="r1">
								<c:choose>
									<c:when test="${list.CANCEL_FG eq '2'}">
										<td align="center" colspan="7">이 전표는 배달 취소 전표 입니다. <a href="javascript:cancelJyunPyo('${list.ORGIN_DELI_SLIP_NO }');"> <B>원전표 보기</B></a></td>
									</c:when>
									<c:otherwise>
										<td align="center" colspan="7">&nbsp; ${list.MAP }</td>
									</c:otherwise>
								</c:choose>
							</tr>
						</c:forEach>
