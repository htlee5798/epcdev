<%--
	Page Name 	: NEDMBUYO0040.jsp
	Description : 매입정보 전표별 조회 페이지 
...
<script id="dataListTemplate" type="text/x-jquery-tmpl">
    ...
    <tr class="r1">
		<td align="center"><c:out value="\${buyDy }"/></td>
    	<td align="center"><a href="javascript:forwarding('<c:out value="\${buyDy }"/>');"><c:out value="\${strNm }"/></a></td>
    	<td align=center style="display:none;"><c:out value="\${strCd }"/></td>
    	<td align="center" style="mso-number-format:'\@'"><c:out value="\${buySlipNo }"/></td>
    	<td align="center"><c:out value="\${ordDy }"/></td>
...
<script>
	...
	function doSearch() {
		...
		if (dateValid()) {
			$.ajax({
				...
				dataType : 'json',
				async : false,
				url : '<c:url value="/edi/buy/NEDMBUY0040Select.json"/>',
	...
	function doExcel(tbName1, tbName2, tbName3){		
		...
		var bodyValue = "";
		bodyValue += "<CAPTION><spring:message code='text.buy.field.buySlipNoList'/><br>";
	...
	function printText() {
		...
		$("#searchForm").attr("action", "<c:url value='/edi/buy/NEDMBUY0040Text.do'/>");
		$("#searchForm").submit();	
	}
</script>
					...
					<div class="bbs_list">
						...
						<table class="bbs_list" cellpadding="0" cellspacing="0" border="0" id="sumTbl">
							...
							<tr id="sumRow" style="display: none;">
								<th colspan="4" class="fst" align=center><b><spring:message code="epc.buy.header.total" arguments="" /></b></th>
			...
			<div id="footbox">
				...
				<div class="location">
					<ul>
						...
						<li class="last"><spring:message code='epc.buy.menu.slipNo' /></li>
					</ul>
				</div>
			</div>
		</div>
