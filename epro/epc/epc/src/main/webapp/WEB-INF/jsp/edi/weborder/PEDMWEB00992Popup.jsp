...
<script  type="text/javascript" >
	...
	function _init(){
		...
		_setTbodyNoResult($("#datalist"), 	4, '상품코드,판매코드,상품명 검색조건을 입력 후 검색하세요!');	// MDer tBody 설정
		$("#prodCd").focus();
	}
	...
	function doSearch(){
		...
		if(!$.trim($("#prodCd").val()) && !$.trim($("#prodNm").val()) && !$.trim($("#srcmkCd").val()) ){
			alert("상품코드 또는 상품명 또는 판매코드 중 하나이상의 검색조건을 입력하세요!");
			return false;
		}
		...
		if($.trim($("#srcmkCd").val()) && $.trim($("#srcmkCd").val()).length < 5) {
			alert("판매코드를 이용하여 조회할 경우 5자이상  검색조건 입력하세요!");
			$("#srcmkCd").focus();
			return false;
		}
		
		if($.trim($("#prodCd").val()) && !isNumber($("#prodCd").val()) ){
			alert("정상적인 상품코드 입력하세요!");
			$("#prodCd").val('');
			$("#prodCd").focus();
			return false;
		}
		...
		$.post(
				"<c:url value='/edi/weborder/PEDMCOM0009SelectVenProd.do'/>",
				JSON.stringify(ven),
				function(data){
