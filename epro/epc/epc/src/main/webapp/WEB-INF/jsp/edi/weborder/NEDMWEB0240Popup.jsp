...
<script  type="text/javascript" >
	...
	function pageOnloadInit(){
		...
		var opener= parent.opener.getDialogArgumentsInner();		// 넘어온 데이터 받아서 setting
	...
	function _eventCopyAndMoveEmpPool(types){
		...
		var typeName = "<spring:message code='text.web.field.copy' />";
		if(types == "M")  typeName = "<spring:message code='text.web.field.modify' />";
		...
		$.post(
				"<c:url value='/edi/weborder/NEDMWEB0240MoveAndCopyEmplPool.json'/>",
				JSON.stringify(str),
				function(data){
					...
					if(data == null || data.state !="0"){
						...
						if(data.state =="1") 
						{
							alert("<spring:message code='msg.web.error.noInfo' arguments='"+typeName+","+data.state+"'/>");
						}
						else 
						{
							alert("<spring:message code='msg.web.error.empProcError' arguments='"+typeName+","+data.state+"'/>");
							hideLoadingMask();
							return;
						}
						hideLoadingMask();
					...
					{
						alert('<spring:message code="msg.web.error.saveEmpNo" arguments="' + typeName + '"/>');
						setClearData(true);
					}
					hideLoadingMask();
				},	"json"
	...
	function doCheckVal(type){
		...
		var typeName = "<spring:message code='text.web.field.copy' />";
		if(type == "M")  typeName = "<spring:message code='text.web.field.modify' />";
		
		if(!frEmpNo){
			alert('<spring:message code="msg.web.error.emptyEmpNo" arguments="' + typeName + '"/>');
			return false;
			self.close();
		}
		
		if(!toEmpNo){
			alert('<spring:message code="msg.web.error.applyEmpNo" arguments="' + typeName + '"/>');
			$("#empNo_search").focus();
			return false;
		}
		return true;
	...
	function _eventSearchEmpPool(){
		...
		<%-- 조회조건 검증 ============================= --%>
		/* 사번 체계 확인 후 사용 가능한 검증
			문자와 숫자로 된 데이터 및 길이가 9자리가 안되는 애들이 있음.
			if(!$.trim(toEmpNo) || toEmpNo.length != 9 ){
			alert("<spring:message code='msg.web.error.empNoMin9'/>");
			$("#empNo_search").focus();
			return false;
		} */
		
		if( $.trim($("#empNo_search").val()) == "" ){
		...
		if(toEmpNo == frEmpNo) {
			alert("<spring:message code='msg.web.error.neEmpNo'/>");
			$("#empNo_search").val('');
			$("#empNo_search").focus();
			return false;
		}
...
<div id="popup">
			...
			<div id="p_title1">
				<h1><spring:message code="text.web.field.empModCopy"/> </h1>
			...
			<div class="popup_contents">
				...
				<div class="bbs_list" style="margin-top: 10px;">
					<ul class="tit">
						<li class="tit"><spring:message code="text.web.field.empModCopyMng"/></li>
						<li class="btn">
							<a href="#" class="btn" id="btnChangeEmpPool"><span><spring:message code="text.web.field.empModify"/></span></a>
							<a href="#" class="btn" id="btnCopyEmpPool"  ><span><spring:message code="text.web.field.empCopy"/></span></a>
						</li>
					</ul>
					<table class="bbs_search" >
						...
						<tr>
							<th><spring:message code="text.web.field.daeEmpNo"/></th>
							<td><span id="fromEmpNo"></span></td>
						</tr>
						<tr height="30">
							<th><spring:message code="text.web.field.applyEmpNoSearch"/></th>
							<td>
								...
								<a href="#" class="btn" id="btnSearchEmpPool" ><span><spring:message code="button.common.search"/></span></a>
							</td>
						</tr>
					</table>
				</div>
				<div class=bbs_search style="margin-top: 2px;">
					<ul class="tit">
						<li class="tit"><spring:message code="text.web.field.applyEmpNoResult"/></li>
						<li class="btn"></li>
					</ul>
					<table class="bbs_search" >
						<tr>
							<td style="text-align: center;"><spring:message code="text.web.field.name"/></td>
							<td style="background-color: #ffffff;">
								<span id="empNm_view" style="color: #5c66c4;"></span>
							</td>
							<td style="text-align: center;"><spring:message code="text.web.field.empNo"/></td>
							<td style="background-color: #ffffff;">
								<span id="empNo_view" style="color: #5c66c4;"></span>
							</td>
						</tr>
						<tr>
							<td style="text-align: center;"><spring:message code="text.web.field.deptNm"/></td>
							<td colspan=3 style="background-color: #ffffff;">
								<span id="orgNm_view" style="color: #5c66c4"></span>
							</td>
						</tr>
						<tr>	
							<td style="text-align: center;"><spring:message code="text.web.field.jobNm"/></td>
							<td colspan=3 style="background-color: #ffffff;">
