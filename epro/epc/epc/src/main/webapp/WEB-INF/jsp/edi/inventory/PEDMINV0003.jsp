...
<script>
	...
	function doSearch() {
		...
		if(dateValid(form)){
			loadingMaskFixPos();
			form.action  = "<c:url value='/edi/inventory/PEDMINV0003Select.do'/>";
			form.submit();	
		}	
	...
	function popupSearch(tbName1, tbName2){		
		...
		form.staticTableBodyValue.value = "<CAPTION>재고정보 센터점출입 현황표<br>"+
	...
	function printText(){
		...
		if(dateValid(form)){
			...
			form.action  = "<c:url value='/edi/inventory/PEDMINV0003Text.do'/>";
			form.submit();	
					...
					<c:if test="${not empty inventoryList }">
						<c:forEach items="${inventoryList}" var="list" varStatus="index" >
							<tr class="r1">
								...
								<td align="center">${list.stroQty }</td>
								<td align="center">${list.stroSaleAmt }</td>
								<td align="center">${list.striQty }</td>
								<td align="center">${list.striSaleAmt }</td>
								<td align="center">${list.strioQty }</td>
								<td align="center">${list.strioSaleAmt }</td>
							</tr>
						</c:forEach>
					</c:if>
