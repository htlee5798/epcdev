...
/* 			var srchEntpCd = "<c:out value='${param.srchEntpCd}'/>";  //검색조건 협력업체코드
		...
		function eventTempleteL1List2(teamL1CdList) {
			var paramInfo = {};
			paramInfo.teamL1CdList = teamL1CdList;
			...
			$.ajax({
				...
				dataType: 'json',
				async: false,
				url: '<c:url value="/edi/product/selectTempL1List2.json"/>',
				data: JSON.stringify(paramInfo),
				success: function(data) {
					var resultList = data.l1List;

					// 각 rnum에 맞는 select box에 값 채우기
					for (var j = 0; j < teamL1CdList.length; j++) {
						var groupCode = teamL1CdList[j].groupCode;
						var selectedL1Cd = teamL1CdList[j].l1Cd;
						var rnum = teamL1CdList[j].rnum;

						var eleHtml = [];

						for (var i = 0; i < resultList.length; i++) {
							if (resultList[i].teamCd == groupCode) {
								if (resultList[i].l1Cd == selectedL1Cd) {
									eleHtml.push("<option value='" + resultList[i].l1Cd + "' selected='selected'>" + resultList[i].l1Nm + "</option>");
								} else {
						...
						$("#l1Cd" + rnum + " option").not("[value='']").remove(); // 기존 옵션 제거
						$("#l1Cd" + rnum).append(eleHtml.join(''));
					}
				}
			});
...
/* 		            var checkedValues = data[i].sellChanCd;
		         ...
		         //   ajax 지금은 여러번 타는데 한번만 태우고싶으면 아래 주석 해제 
		          //  	l1SetList.groupCode = data[i].teamCd;
		          //  	l1SetList.l1Cd = data[i].l1Cd;
		          //  	l1SetList.rnum = data[i].rnum;
		          //  	l1SetList.prdtReqYn = data[i].prdtReqYn;
		          //  	l1SetList.prdtStatus = data[i].prdtStatus;
		            	
		            	
		         //   	l1TeamList.push(l1SetList);
		      			eventTempleteL1List(data[i].teamCd,data[i].l1Cd,data[i].rnum,data[i].prdtReqYn,data[i].prdtStatus);
		            }
		            
		        }
		        // 조회된 팀의 대분류 조회 한번에 다건 조회 
		//        eventTempleteL1List2(l1TeamList);
		    } else {
