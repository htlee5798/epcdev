...
<%-- 계열사별 상품조회 --%>
const srchProdMart 		= "<c:out value='${param.srchProdMart}'		default=''/>";
const srchProdMtAll	= "<c:out value='${param.srchProdMtAll}'	default=''/>";
const srchProdSuper		= "<c:out value='${param.srchProdSuper}'	default=''/>";
const srchProdMaxx		= "<c:out value='${param.srchProdMaxx}'		default=''/>";
const srchProdCfc		= "<c:out value='${param.srchProdCfc}'		default=''/>";
...
$(document).ready(function(){
  ...
  //----- 팀 변경시 이벤트
	...
	// 파트너사 코드
	let entpCd = "${param.entpCd}";
	if(entpCd && entpCd.trim() != '' ){
		$("#dataForm select[name=srchEntpCd]").attr('disabled', true);
	}
	
	// 팀코드
	let teamCd = "${param.teamCd}";
	if(teamCd && teamCd.trim() != '' ){
		$("#dataForm select[name=srchTeamCd]").val(teamCd).trigger('change');
		$("#dataForm select[name=srchTeamCd]").attr('disabled', true);
	}
...
function goPage(pageIndex) {
	...
	<%-- 계열사별 상품조회 조건 추가 --%>
	searchInfo.srchProdMart 	= srchProdMart;
	searchInfo.srchProdMtAll	= srchProdMtAll;
	searchInfo.srchProdSuper	= srchProdSuper;
	searchInfo.srchProdMaxx		= srchProdMaxx;
	searchInfo.srchProdCfc		= srchProdCfc;
...
function selectProdDisplay(obj) { 
	...
	<script id="dataListProdTemplate" type="text/x-jquery-tmpl">
		<tr class="r1" bgcolor=ffffff>
			...
			<input type="hidden" name="orgCostKr02" value="<c:out value='\${orgCostKr02}'/>"/>
			<input type="hidden" name="orgCostKr03" value="<c:out value='\${orgCostKr03}'/>"/>
			<input type="hidden" name="orgCostKr04" value="<c:out value='\${orgCostKr04}'/>"/>
			<input type="hidden" name="orgCostKr09" value="<c:out value='\${orgCostKr09}'/>"/>
			<input type="hidden" name="prodPatFg" value="<c:out value='\${prodPatFg}'/>"/>
			<input type="hidden" name="prodTypFg" value="<c:out value='\${prodTypFg}'/>"/>
			...
			<input type="hidden" name="taxFg" value="<c:out value='\${taxFg}'/>"/>
			<td align="center">
				<c:out value="\${rnum}"/>
			</td>
			<td align="center">
			...
			<td align="center">
				<a href="javascript:void(0);" onclick="selectProdDisplay(this);" class="tooltip" data-tooltip-content="#tt_\${prodCd}">
					 <c:out value="\${prodCd}" />
				</a>
				<div class="tooltip_templates" style="display:none">
					<div id="tt_\${prodCd}">
						기본단위: <c:out value='\${ordUnit}'/> &nbsp;&nbsp; 과세유형: <c:out value='\${taxFgNm}'/> &nbsp;&nbsp; 신선(규격)여부 : <c:out value='\${freshStdYn}'/>
						<br/>
						[정상원가] <c:out value='\${orgCostTit}'/>
					</div>
				</div>
			</td>
			<td align="center">
	...
	<script id="dataListTemplate" type="text/x-jquery-tmpl">
		<tr class="r1" bgcolor=ffffff>
			...
			<td align="center">
    			<a href="javascript:void(0);" onclick="selectSellCdDisplay(this);" class="tooltip" data-tooltip-content="#tt_\${prodCd}">
        			<c:out value="\${srcmkCd}" />
    			</a>
				<div class="tooltip_templates" style="display:none">
					<div id="tt_\${prodCd}">
...
<input type="hidden" id="srchFreshStdYn" name="srchFreshStdYn" value="<c:out value='${param.freshStdYn}'/>"/>	<%-- 신선_규격상품구분 --%>
<input type="hidden" id="srchPurDept" name="srchPurDept" value="<c:out value='${param.srchPurDept}'/>"/>		<%-- 구매조직 --%>
<input type="hidden" id="srchChgOrgCostYn" name="srchChgOrgCostYn" value="<c:out value='${param.srchChgOrgCostYn}'/>"/>		<%-- 원매가변경가능상품여부 --%>
<div id="popup">
    <!------------------------------------------------------------------ -->
    <!--    title -->
    <!------------------------------------------------------------------ -->
		...
		<div class="bbs_search2" style="width:100%;">
	        ...
	        <table class="bbs_search" cellpadding="0" cellspacing="0" border="0">
	            ...
	            <tr>
	            	<th>파트너사</th>
	            	<td>
	            		<html:codeTag objId="srchEntpCd" objName="srchEntpCd"  width="150px;" dataType="CP" comType="SELECT" formName="form" defName="선택" selectParam="${param.entpCd}"/>
					</td>
					<th>상품명</th>
	            	<td>
