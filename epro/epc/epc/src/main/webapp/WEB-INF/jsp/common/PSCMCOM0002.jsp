					...
					<c:forEach var="list" items="${list}">
						...
						<c:choose>
							<c:when test="${searchVO.gubn eq 'CONO' }">
								<td align="center"><a href="javascript: popupPushValue2('<c:out value="${list.VENDOR_ID}"/>','<c:out value="${list.VENDOR_NM}"/>','<c:out value="${list.PARAM_CONO}"/>');"><c:out value='${list.VENDOR_NM}' /></a></td>
							</c:when>
							<c:otherwise>
