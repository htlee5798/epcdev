...
function popupSearch(tbName1, tbName2, tbName3){
	...
	form.staticTableBodyValue.value = "<CAPTION>매출정보 상품상세별 현황표<br>"+
...
function printText(){
	...
	if(dateValid(form)){
		form.action = "<c:url value='/order/viewSaleDetailListText.do'/>";
		form.submit();
	}
}
</script>
			...
			<div class="bbs_list">
				...
				<c:if test="${not empty saleList }">
					...
					<tr>
						...
						<th align="right"><fmt:formatNumber value="${total_sell_amt }" type="number" currencySymbol="" /></th>
						<c:set var="total_qty" value="${0 }" />
						<c:set var="total_amt" value="${0 }" />
						<c:set var="total_sell_amt" value="${0 }" />
					</tr>
					...
					<tr class="r1">
						...
						<c:set var="total_sell_amt_sum" value="${total_sell_amt_sum + list.totSellAmt }" />
				...
				<c:if test="${empty saleList }">
					<tr><td colspan="8" align="center">${resultMsg }</td></tr>
				</c:if>
				</table>
				</div>
				<table class="bbs_list" cellpadding="0" cellspacing="0" border="0" id="testTable3">
					...
					<tr class="r1">
						...
						<th align="right"><b><fmt:formatNumber value="${total_sell_amt_sum }" type="number" currencySymbol="" /></b></th>
						<th>&nbsp;</th>
					</tr>
					</c:if>
