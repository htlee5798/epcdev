...
<script>
	...
	function doSearch() {
		...
		function _setTbodyMasterValue(json) { 
			...
			if (data.length > 0) {
				...
				//[PIY] 생략한 정보를 제외 한 나머지 정보를 옮길 객체
				var tmpData = [0];
				tmpData.pop();
				...
				for (var i = 0; i < data.length; i++) {
					...
					realPayBuy_sum		 += parseInt(Number(realPayBuy));
					...
					data[i]["occationSubPay"]  = setComma(occation_sub_pay);
					...
					if(data[i]["strCd"] === "210")continue;
					tmpData.push(data[i]);
				}
				$("#dataListTemplate").tmpl(tmpData).appendTo("#dataListbody"); // setComma(n)
	...
	function popupSearch(tbName1,tbName2, tbName3){		
		...
		var bodyValue = "";
		bodyValue += "<CAPTION><spring:message code='epc.pay.dash.perStorePayment'/><br>";
		bodyValue += "[<spring:message code='epc.ord.period'/> : " + date + "]";
		bodyValue += "[<spring:message code='epc.ord.store'/> : " + storeNm + "]";
	...
	function forwarding(val) {
		...
		$("#searchForm").attr("action", "<c:url value='/edi/payment/NEDMPAY0020.do'/>");
		$("#searchForm").submit();
	}
	...
	function printText(){
		...
		textData += "[<spring:message code='epc.ord.store'/> : " + storeNm + "]";
		...
		$("#searchForm").attr("action", "<c:url value='/edi/payment/NEDMPAY0030Text.do'/>");
		$("#searchForm").submit();
...
<tr class="r1">
	...
	<td rowspan="3" align="center"><c:out value="\${strNm}"/></td>
...
<body>
	...
	<div>
		...
		<input type="hidden" id="serchEntpCd" name="searchEntpCd" />
		...
		<div id="wrap_menu" style="width:1000px">
			...
			<div>
				<br><span>&nbsp&nbsp▶지급대상액 = 공급가액 + 세액</span><br> &nbsp&nbsp* 세금계산서 4일 이후 승인시 세액 익월 지급(10일 18시 이후 승인 불가)</span>
			</div>
				...
				<div class="bbs_list">
					...
					<colgroup>
						...
						<tr style="display:none;" class="r1 sum">
							<th rowspan="3" colspan="2" align="center"><b><spring:message code='epc.pay.header.totalSum'/></b></th>
		...
		<div id="footbox">
			...
			<div class="location">
				<ul>
					...
					<li class="last"><spring:message code='epc.pay.perStore'/></li>
				</ul>
			</div>
		</div>
	</div>
