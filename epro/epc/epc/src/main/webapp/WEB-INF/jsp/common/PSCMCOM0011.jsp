...
<body>
				...
				<div class="bbs_search3">
					...
					<table class="bbs_grid2" cellspacing="0" border="0">
						...
						<tr>
							<th>주문일자</th>
							<td>
								<input type="text" name="startDate" value="<c:out value='${startDate}' />" id="startDate" class="day" readonly style="width: 30%;"/>
							  ...
							  ~ <input type="text" name="endDate" value="<c:out value='${endDate}' />" id="endDate" class="day" readonly style="width: 30%;"/>
	...
	<c:forEach var="list" items="${list}">
		...
		<c:when test="${list.ROWSPAN > 1}">
			...
			<c:if test="${list.ORDER_ID != T_ORDER_ID}">					
								<td rowspan="<c:out value='${list.ROWSPAN}' />">
									<a href="javascript:setOrderId('<c:out value="${list.ORDER_ID}" />','<c:out value="${list.PROD_NM}" />')"><c:out value="${list.ORDER_ID}" /></a>
								</td>
			</c:if>				
		...
		<c:otherwise>
							<tr class="r1">
								<td><a href="javascript:setOrderId('<c:out value="${list.ORDER_ID}" />','<c:out value="${list.PROD_NM}" />')"><c:out value="${list.ORDER_ID}" /></a></td>
								<td style='text-align: center'><c:out value="${list.CUST_NM}" /></td>	
								<td style='text-align: center'><c:out value="${list.RECP_PSN_NM}" /></td>								
								<td style='text-align: left'><c:out value="${list.PROD_NM}" /></td>
