...
<body>
	<div id="popup">
		...
		<form name="offlineImageForm" id="offlineImageForm" method="POST" action="<c:url value='/edi/product/insertNewProdImgPop.do'/>"	 enctype="multipart/form-data">			
			...
			<input type="hidden" name="trdTypeDivnCd"		id="trdTypeDivnCd"		value="<c:out value='${param.trdTypeDivnCd}'/>"	/>
			<input type="hidden" name="srcmkCd"				id="srcmkCd"			value="<c:out value='${param.srcmkCd}'/>"		/>
			<input type="hidden" name="proxyNm"  			id="proxyNm"			value="MST0820"	/>										<!-- RFC CALL 명 -->
			<input type="hidden" name="cfmFg"  				id="cfmFg"				value="1"		/>										<!-- 오프라인 이미지 등록시 이미지 확정상태값은 1:심사중으로 준다 -->
			<input type="hidden" name="prodCd"  			id="prodCd"				value="<c:out value='${param.prodCd}'/>"			/>	<!-- 상품으로 확정이 되고 난 이후의 REG_SAP 테이블의 상품코드 -->
			<input type="hidden" name="fixSrcmkCd" 			id="fixSrcmkCd"			value="<c:out value='${param.fixSrcmkCd}'/>"		/>	<!-- 상품으로 확정이 되고 난 이후의 REG_SAP 테이블의 판매코드 -->
