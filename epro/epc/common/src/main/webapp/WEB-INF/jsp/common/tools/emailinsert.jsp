...
<%
 /**
  * @Class Name : email.jsp
  * @Description : email insert test
  * @Modification Information
  * @
  * @  수정일         수정자                   수정내용
  * @ -------    --------    ---------------------------
  * @ 2011.09.27    고성훈          최초 생성
  *
  *  @author 공통서비스 AA 고성훈
  *  @since 2011.09.27
  *  @version 1.0
...
<c:set var="registerFlag" value="${empty LteMailVO.MID ? '등록' : '수정'}"/>
<title>Sample <c:out value="${registerFlag}"/> </title>

<!--For Commons Validator Client Side-->
<script type="text/javascript" src="<c:url value='/validator.do'/>"></script>
<validator:javascript formName="lteMailVO" staticJavascript="false" xhtml="true" cdata="false"/>
...
function fn_save() {	
	...
	if(!validateLtsmsVO(frm)){
    	...
    	frm.action = "<c:url value="${registerFlag == '등록' ? '/sample/email/insert.do' : '/sample/email/update.do'}"/>";
        frm.submit();
    }
}
...
<form:form commandName="lteMailVO" name="detailForm">
<div>
	<table width="100%" border="1" cellpadding="0" cellspacing="0" bordercolor="#D3E2EC" bordercolordark="#FFFFFF" style="BORDER-TOP:#C2D0DB 2px solid; BORDER-LEFT:#ffffff 1px solid; BORDER-RIGHT:#ffffff 1px solid; BORDER-BOTTOM:#C2D0DB 1px solid; border-collapse: collapse;">
		...
		<tr>
			...
			<td class="tbtd_content">
				<form:input path="MID" cssClass="essentiality" maxlength="15" readonly="true" />
			</td>			
		</tr>
		</c:if>
		<tr>
			<td>발송자 명</td>
			<td>
				<form:input path="RNAME" maxlength="30"/>
		...
		<tr>
			<td>받는사람 주소</td>
			<td>
				<form:input path="RMAIL" maxlength="10"/>
			</td>
		</tr>
	</table>
	<div>
