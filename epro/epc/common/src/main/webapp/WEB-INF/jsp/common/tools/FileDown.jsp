...
<c:forEach var="fileVO" items="${fileList}" varStatus="status">
	<tr>        	
		...
		<c:choose>
		   <c:when test="${fileVO.fileExtsn == 'jpg'}">
					<td><img src='<c:url value='/files/getImage.do'/>?atchFileId=<c:out value="${fileVO.atchFileId}"/>&fileSn=<c:out value="${fileVO.fileSn}"/>'  width="640" /></td>
			</c:when>
			<c:otherwise>
				<td>&nbsp;</td>
			</c:otherwise>
