...
function DQmainSearch_submit() {
	...
	if (searchTerm.length < 1) {
		location.href=$('#search_link_url').val();
		return false;
	}
...
function changeType(obj){
	var $obj = $(obj),
	$tgt = $('#tgtGoodsTrans'), //div
	objCls = $obj.attr('class'); // button

	var _label = '';
	if(objCls == 'type-image'){	// 리스트일 경우 클릭 -> 2단 갤러리 형
		document.pageNavigator.viewType.value = "G";
		$obj.attr('class', 'type-list');
		$tgt.attr('class', 'type-list');
		_label = '목록형';
	}else if(objCls == 'type-list'){	// 2단 갤러리 형일 경우 클릭 -> 갤러리형
		document.pageNavigator.viewType.value = "T";
		$obj.attr('class', 'type-gallery');
		$tgt.attr('class', 'type-gallery');
		_label = '갤러리형';
	}else if(objCls == 'type-gallery'){	// 갤러리형일 경우 클릭 -> 리스트
		document.pageNavigator.viewType.value = "gallery";
		$obj.attr('class', 'type-image');
		$tgt.attr('class', 'type-image');
		_label = '이미지형';
	}
