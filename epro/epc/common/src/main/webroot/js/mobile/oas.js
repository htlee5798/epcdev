 ...
 * 검색조건명 취득
 ******************************************************** */
function fn_get_conditionNm(condition) {
 ...
 * 일반 날씨 판정
 ******************************************************** */
function fn_judge_weather(weatherStatusNumber, cloudAmouant) {
	var icon = "";
	var desc = "";

  	if (weatherStatusNumber == "" || weatherStatusNumber < 5) {
  		icon = fn_judge_cloudAmouant(cloudAmouant);
  	}
 ...
 ******************************************************** */
function fn_judge_cloudAmouant(cloudAmouant) { 
	icon = "";
 ...
 ******************************************************** */
function fn_get_today(dateFormat) {
	var date = new Date();
	...
	for (var i = 0; i < dateFormat.length; i++) {
		if (dateFormat.substr(i, 4) == 'yyyy') {
			...
			dateFormat = dateFormat.substr(0, i) + fn_fill_char(date.getMonth() + 1, 2, "0") + dateFormat.substr(i + 2, dateFormat.length);
			i += 2;
			...
			dateFormat = dateFormat.substr(0, i) + fn_fill_char(date.getDate(), 2, "0") + dateFormat.substr(i + 2, dateFormat.length);
			i += 2;
 ...
 * c : 채울 문자
 ******************************************************** */
function fn_fill_char(orgStr, n, c) {
	var fillChar = "";
