	...
	function fn_offlineList() {
		var uri = contextPath + "/mobile/ows/selectMobileOfflineWebList.mdo"; 
		// url        : "/mobile/ows/selectMobileOfflineWebList.mdo",
		$.ajax({
	        type       : "POST",
	        cache      : false,                   
	        url        : uri,
	...
	function fn_goOfflineWebView(offlineSn) {		
		...
		if(window.navigator.onLine == true) {
			// 1. 오프라인 서비스 상세조회 					
			// location.href = "<c:url value='/mobile/ows/goMobileOfflineWeb.mdo?sn="+offlineSn+"'/>";
			location.href = contextPath + "/mobile/ows/goMobileOfflineWeb.mdo?sn="+offlineSn
	        ...
	        // location.href = "<c:url value='/mobile/ows/goMobileOfflineWeb.mdo'/>";
	        location.href = contextPath + "/mobile/ows/goMobileOfflineWeb.mdo";
		}
	};	