	...
	******************************************************** */			
	 ...
	 ******************************************************** */			
	function fn_rnsList() {
		 var uri = contextPath + "/mobile/rns/selectRealtimeNoticeList.mdo"; 
		 $.ajax({
			 type       : "POST",
			 cache      : false,				      
			 url        : uri,
			 ...
			 success :function(json) {
				...
				for(var k=0; k<rnsList.length; k++) {
					...
					var title = rnsList[k].noticeSj;
					var regDate = rnsList[k].creatDt;
					var searchSn = rnsList[k].searchSn;
				
					
					html += "<li><a href=javascript:fn_goRnsView('"+searchSn+"')><h3>"+title+"</h3><p>"+regDate+"</p></a></li>";
	...
	function fn_goRnsInsert() {
	      ...
	      form.action = contextPath + "/mobile/rns/goRealtimeNoticeInsert.mdo";
	      form.submit();  
	};
	...
	function fn_goRnsView(noticeSn) {
	    ...
	    form.action = contextPath + "/mobile/rns/goRealtimeNotice.mdo";
	    form.submit();  
	};
	 ...
	 ******************************************************** */
	function fn_view(sn) {
		var uri = contextPath + "/mobile/rns/selectRealtimeNotice.mdo"; 
		$.ajax({
			type       : "POST",
			cache      : false,				      
			url        : uri,
		...
		jConfirm('', '삭제 하시겠습니까?', 'b', function(r) {
			if(r == true) {
				form = document.searchVO;
			  	form.sn.value = sn;
		      	form.action = contextPath + "/mobile/rns/deleteRealtimeNotice.mdo";
		      	form.submit();  
			}
		});
	};
	...
	function fn_insert() {
		if(fn_validation()) {
			var uri = contextPath + "/mobile/rns/insertRealtimeNotice.mdo";
			var noticeSj = $("#noticeSj").val();
			var noticeCn = $("#noticeCn").val();
			var noticeBgnDt = $("#noticeBgnDt").val();
			var noticeEndDt = $("#noticeEndDt").val();
			...
			$.ajax({
				...
				success :function(json) {
					// fn_goList(); 
					location.href = contextPath + "/mobile/rns/goRealtimeNoticeList.mdo";
				} 
			});
		}
	};
	...
	function fn_serverSentEvent() {
		eventURI = contextPath + "/mobile/rns/goEvent.mdo";	
