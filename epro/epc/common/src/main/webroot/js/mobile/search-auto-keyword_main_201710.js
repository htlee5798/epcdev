...
//			document.getElementById('searchDelete').style.display="block";
			...
			$( '.wrap-default-app' ).addClass( 'hidden' );
			$( '.search-header' ).addClass( 'layer-open-type1').removeClass( 'layer-open-type2' );
			$('.search-cancel').text("닫기");	//닫기버튼추가
			$('.search-header').removeClass('no-search-cancel');
//			$( '#quickmenu-bar' ).addClass( 'enable' );
//			schemeUrl = schemeUrl + 'showBar';

		} else {
			$( '.search-header' ).addClass( 'layer-open-type2' ).removeClass( 'layer-open-type1' );
			$( '#searchDelete' ).addClass( 'hidden' );
			$( '.wrap-default-app' ).removeClass( 'hidden' );
...
//			schemeUrl = schemeUrl + 'hideBar';
		...
		if((document.getElementById('searchTerm').value == "") &&($( '.search-header' ).find('.text-list-wrap').css('display')=='none')){
			//alert("201710");
			$('.search-cancel').text("닫기");	//닫기버튼추가
			$('.search-header').removeClass('no-search-cancel');
		}
