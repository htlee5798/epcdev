...
$(window).resize(function(){
	...
	// 차트/그래프 그리기
	fn_draw_linechartgraph();
	fn_draw_columnchartgraph();
	fn_draw_piechartgraph();
	fn_draw_scatterchartgraph();
	fn_draw_barchartgraph();
});

/*********************************************************
 * 초기화
 ******************************************************** */
function fn_initl_mobilechartgraphlist(){
	
	$.getJSON(contextPath + "/mobile/mcg/selectMobileChartGraphList.mdo", function(json) {
 ...
 * Line 차트 그리기
 ******************************************************** */
function fn_draw_linechartgraph(){
	var canvas;
	var context;
	...
	fn_draw_scalemark(context, scale_rect, x_gradations, y_gradations, false, true);
	...
	// 범례 그리기
	fn_draw_legend(context, pie_data);
}

/*********************************************************
 * Column 차트 그리기
 ******************************************************** */
function fn_draw_columnchartgraph(){
	var canvas;
	var context;
	var scale_rect = [50*ratio, 50*ratio, 300*ratio, 300*ratio];// 위치(X, Y좌표) 및 사이즈(Width, Height)
	var x_value_interval = 1;									// X축 눈금 값의 간격 (반영안됨)
 ...
 * Pie 차트 그리기
 ******************************************************** */
function fn_draw_piechartgraph(){
	var canvas;
	var context;
 ...
 * Scatter 차트 그리기
 ******************************************************** */
function fn_draw_scatterchartgraph(){
	var canvas;
	var context;
	var scale_rect = [50*ratio, 50*ratio, 300*ratio, 300*ratio];	// 위치(X, Y좌표) 및 사이즈(Width, Height)
	...
	fn_draw_scalemark(context, scale_rect, x_gradations, y_gradations, true, true);
	...
	// 범례 그리기
	fn_draw_legend(context, scatter_data);
}

/*********************************************************
 * Bar 차트 그리기
 ******************************************************** */
function fn_draw_barchartgraph(){
	var canvas;
	var context;
	var scale_rect = [50*ratio, 50*ratio, 300*ratio, 300*ratio];	// 위치(X, Y좌표) 및 사이즈(Width, Height)
	...
	fn_draw_scalemark(context, scale_rect, x_gradations, y_gradations, true, false);
	...
	// 범례 그리기
	fn_draw_legend(context, bar_data);
}

/*********************************************************
 * 눈금선 그리기
 ******************************************************** */
function fn_draw_scalemark(context, scale_rect, x_gradations, y_gradations, x_line, y_line){
 ...
 * 범례 그리기
 ******************************************************** */
function fn_draw_legend(context, data){
