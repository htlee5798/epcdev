(function (root, factory) {
    ...
    var _init = function (obj) {
        ...
        if(window.getHistoryBack && window.getHistoryBack()) {
            window.setHistoryBack(false);
          	...
          	if($('[name=categoryByPriceTab].active').data('categoryId') != ''){
         		showProductsByPrice($('[name=categoryByPriceTab].active'));        		
         	}
         	if($('[name=bestCategoryTab].active').data('categoryId') != ''){
         		showProductsByCategory($('[name=bestCategoryTab].active'));        		
         	}
         	if($('[name=categoryBySpecialTab].active').data('categoryId') != ''){
         		showProductsBySpecial($('[name=categoryBySpecialTab].active'));        		
         	}
         	if($('[name=categoryByBigTab].active').data('categoryId') != ''){
         		showProductsByBig($('[name=categoryByBigTab].active'));        		
         	}
         	if($('[name=recommend].active').data('categoryId') != ''){
         		showRecommendProducts($('[name=recommend].active'));        		
         	} 
         	
			if($('[name=hotFlag]').length == 0){
				if($('[name=otherPriceBnr].active').data('categoryId') != ''){
					showotherPrice($('[name=otherPriceBnr].active'));        		
				}
				$("[name='planContainer'].active").each(function (i) {
					showPlanProducts($('[name=planContainer].active').eq(i));		//한가위장보기
				});
			}
    ...
    function setHoliMainPosition() {
        ...
        var $this = $holiMainInfotab.find('article[data-index='+nDataIndex+']'),
    ...
    function showPlanProducts($tab) {
        var $planContainer = $('#'+$tab.data('listContainer'));
        var data = {
                apiName : 'getPlanProdList',
                CATEGORY_ID : $tab ? $tab.data('categoryId') : '',
                ROW_SIZE : 6,
                mkdpSeq : $tab.data('mkdpSeq'),
                divnSeq : $tab.data('divnSeq'),
                dispCd : '01',
                returnPagePath : 'mobile/holidays/includes/planProductList',
                sortType : 'DISPLAY_SEQ_ASC',
            },
            templateParams = {
                successCallback : function(productListHtml, params){
                    $planContainer.append(productListHtml);
                    var planUrl = '/mobile/plan/planDetail.do?CategoryID='+$tab.data('categoryId')+'&MkdpSeq='+$tab.data('mkdpSeq')+'&pdivnSeq='+$tab.data('divnSeq');
                    $planContainer.find('.holi-ny-shopping-more-btn').attr("href", planUrl);
                },
                afterRender : function(params){
                    $.ui.helper.activeProductsWithTab($tab, params.getContainer());
                },
                siteLocation : $tab.data('siteLocation'),
                categoryListUrl : 'plan/product/listPage',
                getContainer : function(){
                    return $planContainer.find('[name=productPanel][data-category-id=' + $tab.data('categoryId') + ']');
                }
            };

        $.ui.helper.showProductsByTab(data, templateParams);
    }
