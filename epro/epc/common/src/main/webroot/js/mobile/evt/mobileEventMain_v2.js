(function(root, factory) {
	...
	var _init = function(obj) {
		...
		$container
			...
			.on('click', 'button[name=loadMcouponNew]', loadMcouponNew)
		...
		$.ajax({
			...
			success: function(data) {
				...
				if (data.rtnCode == 'Done') {
					$container
						.find('.couponMessage')
						.html('쿠폰을 이미 등록하셨습니다.<br><span class="point">‘마이롯데 > 쿠폰’</span>에서 확인하세요.');
					$container.find('.acenter').show();
					insertCouponLayer();

					ga('send', 'event', {
						eventCategory:
							'#몰구분=Mart #기기=M #분류=컨텐츠전시페이지(메인GNB) #페이지명=이벤트/쿠폰 #URL=' + requestUri,
						eventAction: '혜택/참여컨텐츠 보기',
						eventLabel: '#타입=쿠폰 #고객액션=쿠폰등록실패'
					});
					...
					$container
						.find('.couponMessage')
						.html('쿠폰이 등록 완료되었습니다.<br><span class="point">‘마이롯데 > 쿠폰’</span>에서 확인하세요.');
					$container.find('.acenter').show();
					insertCouponLayer();
	...
	//M쿠폰 상세 이동
	function goMCouponDetail() {
		location.href =
			$.utils.config('LMAppUrlM') + '/mobile/evt/mobileEventMCouponDetail.do?couponId=' + $(this).data('couponid');
	}

	function goCouponLogin() {
		if (!global.isLogin($.utils.config('LMAppUrlM') + '/mobile/evt/eventMain.do?tabId=1')) {
			return;
		}
	}

	function goMcouponLogin() {
		if (!global.isLogin($.utils.config('LMAppUrlM') + '/mobile/evt/eventMain.do?tabId=2')) {
			return;
		}
	}
	...
	function appDown() {
		...
		if (
			userAgent.toUpperCase().indexOf('LOTTEMART-APP-SHOPPING-ANDROID') !== -1 ||
			userAgent.toUpperCase().indexOf('TOYSRUS.ANDROID.SHOPPING') !== -1
		) {
			location.replace('lmscp://m.coupon.lottemart');
		} else {
			if (userAgent.toUpperCase().indexOf('IPHONE') !== -1 || userAgent.toUpperCase().indexOf('IPAD') !== -1) {
	...
	function loadMcouponNew() {
		var pUrl = '';
		pUrl = $.utils.config('LMAppUrlM') + '/mobile/evt/ajax/mobileEventMcouponDownload.do';
...
//							break;
//						case "notScpMember":
//							alert("M쿠폰 회원가입을 확인해주세요. ");
//							break;
//						default:
//							alert("M쿠폰 불러오기를 확인해 주세요. ");
//							break;
//					}
					alert('M쿠폰 회원가입 후 다시 확인해 주세요.');
	...
	function openlayerForMcoupon(e) {
		// tooltip 변경
		e.preventDefault();
		var obj = $(this),
