...
function ShowEventStampList() {
	...
	console.log("jsonStampList ::: " + jsonStampList);
	...
	for (var i = 0; i < jsonStampList.length; i++) {
		...
		stampList = '<li>' +
			...
			if (jsonStampList[i].grp_aply_yn == "Y"){
				stampList  +=	 '<em class="tag-stamp-type tag-type2">승인대기</em>';
			}	

			// 참여 여부 
			if (jsonStampList[i].grp_ptc_yn == "Y"){
				stampList  += '<em class="tag-stamp-type">참여중</em>';
			}	
			stampList  += '<header class="header-info">'+
					'<strong class="tit">' + jsonStampList[i].evt_title + '</strong>' +
					'<p class="txt2">' + jsonStampList[i].evt_desc + '</p>' +
					'<em class="info-opt">' + userCnt + '</em>' +
					'<em class="info-opt">' + jsonStampList[i].use_psbt_excp_brnch_content + '</em>' +
					'<dl class="info">' +
						'<dt class="info-tit">기간 : </dt>' +
						'<dd class="info-txt">' + period + '</dd>' +
					'</dl>' +	
				'</header>' +	
				'<div class="wrap-thumb">' +
					'<img src="' + jsonStampList[i].stamp_img_full_url + '" class="thumb" alt="스탬프 이미지" width="175" height="176" >' +
				'</div>' +
				'<a style="cursor:pointer;"  onclick="javascript:GetEventStampDetailInfo(' +i + ');" class="link-detail layerpop-trigger'+i+'-'+ jsonStampList[i].stamp_id+'" data-trigger="layerpop-mstamp-detail">M 스탬스 상세</a>' +
				'		<script> '+
				'		$(".layerpop-trigger'+i+'-'+ jsonStampList[i].stamp_id+'").on("click.f", function (event) {' +
