...
//	var dq_searchFormTextbox = dq_searchForm.searchTerm;
 	...
 	function dq_acKeywordTextViewFF(fireFoxEvent)
	{
	 	...
	 	if(fireFoxEvent.keyCode==40 || fireFoxEvent.keyCode==9)
	 	{
			...
			if(dq_acLine < qs_ac_len)
			{
				...
				dq_acLine++;
			 	...
			 	ac = document.getElementById('dq_ac' + dq_acLine) ? document.getElementById('dq_ac' + dq_acLine) : '';
			 	acq = document.getElementById('dq_acqHidden' + dq_acLine) ? document.getElementById('dq_acqHidden' + dq_acLine) : ''; 
			 	dumy_searchTerm = document.getElementById('dumy_searchTerm' + dq_acLine) ? document.getElementById('dumy_searchTerm' + dq_acLine) : '';
 	...
 	function dq_reqSearch()
 	{
		...
		sv_f = "/search/ajax/headGetAutoKeyword.do?p=1&q=" + escape(encodeURIComponent(ke));
	...
	//자동완성 레이어 mouse out
 	...
 	function dq_getAcResultList(dp)
 	{
 		...
 		if(dq_searchKeyword!="") {
			...
			{
	 			text += "<div class=\"search-inner\">";

	 			//FKEY
	 			if(dpFrontList != ""){

		 			text += "<div id=\"serachResultCookie\" class=\"search-initial\" style=\"display: block;\">";
				 	...
				 	for(i=0;i<dpFrontList.length;i++)
				 	{
				 		...
				 		len = dqc_strlen(rekeyword);
				 		if(len>45){
				 			keywordSumm = dqc_substring(rekeyword, 0, 45) + "...";
				 		}else{
				 			keywordSumm = dqc_substring(rekeyword, 0, len);
				 		}
						count = result[1];
				 	...
				 	text += "</div>";
				 	text += "<div class=\"search-btm\"><div class=\"btm-type-c\"><button type=\"button\" class=\"close search-layer-close\" onclick=\"dq_acDisplayHide();\">닫기</button></div></div>";
 	...
 	function dq_getAcMySearchTermList() {
 		...
 		var saveStatus = getSearchCookie("dq_mykeyword_save");
 		text += "<div class=\"search-inner\">";
 		text += "<div id=\"serachResultCookie\" class=\"search-word\">";

 		if(dq_searchKeyword == ""){
	 		if($("#headChangeKey").val() == "C" || ($("#headChangeKey").val() == '' &&  (str != "" && str != ";"))){
	 			text += "<strong class=\"tab01\"><a href=\"#\" onmouseover=\"changeKey('C');\" class=\"active\" title=\"현재 선택\">최근검색어</a></strong>";
		 		text += "<div>";
				...
				if ( str != "" && str != ";" ) {
					...
					for( var i=0; i<cookieCount; i++ ) {
						...
						len = dqc_strlen(CookieVal);
						if(len > 24){
							text += dqc_substring(CookieVal, 0, 24) + "...";
						}else{
							text += dqc_substring(CookieVal, 0, len);
						}
						if(CookieDate != undefined){
							text += "<span>"+CookieDate+"</span></a>";
						}else{
							text += "</a>";
						}
				...
				text += "</div>";
				text += "<div class=\"search-btm\" id=\"test1\"><div class=\"btm-type-a\">";
				text += "<button type=\"button\" onclick=\"javascript:delSearchCookieAll('dq_cookie_search');\">전체삭제</button>";
				if(saveStatus.split( '∃' )[0] == 'off'){
					text += "<button type=\"button\" id=\"target\" class=\"save-off\" onclick=\"javascript:saveOn();\">검색어 저장켜기</button>";
				}if(saveStatus.split( '∃' )[0] == 'on'){
					text += "<button type=\"button\" id=\"target\"  class=\"save-off\" onclick=\"javascript:saveOff();\">검색어 저장끄기</button>";
				}
				text += "<button type=\"button\" class=\"close search-layer-close\" onclick=\"dq_acDisplayHide();\">닫기</button>";
				text += "</div></div>";
				text += "<strong class=\"tab02\"><a href=\"#\" onmouseover=\"changeKey('T');\">급상승검색어</a></strong>";
			 	cnt = cookieCount-1;

	 		}else if($("#headChangeKey").val() == "T" || ($("#headChangeKey").val() == '' &&  str == "")){
			 	//급상승검색
	 			text += "<strong class=\"tab01\"><a href=\"#\" onmouseover=\"changeKey('C');\">최근검색어</a></strong>";
	 			text += "<strong class=\"tab02\"><a href=\"#\" onmouseover=\"changeKey('T');\"  class=\"active\" title=\"현재 선택\">급상승검색어</a></strong>";
	 			text += "<div>";
				...
				var Year = dt.getFullYear();
			 	...
			 	if(dqTrendList != ""){
				 	...
				 	for(i=0;i<dqTrendList.length;i++)
				 	{
						...
						text += "<a href=\"#\" search-num-data=\"" + (i+1+ranking_) + "\" onclick=\"dq_keywordSearch('" + keywordOrign + "');\" id=\"dq_alink" + (i+1) + "\">";
						text +=  keyword + "</a>";
				 	...
				 	text += "</ol>";
				 	text += "</div>";
				 	text += "<div class=\"search-btm\">";
				 	text += "<div class=\"btm-type-b\"><span>"+ toDate + "&nbsp;(1시간 기준)</span><button type=\"button\" class=\"close search-layer-close\" onclick=\"dq_acDisplayHide();\">닫기</button></div>";
					text += "</div>";
				}
	 		}
		 	text += "</div>";
 ...
 function dq_getAcNoResultList()
	...
	var keywordTimeout = null;
 	$("#searchTerm")
	    ...
	    .on('keydown keypress', function() {
			...
			keywordTimeout = setTimeout(dq_wi, dq_intervalTime);
		});
	setTimeout("dq_wi()", dq_intervalTime);
	
	$('.search-layer')
		.off('keydown.focusMove')
		.on('keydown.focusMove', '.search-layer-close', function(e) {
			if(_wac.isEnterEvent(e)) {
				setTimeout(function() {
					$('#searchTerm').next('button[type="submit"]').focus();
				}, 100)
			}
		});
	...
	// 181008 [웹접근성 : 원은재]
	var wacSearchWindow = function(e){
			var $nowFocus, $sLayer, $itemsOfslayer, $sTab,
					$sWindow = $('.swindow'),
					$itemsOfsWindow = _wac.getFocusables($sWindow),
					$inputOfsWindow = $sWindow.find('#searchTerm'),
					$btnOfsWindow = $inputOfsWindow.next(),
					$lastOfsWindow = $itemsOfsWindow.last();
		
			var sWindowKeydownHandler = function(e){
				$nowFocus = $(document.activeElement);
				$sLayer = $sWindow.find('.search-layer');
				$itemsOfslayer = _wac.getFocusables($sLayer);
				$sTab = $sLayer.find('strong[class^="tab"] > a');
			
				//ESC 키
				if(_wac.isEscEvent(e)){
					_wac.preventDefaultAction(e);
					dq_acDisplayHide();
				}
				//검색 레이어 창이 닫히는 경우들
				else if(_wac.isShiftTabEvent(e) && $nowFocus.is($inputOfsWindow)
						|| _wac.isTabEvent(e) && $nowFocus.is($itemsOfslayer.last())){
					setTimeout(function(){
					dq_acDisplayHide();
					},100);
				}
				
				var tabKeydownHandler = function(e){
					$nowFocus =$(this);
					
					if(_wac.isEnterEvent(e)){
						if($nowFocus.is($sTab.first())){
							changeKey('C');
							return false;
						}else if($nowFocus.is($sTab.last())){
							_wac.preventDefaultAction(e);
							changeKey('T');
							setTimeout(function(){
								$('strong[class^="tab"] > a.active').focus();
							}, 100)
						}
					}
				}
				_wac.wireUpEventAsync($sTab, 'keydown', tabKeydownHandler);
			};

			$sWindow.on({
				keydown : sWindowKeydownHandler
			});
	}();
