 	...
 	function DQmainSearch_submit(){
 		...
 		if(searchTerm == search_link_nm && search_link_url != ""){
	 		...
	 		if(len < 1){
	 			...
	 			alert("검색어 20자이상 입니다.");	//검색하실 내용을 입력하세요.
	 			document.getElementById("searchTerm").focus();
	 			return;	
	 		}else {
	 			if(dumy_searchTerm.substring(0, 10) == "HHHHHHHHHH"){     
	 				location.href="/search/search.do?searchField="+searchField+"&searchTerm="+encodeURIComponent(dumy_searchTerm.replace(/%/g, '%25').replace(/\+/g, '%2B'))+ "&viewType="+searchpViewType;
	 			}else{
	 				location.href="/search/search.do?searchField="+searchField+"&searchTerm="+encodeURIComponent(searchTerm.replace(/%/g, '%25').replace(/\+/g, '%2B'))+ "&viewType="+searchpViewType;
	 			}
	 			if(saveStatus.split( '∃' )[0] != "off") {
	 				var dt = new Date();
	 			 	...
	 			 	searchTerm = searchTerm.replace(/(^\s*)|(\s*$)/gi, "");
	 			 	if(searchTerm != ""){
