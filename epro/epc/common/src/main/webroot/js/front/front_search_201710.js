...
function fnmallType(type){
	...
	searchForm.maxPrice.value = "";
	searchForm.brandCd.value = "";
	searchForm.discount.value = "";
	searchForm.plus.value = "";
...
function DetailSearch(){
	...
	//20170918 즉시할인추가
	if($("#check-benef-7").is(":checked") == true){
		document.pageNavigator.discount.value = "Y";
	}else{
		document.pageNavigator.discount.value = "N";
	}
	//20170921 1+1 추가
	if($("#check-benef-8").is(":checked") == true){
		document.pageNavigator.plus.value = "Y";
	}else{
		document.pageNavigator.plus.value = "N";
	}
	...
	if($("dl").hasClass("price-form add-form clear-after") == true){
		...
		document.pageNavigator.minPrice.value=minPrice && minPrice.replace(/,/g, '');
		document.pageNavigator.maxPrice.value=maxPrice && maxPrice.replace(/,/g, '');
	}else{
...
//	console.log(url+"/product/ProductDetail.do?CategoryID="+cateId+"&ProductCD="+prodCd+"&socialSeq=&koostYn=");
