	...
	//자동완성 레이어 mouse out
 	...
 	{ 	 
 		...
 		if(dq_searchKeyword!="") {
			...
			{	 	
	 			...
	 			text += "<div class=\"search-initial\" style=\"display: block;\">";
	 			...
	 			text += "<div class=\"btm-type-c\">";	
	 			text += "<button type=\"button\" class=\"close\">닫기</button>";	
	 			text += "</div>";	
	 			text += "</div>";	
 	...
 	function dq_getAcMySearchTermList() {
 		...
 		text += "<div class=\"search-inner\">";
 		text += "<div class=\"search-word\" style=\"display: block;\">";
 	
 		if(dq_searchKeyword == ""){
 			
 		if($("#headChangeKey").val() == "C"  || ($("#headChangeKey").val() == '' &&  (str != "" && str != ";"))){
	 		text += "<strong class=\"tab01\"><a href=\"#\" onmouseover=\"changeKey('C');\"  class=\"active\">최근검색어</a></strong>";
	 		text += "<div style=\"display: block;\">";
				...
				cnt = cookieCount-1;
	 			...
	 			text += "<strong class=\"tab02\"><a href=\"#\" onmouseover=\"changeKey('T');\" class=\"active\">급상승검색어</a></strong>";
				text += "<div>";
				...
				var nowYear = dt.getFullYear();
			 	...
			 	if(dqTrendList != ""){
				 	...
				 	for(i=0;i<dqTrendList.length;i++)
				 	{
						...
						text += "<a href=\"#\" onclick=\"dq_keywordSearch('" + keywordOrign + "');\" id=\"dq_alink" + (i+1) + "\" search-num-data=\""+(i+1+ranking_)+"\">";
						text +=  keyword + "</a>";
		 		...
		 		text += "<div class=\"search-btm\">";
				text += "<div class=\"btm-type-a\">";
				
				if($("#headChangeKey").val() == "C"  || ($("#headChangeKey").val() == '' &&  (str != "" && str != ";"))){
					text += "<button type=\"button\" onclick=\"javascript:delSearchCookieAll('dq_cookie_search');\">전체삭제</button>";
					if(saveStatus.split( '∃' )[0] == 'off'){
						text += "<button type=\"button\" onclick=\"javascript:saveOn();\">검색어 저장켜기</button>";
					}if(saveStatus.split( '∃' )[0] == 'on'){
						text += "<button type=\"button\" onclick=\"javascript:saveOff();\">검색어 저장 끄기</button>";
					}
					text += "<button type=\"button\" class=\"close\">닫기</button>";
				}
				else if($("#headChangeKey").val() == "T" || ($("#headChangeKey").val() == '' &&  str == "")){
					text += "<span>"+ nowDate + "기준</span>";
					text += "<button type=\"button\" class=\"close\">닫기</button>";
				}
			text += "</div>";
			text += "</div>";
