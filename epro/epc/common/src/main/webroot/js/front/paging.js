...
function getRenderHtml(totalCnt, fromPageNo, toPageNo, lastPageNo, curPage, previousPage, nextPage, funcName, img){
    ...
    */
    // 이전 페이지 출력
    if(previousPage !='1'){
    	htmlString += "<a href='javascript:;' onclick=\"" + funcName + "(" + previousPage + ")" + "\"><img src='"+preImgPath+"' alt='이전리스트' style='cursor:pointer' onclick=\"javascript:" + funcName + "('" + previousPage + "');\" ></span>";
    }else{
    	// htmlString += "<span></span>";
    }
    ...
    for (var i=fromPageNo; i<=toPageNo; i++) {
    	...
    	if (i == curPage){
    		...
    		htmlString += "<a href='javascript:;' onclick=\"" + funcName + "('" + i + "'); return false;\">" + i + "</a>";       
    	}
    }
    htmlString += "</span>";
    ...
    if (curPage == nextPage) {
    	...
    	htmlString += "<a href='javascript:;' onclick='" + funcName + '('+ nextPage +')' + "'><img src='"+nextImgPath+"' alt='다음리스트'></a>";
    }
    // 마지막 페이지 출력
    /*
    if (curPage == lastPageNo) {
 ...
 * @return
 */
function setLMPaging2(totalCntStr, rowsPerPageStr, curPageStr, funcName, pagingDivId ) {
	var totalCnt = 0;
	var curPage = 0;
	var rowsPerPage = 0;
	try{
		...
		rowsPerPage = parseInt(rowsPerPageStr, 10);
		curPage = parseInt(curPageStr, 10);
		document.getElementById(pagingDivId).innerHTML = render2(totalCnt, curPage, rowsPerPage, funcName);
	}catch(e){
		alert(e);
	}
 ...
 * @param funcName
 */
function render2(totalCnt, curPage, rowsPerPage, funcName){
	var lastPageNo;					// 마지막 페이지 번호
	var fromPageNo;					// 페이지 인덱스 시작 번호 
	var toPageNo;					// 페이지 인덱스 끝 번호
	var previousPage;				// 이전 페이지
    ...
    return getRenderHtml2(totalCnt,fromPageNo, toPageNo, lastPageNo, curPage
        ,previousPage, nextPage, funcName);
}
...
function getRenderHtml2(totalCnt, fromPageNo, toPageNo, lastPageNo, curPage, previousPage, nextPage, funcName){
	var htmlString = "";
    ...
    if (curPage == 1) {
    	...
    	htmlString += "<span><img src='/images/front/product/btn-pagePrevFF.gif' alt='처음으로' style='cursor:pointer' onclick=\"javascript:" + funcName + "(1);\" ></span>";
    }
    
    // 이전 페이지 출력
    if(previousPage !='1'){
    	htmlString += "<span><img src='/images/front/product/btn-pagePrev.gif' alt='이전리스트' style='cursor:pointer' onclick=\"javascript:" + funcName + "('" + previousPage + "');\" ></span>";
    }else{
    	// htmlString += "<span></span>";
    }
    htmlString += "<span class=\"page_num\">";
    ...
    for (var i=fromPageNo; i<=toPageNo; i++) {
    	...
    	if (i == curPage){
    		...
    		htmlString += "<span onclick=\"javascript:" + funcName + "('" + i + "');\" style='cursor:pointer' >" + i + "</span>";
    	}
    }
    htmlString += "</span>";
    ...
    if (curPage == nextPage) {
    	...
    	htmlString += "<span><img src='/images/front/product/btn-pageNext.gif' alt='다음리스트' style='cursor:pointer' onclick=\"javascript:" + funcName + "('" + nextPage + "');\" ></span>";
    }
    ...
    if (curPage == lastPageNo) {
    	...
    	htmlString += "<span><img src='/images/front/product/btn-pageNextBB.gif' alt='마지막으로' style='cursor:pointer' onclick=\"javascript:" + funcName + "('" + lastPageNo + "');\" ></spna>";
    }
