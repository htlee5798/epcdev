...
function SetPageInfos(totItemCount, selPage, searchFuncName, showFuncName, redraw) {
	...
	if (selPage > 10) {
		htmlString += '<a  class="page-prev" alt="이전리스트" style="cursor:pointer" ' +
 				  	  'onclick="javascript:' + searchFuncName + '(' + (startPage - 1) + ');"></a>';		
	}
	...
	for (var i = startPage; i < endPage; i++) {
		if (i == selPage)
			...
			htmlString += '<a onclick="javascript:' + searchFuncName + '(' + i + ');" style="cursor:pointer">' + i + '</a>';
	}
	htmlString += "</span>";
	
	// 이후 페이지 출력
	if (endPage < maxPage + 1)
		htmlString += '<a  class="page-next" alt="다음리스트" style="cursor:pointer" ' +
					  'onclick="javascript:' + searchFuncName + '(' + endPage + ');"></a>';
