...
var _n_sid = n_getSid();			/* 도메인을 sid로 사용 */
var _n_uid_cookie = "nethru_cust_id";					/* uid */

var _n_ls = "http://wlog.lottemart.com/wlo/Logging";		/* logging server */
var _n_uls = "http://wlog.lottemart.com/wlo/UserLogging";	/* user logging server */

var _n_c_field1="malltype";								/* 추가쿠키 필드1 */
var _n_c_field2="divcode";								/* 추가쿠키 필드2 */
var _n_c_field3="AffiliateID";								/* 추가쿠키 필드3 */
var _n_c_field4="CHANNEL_CD";							/* 추가쿠키 필드4 */
var _n_c_field5="StrCD";								/* 추가쿠키 필드5 */
...
var _n_use_channel = false;

/* https logging */
if ( document.location.protocol == "https:" ) {
	_n_ls = "https://wlog.lottemart.com/wlo/Logging";
	_n_uls = "https://wlog.lottemart.com/wlo/UserLogging";
}
...
function n_getSid() {
	var _user_agent = navigator.userAgent,
		_host_name = location.hostname,
		_lotte_mart_web = new RegExp( 'lottemart.com', 'gi' ),
		_toysrus_web = new RegExp( 'toysrus.lottemart.com', 'gi' ),
		_lotte_mart_mobile_web = new RegExp( 'm.lottemart.com', 'gi' ),
		_toysrus_mobile_web = new RegExp( 'm.toysrus.lottemart.com' ),
		_lotte_mart_app = new RegExp( 'lottemart-app-shopping-android|lottemart-app-shopping-ios', 'gi' ),
		_toysrus_app = new RegExp( 'toysrus.android.shopping|toysrus.iphone.shopping', 'gi' ),
		_company_web = new RegExp( 'company.lottemart.com', 'gi' );
		returnValue = '';

	if( _lotte_mart_mobile_web.test( _host_name ) ) {
		if( _lotte_mart_app.test( _user_agent ) ) {
			returnValue = 'app.lottemart.com';
		} else {
			returnValue = 'm.lottemart.com';
		}
	} else if( _lotte_mart_web.test( _host_name ) ) {
		returnValue = 'www.lottemart.com';
	}
	
	if( _toysrus_mobile_web.test( _host_name ) ) {
		if( _toysrus_app.test( _user_agent ) ) {
			returnValue = 'app.toysrus.lottemart.com';
		} else {
			returnValue = 'm.toysrus.lottemart.com';
		}
	} else if( _toysrus_web.test( _host_name ) ) {
		returnValue = 'toysrus.lottemart.com';
	}
	
	if( _company_web.test( _host_name ) ) {
		returnValue = 'company.lottemart.com';
	}
...
function n_get_channel_cookieparam() {
	...
	convdate = n_GetCookie(_n_convday_param);
	mid = n_GetCookie(_n_mid_cookie);
	
	var current = new Date();
	convday = (current.getTime() - convdate.getTime()) / 1000 / 60 / 60 / 24;
	
	if ( convday <= 0 ) convday = 1;
	if ( mid == null) mid = "0";
	if ( src == null) src = "1";
...
function n_create_channel_cookie(mid, src, date, keyword) {
	...
	expiredDate.setTime(currentDate.getTime() + 1000 * 60 * 60 * 24 * _n_max_conv_day)
	...
	if ( keyword != "") {
		n_SetCookie(_n_keyword_cookie, keyword, "/", domain);
	}
}
