		...
		if (CheckMessageCode(json, jqXHR)) {
			...
			if (jsonDetail.stamp_divn_cd == "GROUP") {
				...
				if (jsonDetail.group_reg_id != undefined && jsonDetail.group_reg_id != null)
					...
					groupMinNum += '정원 : ' + jsonDetail.stamp_psbt_user_cnt + '명';
					if(jsonDetail.stamp_card_issue_cnt > 0)
						groupMinNum += ' 중 <a  style="cursor:pointer;" id ="GroupMemId" onclick="javascript: GetGroupMembersInfo(\'' + 
						jsonDetail.stamp_id + '\',\'' + 
								jsonDetail.stamp_card_id + '\'); return false;"> ' + jsonDetail.stamp_card_issue_cnt + ' 명 참여 </a>';
						groupMinNum += '<p class="desc-foot">* 신청자 포함 멤버 2인부터 스템프 날인</p>';
			}
			else {
				$('#stampType').html("혼자하기");
			}
			...
			if (jsonDetail.stamp_spc_cnt > 0) {
				for (var i = 0; i < count; i++) {
					if (i < jsonDetail.stamp_spc_seal_cnt)
						checkStamps += '<span class="stamp-special-blue"></span>';
					else if (i < jsonDetail.stamp_spc_cnt)
						checkStamps += '<span class="stamp-special-gray"></span>';
					else
						checkStamps += '<span></span>';				
				}
			}
			...
			if (jsonDetail.stamp_bsis_cnt > 0) {
				for (var i = 0; i < count; i++) {
					if (i < jsonDetail.stamp_bsis_seal_cnt)
						checkStamps += '<span class="stamp-red"></span>';
					else if (i < jsonDetail.stamp_bsis_cnt)
						checkStamps += '<span class="stamp-gray"></span>';
					else
						checkStamps += '<span></span>';				
				}
			}
			...
			for (var i = 0; i < missionList.length; i++) {
				...
				if (missionList[i].stamp_apply_to_cd == "PRICE") {
					...
					for (var j = 0; j < missionList[i].mison_prod_list.length; j++)
						detailMission += '<li><a href="/mobile/cate/PMWMCAT0004_New.do?ProductCD=' + missionList[i].mison_prod_list[j].prod_sell_cd + '" class="link">[' + missionList[i].mison_prod_list[j].prod_sell_cd + '] ' + missionList[i].mison_prod_list[j].prod_nm+ '</a></li>';
				}
				else
					detailMission = setComma(missionList[i].base_buy_amt) + "원 이상 구매";
				...
				if (missionList[i].bsis_mison_yn == "Y") {				
					normalMission += '<div class="wrap-tit"><p class="tit-type1"> 도전하기1 [' + missionList[i].stamp_apply_to_cd_name +  ']</p>' + 
						' <i class="icon-tag-eventcoupon7">' + missionList[i].fulfill_chanel_cd_name + '</i></div>' ;
					if (detailMission != "") {	
						normalMission += '<hr><ul class="textlist"><li class="point-underline">' + detailMission + '</li></ul>';
					}
				}
				else {
					specialMissions += '<div class="wrap-tit"><p class="tit-type2"> 도전하기2 [' + missionList[i].stamp_apply_to_cd_name + ']</p>' +
						'  <i class="icon-tag-eventcoupon7">' + missionList[i].fulfill_chanel_cd_name + '</i></div>' ;
					if (detailMission != "") {	
						specialMissions += '<hr><ul class="textlist"><li>' + detailMission+ '</li></ul>';
					}	
				}
			}
			
			if (normalMission != ""){
				//$('#missions').addClass('challenge first');
				$('#missions').css('display','block');
				$('#missions').append(normalMission);
			}	
			if (specialMissions != ""){
				//$('#specialMissions').addClass('challenge');
				$('#specialMissions').css('display','block');
			...
			if (jsonDetail.stamp_divn_cd == "GROUP") {	// 그룹인 경우
				if (jsonDetail.grp_aply_yn == "Y") { //승인전
					$('#waiting').parent().css('display', 'block');
				}
			...
			if (jqXHR.readyState == 4 && jqXHR.status == 200) {
				if (CheckMessageCode(json, jqXHR)) {
					...
					if (reserveList.length > 0) {
						reserveInfo = '<table><colgroup><col width="30%"><col width="auto"></colgroup><caption>스탬프보기</caption>' ;
						reserveInfo += '<thead><tr><th scope="col">날짜</th><th scope="col">내용</th></tr></thead>' ;
						reserveInfo += '<tbody> ';
							for (var i = 0; i < reserveList.length; i++) {
								reserveInfo += '<tr>' +
									'<td>' + reserveList[i].reg_date + '</td>' +
									'<td>' + reserveList[i].str_nm + '(' + reserveList[i].user_nick + ') ' + reserveList[i].stamp_seal_cnt + '개</td>' +
								'</tr>';
							}
						reserveInfo += '</tbody></table> ';
					} else {
						$("#saveInfos").removeClass();
						reserveInfo = '<p class="list-empty">내역이 없습니다.</p>';
					}
				}
				else
					console.log(json.message);
			...
			if (jqXHR.readyState == 4 && jqXHR.status == 200) {
				if (CheckMessageCode(json, jqXHR)) {
					...
					if (0 < rewardList.length) {
						...
						rewardInfo = '<table><caption>스탬프 보기</caption><colgroup><col width="20%"><col><col width="35%"></colgroup>' +
										' <thead><tr><th scope="col">구분</th><th scope="col">쿠폰명</th><th scope="col">유효기간</th></tr></thead>'+
										'<tbody>';
							...
							if(day <=0 ){
								...
								rewardInfo += '<tr><td class="acenter">완료<br>미션</td>' +
										'<td>' + rewardList[0].evt_title +  '</td>' +
										'<td class="acenter">' + endDate +  '까지 </td></tr>';
							// 최종보상 end -------------------------------------
						...
						if(rewardList[1] == null){ 
							...
							rewardInfo += '<tr><td class="acenter">중간<br>미션</td>' +
													'<td>' + rewardList[1].evt_title +  '</td>' +
													'<td class="acenter">' + endDate +  '까지 </td></tr>';
						}	// 중간보상 있음 end ------------------------------------
						
						rewardInfo += '<div class="giftcheck"><p class="tit">보상신청 안내</p><ul><li>스탬프의 보상신청은 모바일 앱에서만 가능합니다.</li></ul></div>';
						//$('#rewardInfos').addClass('list-complete');
					}else{ 
						rewardInfo = '<p class="list-empty">보상내역이 없습니다.</p>';		 
						//$('#rewardInfos').addClass('tabcontents view-stamp');
					}
				}else
					console.log(json.message);
			}	// 조회 성공 end -----------------------
...
function GetGroupMembersInfo(stampId,stampCardId){
	...
	location.href="/mobile/evt/mobileEventMStampMemberDetail.do?stampCardId="+stampCardId + "&stampId="+stampId;
}

//스탬프 그룹 멤버 정보 조회
function NewGetGroupMembersInfo(stampCardId,stampId,userId) {
	var data = [];
	...
	GetStampInfo(stampId,userId);
	GetGroupMembersApplicantInfo(stampCardId,userId);
}

function GetStampInfo(stampId,userId){
	var data = [];
		...
		if (CheckMessageCode(json, jqXHR)) {
			...
			for(var i=0; i<memberList.length; i++){
				if(json.data.grp_gen_rep_id == memberList[i].ptc_user_rep_id){
					groupMembers = '<li><div class="conts"><p class="indent-icon"><i class="icon-evtcpn-crown"></i> ' + memberList[i].user_nick + '</p></div> <div class="desc">' + memberList[i].reg_date + ' 참여</div></li>';
				}
			}
			//그룹장을 뺀 나머지 멤버를 순서대로 넣기
			for(var i=0; i<memberList.length; i++){
				if(json.data.grp_gen_rep_id != memberList[i].ptc_user_rep_id){
					groupMembers += '<li><div class="conts"><p class="indent-icon">' + memberList[i].user_nick + '</p></div> <div class="desc">' + memberList[i].reg_date + ' 참여</div></li>';
				}
			}
			$('#groupMembers').empty();
...
//스탬프 그룹 참여 신청자 정보 조회
function GetGroupMembersApplicantInfo(stampCardId,userId) {	
	var data = [];
		...
		if (CheckMessageCode(json, jqXHR)) {
			...
			if(appList.length == '0'){
				...
				for (var i = 0; i < appList.length; i++) {
					applicants = '<li><div class="conts">' + appList[i].user_nick + '</div> <div class="desc">모바일에서 승인가능</div></li>';
