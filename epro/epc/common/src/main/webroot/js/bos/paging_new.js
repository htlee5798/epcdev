...
function setLMPaging(totalCntStr, rowsPerPageStr, curPageStr, funcName, pagingDivId, gubun ) {
	...
	//try {
		...
		if( document.getElementById(pagingDivId+"Cnt") != "undefined" && document.getElementById(pagingDivId+"Cnt") != null){
			document.getElementById(pagingDivId+"Cnt").innerHTML = getRenderCnt(totalCnt, curPage, rowsPerPage, funcName, gubun);
		}
		if( document.getElementById(pagingDivId+"Cnt") != "undefined" && document.getElementById(pagingDivId+"Cnt") != null){
			document.getElementById(pagingDivId).innerHTML = renderPageList(totalCnt, curPage, rowsPerPage, funcName, gubun);
		}
	//}catch(e) {
	//	alert(e);
	//}
 ...
 * @param funcName
 */
function renderPageList(totalCnt, curPage, rowsPerPage, funcName, gubun) {
	var lastPageNo;					// 마지막 페이지 번호
	var fromPageNo;					// 페이지 인덱스 시작 번호 
	var toPageNo;					// 페이지 인덱스 끝 번호
	var previousPage;				// 이전 페이지
    ...
    return getRenderPageHtml(totalCnt, fromPageNo, toPageNo, lastPageNo, curPage
    				   , previousPage, nextPage, funcName, rowsPerPage, gubun);
}
...
function getRenderHtml(totalCnt, fromPageNo, toPageNo, lastPageNo, curPage, previousPage, nextPage, funcName, rowsPerPage, gubun) {
    ...
    if (gubun == "1") {
    	...
    	htmlString += "	<option value=\"1000\" "+(rowsPerPage == 1000 ? 'selected':'')+">1000건</option>";
    	htmlString += "	<option value=\"10000\" "+(rowsPerPage == 5000 ? 'selected':'')+">5000건</option>";
 ...
 * @param funcName
 */
function getRenderCnt(totalCnt, curPage, rowsPerPage, funcName, gubun) {
	var htmlString = "";
	var pagingGubun = gubun||"1";
	
	htmlString += " [총건수 <em>";
	if (totalCnt == 0) {
		htmlString += "0 </em>건] <span class='btn_frt'>";
	} else {
		htmlString += setComma(totalCnt) + "</em>건] <span class='btn_frt'>";
	}
	
	if (gubun == "1") {
		htmlString += "<select name=\"rowsPerPage\" id=\"rowsPerPage\" onChange=\"javascript:doSearch();\" style=\"width:60px\" title=\"정렬갯수\" class=\"g_arr\" >";
		htmlString += "	<option value=\"100\" "+(rowsPerPage == 100 ? 'selected':'')+">100건</option>";
    	htmlString += "	<option value=\"500\" "+(rowsPerPage == 500 ? 'selected':'')+">500건</option>";
    	htmlString += "	<option value=\"1000\" "+(rowsPerPage == 1000 ? 'selected':'')+">1000건</option>";
    	htmlString += "	<option value=\"5000\" "+(rowsPerPage == 5000 ? 'selected':'')+">5000건</option>";
 ...
 * @param funcName
 */
function getRenderPageHtml(totalCnt, fromPageNo, toPageNo, lastPageNo, curPage, previousPage, nextPage, funcName, rowsPerPage, gubun) {
	var htmlString = "";
	var pagingGubun = gubun||"1";
    ...
    // 첫 페이지
    if (curPage == 1) {
    	htmlString += "<img src=\"/images/bos/new/btn_a_first.png\" alt=\"첫페이지\" title=\"첫페이지\" align=\"middle\" />";
    } else {
    	//htmlString += "<span class=\"fst\" onclick=\"javascript:" + funcName + "(1);\" ></span>";
    	htmlString += "<a href=\"javascript:" + funcName + "(1);\"><img src=\"/images/bos/new/btn_a_first.png\" alt=\"첫페이지\" title=\"첫페이지\" align=\"middle\" /></a>";
    }
    
    // 이전 페이지 출력
    if (previousPage !='1') {
    	htmlString += "<a href=\"javascript:" + funcName + "('" + previousPage + "');\"><img src=\"/images/bos/new/btn_a_prev.png\" alt=\"이전10페이지\" title=\"이전10페이지\" align=\"middle\" /></a>";
    } else {
    	htmlString += "<img src=\"/images/bos/new/btn_a_prev.png\" alt=\"이전10페이지\" title=\"이전10페이지\" align=\"middle\" />";
    }
    ...
    // 이후 페이지 출력
    if (curPage == nextPage) {
        htmlString += "&nbsp;<img src=\"/images/bos/new/btn_a_next.png\" alt=\"다음\" title=\"다음10페이지\" align=\"middle\" />";
    } else {
    	htmlString += "&nbsp;<a href=\"javascript:" + funcName + "('" + nextPage + "');\"><img src=\"/images/bos/new/btn_a_next.png\" alt=\"다음\" title=\"다음10페이지\" align=\"middle\" /></a>";
    }


    // 마지막 페이지 출력
    if (curPage == lastPageNo) {
        htmlString += "<img src=\"/images/bos/new/btn_a_end.png\" alt=\"마지막페이지\" title=\"마지막페이지\" align=\"middle\" />";
    } else {
    	htmlString += "<a href=\"javascript:" + funcName + "('" + lastPageNo + "');\"><img src=\"/images/bos/new/btn_a_end.png\" alt=\"마지막페이지\" title=\"마지막페이지\" align=\"middle\" /></a>";
    }
 ...
 * @param funcName
 */
function getRenderPageHtml_OLD(totalCnt, fromPageNo, toPageNo, lastPageNo, curPage, previousPage, nextPage, funcName, rowsPerPage, gubun) {
	var htmlString = "";
	var pagingGubun = gubun||"1";
