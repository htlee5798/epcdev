...
function render(totalCnt, curPage, rowsPerPage, funcName, gubun){
	...
	fromPageNo = ((curPage-1) / indexSize) * indexSize + 1;
	...
	previousPage = fromPageNo;
	if (previousPage < 1) {
		previousPage   = 1;
	}
	nextPage = fromPageNo + 1;
...
function getRenderHtml(totalCnt, fromPageNo, toPageNo, lastPageNo, curPage, previousPage, nextPage, funcName, rowsPerPage, gubun){
    ...
    if(previousPage !='1'){
    	...
    	htmlString += "<span class=\"pre\" onclick=\"javascript:" + funcName + "('" + (previousPage-1) + "');\" ><a href=\"#\"><img src=\"/images/epc/layout/btn_prev.gif\" alt=\"이전페이지\" class=\"middle\" /></a></span>";
    }else{
    ...
    for (var i=1; i<=toPageNo; i++) {
    	...
    	if(i > 1){
    		...
    		col = "style=\"color:#b52f03;\"";
    	}
    	
    	if (i == curPage){
    		htmlString += "<span class=\""+css+" bar2\"><a href=\"#\" "+col+">" + i + "</a></span>";
    	}else{
    		htmlString += "<span class=\""+css+"\" onclick=\"javascript:" + funcName + "('" + i + "');\" ><a href=\"#\" "+col+">" + i + "</a></span>";
    	}
    }
