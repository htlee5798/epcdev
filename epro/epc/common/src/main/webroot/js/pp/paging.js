 ...
 * @param selectFun      : 페이지 이동 이벤트 함수명
 * @param showSelectBox  : row Count 선택 selectBox 노출여부(true/false)
 * @return
 */
function setLMPaging(totalCntStr, rowsPerPageStr, curPageStr, funcName, pagingDivId, gubun, selectFun, showSelectBox ) {
	
	if(selectFun == undefined) selectFun = "doSearch";
	if(showSelectBox == undefined) showSelectBox = "true";
	var totalCnt = 0;
	var curPage = 0;
	var rowsPerPage = 0;
	...
	try{
		...
		document.getElementById(pagingDivId).innerHTML = render(totalCnt, curPage, rowsPerPage, funcName, gubun, selectFun, showSelectBox);
	}catch(e){
		alert(e);
	}
}
 ...
 * @param funcName
 */
function render(totalCnt, curPage, rowsPerPage, funcName, gubun, selectFun, showSelectBox){
    ...
    return getRenderHtml(totalCnt,fromPageNo, toPageNo, lastPageNo, curPage
        ,previousPage, nextPage, funcName, rowsPerPage, gubun, selectFun, showSelectBox);
}
...
function getRenderHtml(totalCnt, fromPageNo, toPageNo, lastPageNo, curPage, previousPage, nextPage, funcName, rowsPerPage, gubun, selectFun, showSelectBox){
	var htmlString = "";
    ...
    if ( gubun == "1" ) {
	    ...
	    htmlString += "<select name=\"rowsPerPage\" id=\"rowsPerPage\" onChange=\"javascript:"+selectFun+"();\" class=\"select\" align=\"right\">";
	    htmlString += "	<option value=\"5000\" "+(rowsPerPage == 5000 ? 'selected':'')+">5000건</option>";
	    htmlString += "	<option value=\"3000\" "+(rowsPerPage == 3000 ? 'selected':'')+">3000건</option>";
