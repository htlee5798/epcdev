...
function openProduct(mode,aprvYn,periDeli){
	if( typeof aprvYn == "undefined" ) aprvYn = 'ALL';
	if( typeof periDeli == "undefined" ) periDeli = 'N';
	Common.centerPopupWindow(context+'/product/PBOMPRD0061.do?mode='+mode+'&aprvYn='+aprvYn+'&periDeli='+periDeli, '상품검색', {width:800,height:520,scrollbars:'no'});	
}
...
function openProductOnOff(mode,onOffYn){
	...
	if( typeof onOffYn == "undefined" ) onOffYn = 'N';
	Common.centerPopupWindow(context+'/product/PBOMPRD0061.do?mode='+mode+'&onOffYn='+onOffYn, '상품검색', {width:800,height:520,scrollbars:'no'});	
}
/**
* 점포 검색 팝업
*/
function openStr(mode,id){
	if( typeof id == "undefined" ) id = "";
	Common.centerPopupWindow(context+'/product/PBOMPRD0060.do?mode='+mode+'&id='+id, '점포검색', {width:1000,height:700,scrollbars:'YES'});	
}
/**
* 템플릿 검색 팝업
*/
function openTemplate(mode,id){
	if( typeof id == "undefined" ) id = "";
	Common.centerPopupWindow(context+'/newTemplate/PBOMTEM0200.do?mode='+mode, '템플릿검색', {width:1000,height:520,scrollbars:'no'});
}
/**
 * 달력 팝업
 */
//function openCal(inputName) {
//	Common.centerPopupWindow('/html/bos/calendar.html?inputName='+inputName, 'lottemartCalendar', {width:250,height:250,scrollbars:'no'});
//}
 ...
 * 달력수동입력 변경으로 인한 mask, placeholder 값 삭제. 2016-05-23
 * */
...
function openCalWithCallback(inputName, callbackFuncName) {
	Common.centerPopupWindow('/html/bos/calendar.html?inputName='+inputName+'&callbackFunc='+callbackFuncName, 'lottemartCalendar', {width:250,height:250,scrollbars:'no'});
}
...
function loadingMask(){
	...
	var loadingDiv = $('<span id="loadingLayer" style="position:fixed; left:'+childLeft+'px; top:'+childTop+'px; width:100%; height:100%; z-index:10001"><img id="ajax_load_type_img" src="/js/jquery/res/loading.gif"></span>').appendTo($("body"));
	loadingDiv.show();
 ...
 * 
	 ...
	 * @version    : 1.0
	 * @Locaton    : 
	 * @Description : 와이즈그리드 컬럼키들을 문자열로 만들어 반환한다.
	 * 					ex) TOT_ORD_CNT,INSUFF_ORD_CNT,INSUFF_ORD_YUL
     * @param gridObj(와이즈그리드 객체) sIndex(시작인덱스) endIndex(끝인덱스) outOfIndexList(제외할 인덱스 리스트)
	 * @return  String
     * @throws 
*
 */
...
*
 */
function getRateInSurmmary(gridObj, strSummaryBarKey, dividend_colNm, divisor_colNm, mergeIndex){
	
	//피제수
	var dividend = gridObj.GetSummaryBarValue(strSummaryBarKey,dividend_colNm,mergeIndex,false);
	...
	//제수
	var divisor = gridObj.GetSummaryBarValue(strSummaryBarKey,divisor_colNm,mergeIndex,false);
...
function getAverageSurmmary(gridObj, strSummaryBarKey, dividend_colNm, mergeIndex, roundCnt){
...
function getDateFormatted(yyyymmdd, FORMAT){
	return yyyymmdd.substring(0,4)+FORMAT+yyyymmdd.substring(4,6)+FORMAT+yyyymmdd.substring(6,8);
}
...
function categoryChange(gubun, lUrl, mUrl) {
	...
	if (document.dataForm.selDaeGoods.value == "" ) {
		...
		if (mUrl) comboReset(document.dataForm.selSoGoods);
	}
		...
		success: function(data) {
			try {
				if (data.comboNm == "jung") {
					comboCall(document.dataForm.selJungGoods, data.jungCdList, 'ALL');
				} else if (data.comboNm == "so") {
					comboCall(document.dataForm.selSoGoods, data.soCdList, 'ALL');
				}
			} catch (e) {}
		},
		error: function(e) {
...
/*$.extend($.fn, {
	...
	enter : function(func) {
		$(this).bind('keypress', function(e) {
            if (e.keyCode == 13) {
            	func.apply(this, [e]);
            	e.preventDefault(); // do for IE dialog close bug.
            }
        });               
        return this; 
    }
...
if (pref == 'limadmin') lKey = '';
else					lKey = '';
var ibleaders;
ibleaders = ibleaders || {};
ibleaders = { license: lKey };
	...
	$.ajax({
		...
		success: function(data) {
			if (typeof(data) == 'string') {
				...
				RETURN_IBS_OBJ = data;
				sheetName.LoadSearchData(data.ibsList, {Append:1});
				var len = (data.ibsList) ? eval(data.ibsList).length : 0; 
				if (len == 0) {
					$('#resultMsg').text('해당 자료가 없습니다.');
				} else {
...
function fn_checkExtension2(ext)
{
	...
	if( tem.toLowerCase() == 'jpg' || tem.toLowerCase() == 'gif' || tem.toLowerCase() == 'png' )
	{
		return true;
	}
	return false;
...
function fn_productDetail(mallDivnCd,prodCd)
{
	...
	if (mallDivnCd == "00002") {
		targetUrl = "http://toysrus.lottemart.com/product/ProductDetailAdmin.do?ProductCD=" + prodCd + "&strCd=307&approvalGbn=N";
	}
	// 몰구분코드가 '00001'(롯데마트몰)인 경우
	else {
		targetUrl = "http://www.lottemart.com/product/ProductDetail.do?ProductCD=" + prodCd + "&strCd=307&approvalGbn=N&previewYN=Y";
	}
...
function imageDetailPrevView(prodCd){
	var targetUrl = context+'/product/imageDetailPrevView.do?prodCd='+prodCd;
	Common.centerPopupWindow(targetUrl, 'insDealProductView', {width:505, height:540, scrollBars:'NO'});
}
