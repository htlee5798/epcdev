...
var  _com_ajax_async_mode_j$Object ;
...
$(document).ready(function() {
	...
	if($("body").has("#h_loading").html() == null){
        var tmpHtml = "";
        tmpHtml  = "<div id='h_loading' style='position: absolute; top: 0; left: 0; display:none;z-index:200'>"; 
        tmpHtml += "<table width=200 height=100 border=0> ";
        tmpHtml += "    <tr>";
        tmpHtml += "        <td align=center><img id='ajax_load_type_img' src='"+ _BASE_JS_ +"/js/jquery/res/loading.gif'></td>";
        tmpHtml += "    </tr>";
        tmpHtml += "</table>";
        tmpHtml += "</div>";
        ...
        $(tmpHtml)
         ...
         .ajaxStart(function() {
             if(_com_ajax_async_mode_j$Object == false){
                 ...
                 $("#ajax_load_type_img").attr("src", _BASE_JS_ + "/js/jquery/res/loading.gif");
             }else{
                 $("#ajax_load_type_img").attr("src",_BASE_JS_ + "/js/jquery/res/loading.gif");
             }
             $(this).css("left", document.body.clientWidth/2 - $(this).width() / 2);
             $(this).css("top", document.body.clientHeight/4 - $(this).height() / 4);
             $(this).show();
         })
        .ajaxStop(function() {
			try{
 ...
 */
function fn$ajax(in_url, in_send_json, in_call_function, async_type){
