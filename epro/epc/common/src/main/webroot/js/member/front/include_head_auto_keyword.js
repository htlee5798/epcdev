	...
	function dq_keywordSearch(keyword)
	{
		...
		location.href="/search/search.do?searchTerm="+escape(encodeURIComponent(keyword));
	}
	...
	{		
 	...
 	function dq_acResultShow(aq, al_f, al_b)
 	{
		if(aq != dqc_trimSpace(dq_searchTextbox.value))
	 	...
	 	if((dq_searchResultList_f != "" && dq_searchResultList_f.length) || (dq_searchResultList_b != "" && dq_searchResultList_b.length))
		 	dq_acListFlag = 1;
	 	else
			dq_acListFlag = 0;
 	...
 	{ 
		...
		if(dq_searchResultList_f[0] == "" && dq_searchResultList_b[0] == "") {
	...
	{		
 	...
 	{	 				
		...
		while(ke.indexOf("\'") != -1)
	 	...
	 	sv_b = "/search/ajax/headGetAutoKeyword.do?p=2&q=" + escape(encodeURIComponent(ke));
 		...
 		if((dpFrontList != null && dpFrontList != "" && dpFrontList.length > 0) || (dpBackList != null && dpBackList != "" && dpBackList.length > 0))
		{
 			...
 			if(dpFrontList != ""){
			 	for(i=0;i<dpFrontList.length;i++)
			 	{
					...
					text += "<p class=\"word\"><a href=\"#\" id=\"dq_alink" + (i+1) + "\">";
					text +=  keywordSumm;
					text +=  "</a></p>";
					text += "<p class=\"hit\">";
					text +=  count;
		 	...
		 	if(dpBackList != ""){
			 	...
			 	for(i = 0;i<temp_len;i++)
			 	{
					...
					text += "<p class=\"word\"><a href=\"#\" id=\"dq_alink" + (i+1+temt_cnt) + "\">";
					text +=  keywordSumm;
					text +=  "</a></p>";
					text += "<p class=\"hit\">";
					text +=  count;
		 	...
		 	text += "<div class=\"more\">";
		 	if(textCheck == 2 && dp == "B"){
		 		text += "<span class=\"t_left\"><a href=\"javascript:dq_printAcResult('F');\" style=\"cursor: pointer\">앞단어 매칭으로 더보기</a></span>";
	 		}else if(textCheck == 2 && dp != "B"){
	 			text += "<span class=\"t_left\"><a href=\"javascript:dq_printAcResult('B')\" style=\"cursor: pointer\">끝단어 매칭으로 더보기</a></span>";
	 		}
		 	text += "<span class=\"t_right\"><a href=\"javascript:dq_acDisplayHide();\" >닫기</a></span>";
