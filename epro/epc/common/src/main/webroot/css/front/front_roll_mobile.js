...
function FUNC_rolling_horizon( obj, objCnt ) {
	...
	for(i=1; i < objMaxCnt; i++) {
		if( jQuery(divObj + i).length ) {
			...
			jQuery(divObj + i).bind('click focusin', function() {
				clearTimeout( thisSetTime );
			});

			jQuery(divObj + i).bind('click focusout', function() {
	...
	for(i=1; i < objMaxCnt; i++) {
		if( jQuery(txtObj + i).length ) {
			...
			jQuery(txtObj + i).bind('click focusin', function() {
				clearTimeout( thisSetTime );
			...
			jQuery(txtObj + i).bind('click focusout', function() {
				thisSetTime = setTimeout(autoRoll, autoRollTime);
			});
	...
	if( jQuery('#' + obj + 'Left').length ) {
		...
		jQuery('#' + obj + 'Left').bind('click focusin', function() {
		...
		jQuery('#' + obj + 'Left').bind('click focusout', function() {
	...
	if( jQuery('#' + obj + 'Right').length ) {
		...
		jQuery('#' + obj + 'Right').bind('click focusin', function() {
			jQuery('#clearResult').html('Y');
			clearTimeout( thisSetTime );
		...
		jQuery('#' + obj + 'Right').bind('click focusout', function() {
			jQuery('#clearResult').html('N');
			thisSetTime = setTimeout(autoRoll, autoRollTime);
...
function FUNC_tabChange( obj, objCnt, ext ) {
	...
	for(i=1; i < imgCnt + 1; i++ ) {
		jQuery(imgObj + i).bind('click focusin', function() {
			clearTimeout( thisSetTime );
		...
		jQuery(imgObj + i).bind('click focusout', function() {
			thisSetTime = setTimeout(function() { auto_tabChange(); }, btnSetTime);
		});
...
function slideMainContent( obj, rollSpeed, setDealy ) {
	if( jQuery(obj).length ) {
		...
		jQuery(obj).parent().bind('click focusin', function() {
			clearTimeout( mainRollObj );
		});

		jQuery(obj).parent().bind('click focusout', function() {
			mainRollObj = setTimeout(function() { autoRoll(); }, setDealy);
		});
...
function mainContentBtn( obj, targetId ) {
	if( jQuery( obj ).attr('effect') != 'Y' ) {
		jQuery( obj ).bind('click focusin', function(event) {
			jQuery('#' + targetId).show();
		});

		jQuery( obj ).bind('click focusout', function(event) {
