...
// 2014.10.28 lnb_resize
function lnb_resize(){
	var winH = $(window).height();
	var objH = $('.section_lnb').outerHeight(true) + $('#UxHeader').outerHeight(true);
	//윈도우높이가 lnb+header 높이보다 낮을때
	if(winH < objH){
		$('.title2 + .depth_menu > li > .depth_menu2').each(function(){
			var objTop = $(window).height() - $(this).parent().offset().top;
			var objH = $(this).outerHeight(true);
			if(objTop < objH){
				var toplength = $(this).outerHeight(true) / 1.4;
				$(this).css({'top':-toplength});
			}
		});
		$('.title2 + .depth_menu > li > .depth_menu2 > ul > li').mouseenter(function(){
			$(this).children('div').each(function(){
				var objTop02 = $(window).height() - $(this).parent().offset().top;
				var objH02 = $(this).outerHeight(true);
				if(objTop02 < objH02){
					var toplength02 = $(this).outerHeight(true) / 1.25;
					$(this).css({'top':-toplength02});
				}
			});
		});
	} else {
		$('.title2 + .depth_menu > li > .depth_menu2, .title2 + .depth_menu > li > .depth_menu3').css({'top':'-17px'});
	}
}
...
function category(){
	$('.section_cate_all > h2 > a').click(function(event){
		...
		var objH = $('.section_cate_all .cont-wrap').height();
		if(objH == '0'){
			$(this).parent().addClass('close');
			$(this).children('i').text('닫기');
			$(this).parent().next().addClass('open').animate({height:'656px', '.height':'672px', easing:'easeInQuad'}, 300);
	...
	$('.section_cate_other > .menu').bxSlider({
		...
		onAfterSlide:function(){
			$('.section_cate_other .menu > li > a, .section_cate_other > .btn').click(function(event){
				...
				$('.section_cate_other .menuplus > div > h3 > a').click(function(){
					$('.section_cate_other .menuplus').hide();
				});
			});
		}
	});
...
// 2014.10 gnb
function gnb(){
	$('.section_cate_head > ul > li > a').each(function(){
		var obj = $(this).children('span').outerWidth(true) + 40;
		$(this).parent().css({'width':obj});
	});
}
...
function left_tip_banner(){
	var obj = $('.left_Tip_banner li').index();
	if(obj > 0){
		$('.left_Tip_banner').bxSlider({
			pager:false
		});
	}
}
